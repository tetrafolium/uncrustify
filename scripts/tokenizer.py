...
class Tokenizer:
        ...
        try:
            while self.text_idx < len(self.text):
                if self.parse_whitespace():
                    continue
                elif self.text[self.text_idx] == '\\' and self.text[
                        self.text_idx + 1] == '\n':
                    self.text_idx += 2
                    continue
    ...
    def parse_comment(self):
        if not self.text[self.text_idx] == '/' or not self.text[self.text_idx +
                                                                1] in '/*':
            return False
        if self.text[self.text_idx + 1] == '/':
            ...
            while self.text_idx < len(self.text) - 1:
                if self.text[self.text_idx] == '*' and self.text[self.text_idx
                                                                 + 1] == '/':
                    self.text_idx += 2
                    break
                self.text_idx += 1
    ...
    def parse_identifier(self):
        if not self.text[self.
                         text_idx].upper() in '@_ABCDEFGHIJKLMNOPQRSTUVWXYZ':
            return False
        start_idx = self.text_idx
    ...
    def parse_number(self):
        ...
        if not ch.isdigit() and (ch != '.' or
                                 not self.text[self.text_idx + 1].isdigit()):
            return False
