...
def write_conversions(out, args):
    ...
    for enum in enums.values():
        ...
        out.write(header)
        out.write(u'bool convert_string(const char *in, {} &out)\n'.format(
            enum.name))
        out.write(u'{\n' u'   if (false)\n' u'   {\n' u'   }\n')

        for v in enum.values:
            for a in enum.value_aliases[v]:
                out.write(u'   else if (strcasecmp(in, "{}") == 0)\n'
                          u'   {{\n'
                          u'      out = {};\n'
                          u'      return(true);\n'
...
def main():
    ...
    parser.add_argument(
        'output', type=str, help='location of options.cpp to write')
    parser.add_argument(
        'header', type=str, help='location of options.h to read')
    parser.add_argument(
        'template',
        type=str,
