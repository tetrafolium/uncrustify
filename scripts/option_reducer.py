...
def print_config(config_list, target_file_obj=stdout, exclude_idx=()):
    ...
    if exclude_idx[0] != 0:
        print("%s = %s" %
              (config_list[0][0].ljust(31, ' '), config_list[0][1]),
              end='',
...
if __name__ == "__main__":
    ...
    group_general.add_argument(
        ...
        type=lambda x: valid_file(
            arg_parser, x, "../build/uncrustify.exe",
            "../build/Debug/uncrustify", "../build/Debug/uncrustify.exe",
            "../build/Release/uncrustify", "../build/Release/uncrustify.exe"),
    ...
    group_reduce.add_argument('-j',
                              '--jobs',
                              metavar='<nr>',
                              type=int,
                              default=cpu_count(),
                              help='Number of concurrent jobs.')
    group_reduce.add_argument('-p',
                              '--passes',
                              metavar='<nr>',
                              type=int,
                              default=5,
