...
/** Step through all chunks, altering newlines inside parens of if/for/while/do
 * as needed. Handles the style options: nl_multi_line_sparen_open,
 * nl_multi_line_sparen_close, nl_before_if_closing_paren
 */
 ...
 * the closing paren are altered to different lines, as per the respective
 * opening parens. In the given example, first 2 opening paren are in same line,
 * hence the respective closing paren are put in the same line. input:
 * func1(func2(
 *    func3(
 *       func4(
 *       )
 ...
 * exactly 1 newline before (UO_pos_comma == TRAIL) or after (UO_pos_comma ==
 * LEAD). We can't remove a newline if it is right before a preprocessor.
 */
 ...
 * @param  newlines  reference in which the amount of newlines will be written
 * to (will be initialized with 0)
