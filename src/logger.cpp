...
struct log_buf {
  log_buf()
      : log_file(nullptr), sev(LSYS), in_log(0), buf_len(0), show_hdr(false) {
    bufX.clear();
    bufX.resize(256);
  }
...
void log_show_sev(bool show) { g_log.show_hdr = show; }

bool log_sev_on(log_sev_t sev) { return (logmask_test(g_log.mask, sev)); }

void log_set_sev(log_sev_t sev, bool value) {
  logmask_set_sev(g_log.mask, sev, value);
}

void log_set_mask(const log_mask_t &mask) { g_log.mask = mask; }

void log_get_mask(log_mask_t &mask) { mask = g_log.mask; }
...
static size_t log_start(log_sev_t sev) {
  ...
  if (!g_log.in_log && g_log.show_hdr) {
    g_log.buf_len = static_cast<size_t>(
        snprintf(&g_log.bufX[0], g_log.bufX.size(), "<%d>", sev));
  }
...
#define BUFFERLENGTH 200
  ...
  while (true) {
    ...
    size_t lenX = static_cast<size_t>(
        vsnprintf(&g_log.bufX[g_log.buf_len], cap, buf, args));
    va_end(args);
...
log_func::~log_func() { g_fq.pop_back(); }
