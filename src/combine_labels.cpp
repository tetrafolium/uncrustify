...
chunk_t *chunk_get_next_local(chunk_t *pc, scope_e scope = scope_e::ALL) {
	...
	do {
		tmp = chunk_get_next(tmp, scope);
	         ...
	         (chunk_is_comment(tmp) || chunk_is_token(tmp, CT_NOEXCEPT)));
...
chunk_t *chunk_get_prev_local(chunk_t *pc, scope_e scope = scope_e::ALL) {
	...
	} while (tmp != nullptr && (chunk_is_comment(tmp) || chunk_is_newline(tmp) ||
...
void combine_labels(void) {
	...
	while (next != nullptr) {
		if (chunk_is_token(next, CT_NEWLINE)) {
			LOG_FMT(LFCN,
			        "%s(%d): next->orig_line is %zu, next->orig_col is %zu, "
			        "<Newline>, nl is %zu\n",
			...
			LOG_FMT(LFCN,
			        "%s(%d): next->orig_line is %zu, next->orig_col is %zu, text() "
			        "'%s'\n",
			        __func__, __LINE__, next->orig_line, next->orig_col,
			        next->text());
		}
		...
		if (!next->flags.test(
			    PCF_IN_OC_MSG) // filter OC case of [self class] msg send
		    &&
		    (chunk_is_token(next, CT_CLASS) || chunk_is_token(next, CT_OC_CLASS) ||
		...
		if (chunk_is_token(next, CT_QUESTION) &&
			...
			if (cs_top_is_question(cs, next->level)) {
				...
				LOG_FMT(
					LFCN,
					"%s(%d): cur->text() is '%s', orig_line is %zu, orig_col is %zu\n",
					__func__, __LINE__, cur->text(), cur->orig_line, cur->orig_col);
				LOG_FMT(LFCN,
				        "%s(%d): next->text() is '%s', orig_line is %zu, orig_col is "
				        "%zu\n\n",
				        __func__, __LINE__, next->text(), next->orig_line,
				        next->orig_col);
				...
				if (language_is_set(LANG_PAWN)) {
					...
					LOG_FMT(LFCN, "%s(%d): orig_line is %zu, orig_col is %zu, tmp '%s': ",
					        ...
					        (tmp->type == CT_NEWLINE) ? "<Newline>" : tmp->text());
					...
					if (next->flags.test(PCF_IN_FCN_CALL)) {
						...
						set_chunk_type(next, CT_LABEL_COLON);
					           ...
					           (tmp->type != CT_NUMBER && tmp->type != CT_DECLTYPE &&
					            tmp->type != CT_SIZEOF &&
					            get_chunk_parent_type(tmp) != CT_SIZEOF &&
					            !tmp->flags.test_any(PCF_IN_STRUCT | PCF_IN_CLASS)) ||
					           chunk_is_token(tmp, CT_NEWLINE)) {
						/*
						 * the CT_SIZEOF isn't great - test 31720 happens to use a sizeof
						 * expr, but this really should be able to handle any constant expr
						 */
						// Fix for #1242
					...
					} else if (next->flags.test_any(PCF_IN_STRUCT | PCF_IN_CLASS |
					                                PCF_IN_TYPEDEF)) {
					...
					LOG_FMT(LFCN,
					        "%s(%d): nextprev->text() is '%s', orig_line is %zu, "
					        "orig_col is %zu, type is %s\n",
					        __func__, __LINE__, nextprev->text(), nextprev->orig_line,
					        nextprev->orig_col, get_token_name(nextprev->type));
					LOG_FMT(LFCN,
					        "%s(%d): next->text() is '%s', orig_line is %zu, orig_col is "
					        "%zu, type is %s\n",
					        __func__, __LINE__, next->text(), next->orig_line,
					        next->orig_col, get_token_name(next->type));
					...
					{
						LOG_FMT(LFCN,
						        "%s(%d): tmp->text() is '%s', orig_line is %zu, orig_col "
						        "is %zu, type is %s\n",
						        __func__, __LINE__, tmp->text(), tmp->orig_line,
						        tmp->orig_col, get_token_name(tmp->type));
						...
						if (chunk_is_token(tmp, CT_BASE) || chunk_is_token(tmp, CT_THIS)) {
							...
							LOG_FMT(LWARN,
							        "%s(%d): %s:%zu unexpected colon in col %zu n-parent=%s "
							        "c-parent=%s l=%zu bl=%zu\n",
							        __func__, __LINE__, cpd.filename.c_str(), next->orig_line,
							        next->orig_col,
							        get_token_name(get_chunk_parent_type(next)),
							        get_token_name(get_chunk_parent_type(cur)), next->level,
							        next->brace_level);
							cpd.error_count++;
						}
					}
