...
void align_init_brace(chunk_t *start) {
  ...
  if (pcSingle == nullptr || (chunk_is_token(pcSingle, CT_BRACE_CLOSE) &&
  ...
  if (options::align_on_tabstop() && cpd.al_cnt >= 1 &&
  ...
  do {
    ...
    if (idx < cpd.al_cnt) {
      LOG_FMT(LALBR, " (%zu) check %s vs %s -- ", idx, get_token_name(pc->type),
        ...
        if (chunk_is_token(pc, CT_COMMA)) {
          ...
          if (next != nullptr && !chunk_is_newline(next)) {
            ...
            if ((idx < (cpd.al_cnt - 1)) && options::align_number_right() &&
              ...
              LOG_FMT(LALBR,
                      "%s(%d): idx is %zu, al_cnt is %zu, cpd.al[%zu].col is "
                      "%zu, cpd.al[%zu].len is %zu\n",
                      __func__, __LINE__, idx, cpd.al_cnt, idx, cpd.al[idx].col,
                      idx, cpd.al[idx].len);
