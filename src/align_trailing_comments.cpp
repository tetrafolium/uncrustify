	...
	if ((cs.Len() > 1) || (align_single && (cs.Len() == 1))) {
		...
		while ((pc = cs.Pop_Back()) != nullptr) {
			...
			LOG_FMT(sev, "%s(%d): indented [%s] on line %zu to %zu\n", __func__,
			        __LINE__, pc->text(), pc->orig_line, pc->column);
		}
	}
	cs.Reset();
} // align_stack
...
chunk_t *align_trailing_comments(chunk_t *start) {
	...
	while (pc != nullptr && (nl_count < options::align_right_cmt_span())) {
		if (pc->flags.test(PCF_RIGHT_COMMENT) && pc->column > 1) {
			...
			if (cmt_type_cur == cmt_type_start) {
				LOG_FMT(LALADD,
				        "%s(%d): line=%zu min_col=%zu pc->col=%zu pc->len=%zu %s\n",
				        __func__, __LINE__, pc->orig_line, min_col, pc->column,
				        pc->len(), get_token_name(pc->type));
	...
	LOG_FMT(
		LALADD,
		"%s(%d):  -- min_orig=%zu intended_col=%zu min_allowed=%zu ==> col=%zu\n",
...
comment_align_e get_comment_align_type(chunk_t *cmt) {
	...
	if (!options::align_right_cmt_mix() &&
		...
		if (chunk_is_token(prev, CT_PP_ENDIF) || chunk_is_token(prev, CT_PP_ELSE) ||
		    chunk_is_token(prev, CT_ELSE) || chunk_is_token(prev, CT_BRACE_CLOSE)) {
			// TODO: make the magic 3 configurable
			if ((cmt->column - (prev->column + prev->len())) < 3) {
				cmt_type = (chunk_is_token(prev, CT_PP_ENDIF)) ? comment_align_e::ENDIF
				           : comment_align_e::BRACE;
			}
		}
	}
	return (cmt_type);
...
void align_right_comments(void) {
	...
	for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
		if (chunk_is_token(pc, CT_COMMENT) || chunk_is_token(pc, CT_COMMENT_CPP) ||
			...
			if (get_chunk_parent_type(pc) == CT_COMMENT_END) {
				...
				if (pc->orig_col <
				    prev->orig_col_end + options::align_right_cmt_gap()) {
					LOG_FMT(LALTC,
					        "NOT changing END comment on line %zu (%zu <= %zu + %u)\n",
					        pc->orig_line, pc->orig_col, prev->orig_col_end,
					        options::align_right_cmt_gap());
				} else {
					LOG_FMT(LALTC,
					        "Changing END comment on line %zu into a RIGHT-comment\n",
					        pc->orig_line);
			...
			if (get_chunk_parent_type(pc) == CT_COMMENT_WHOLE) {
				...
				if (pc->column >= max_col) {
					LOG_FMT(LALTC,
					        "Changing WHOLE comment on line %zu into a RIGHT-comment "
					        "(col=%zu col_ind=%zu max_col=%zu)\n",
