...
chunk_t *pawn_add_vsemi_after(chunk_t *pc) {
  ...
  if (next != nullptr && (chunk_is_token(next, CT_VSEMICOLON) ||
  ...
  LOG_FMT(LPVSEMI, "%s: Added VSEMI on line %zu, prev='%s' [%s]\n", __func__,
          pc->orig_line, pc->text(), get_token_name(pc->type));
...
static bool pawn_continued(chunk_t *pc, size_t br_level) {
  ...
  if (pc->level > br_level || chunk_is_token(pc, CT_ARITH) ||
      chunk_is_token(pc, CT_CARET) || chunk_is_token(pc, CT_QUESTION) ||
      chunk_is_token(pc, CT_BOOL) || chunk_is_token(pc, CT_ASSIGN) ||
      chunk_is_token(pc, CT_COMMA) || chunk_is_token(pc, CT_COMPARE) ||
      chunk_is_token(pc, CT_IF) || chunk_is_token(pc, CT_ELSE) ||
      chunk_is_token(pc, CT_DO) || chunk_is_token(pc, CT_SWITCH) ||
      chunk_is_token(pc, CT_WHILE) || chunk_is_token(pc, CT_BRACE_OPEN) ||
...
void pawn_prescan(void) {
  ...
  while (pc != nullptr) {
    if (did_nl && pc->type != CT_PREPROC && !chunk_is_newline(pc) &&
        pc->level == 0) {
...
static chunk_t *pawn_process_line(chunk_t *start) {
  ...
  while (((pc = chunk_get_next_nc(pc)) != nullptr) &&
         !chunk_is_str(pc, "(", 1) && pc->type != CT_ASSIGN &&
         pc->type != CT_NEWLINE) {
    if (pc->level == 0 &&
        (chunk_is_token(pc, CT_FUNCTION) || chunk_is_token(pc, CT_WORD) ||
  ...
  //        __func__, start->orig_line, start->text(),
...
static chunk_t *pawn_process_variable(chunk_t *start) {
  ...
  while ((pc = chunk_get_next_nc(pc)) != nullptr) {
    if (chunk_is_token(pc, CT_NEWLINE) && prev != nullptr // Issue 2586
...
void pawn_add_virtual_semicolons(void) {
  ...
  if (language_is_set(LANG_PAWN)) {
    ...
    while ((pc = chunk_get_next(pc)) != nullptr) {
      ...
      if (prev == nullptr ||
          (pc->type != CT_NEWLINE && pc->type != CT_BRACE_CLOSE &&
      ...
      if (!prev->flags.test(PCF_IN_PREPROC) &&
          ...
          prev->type != CT_VSEMICOLON && prev->type != CT_SEMICOLON &&
  ...
  if (start == fcn) {
    ...
    if (chunk_is_token(start, CT_FORWARD) || chunk_is_token(start, CT_NATIVE)) {
      LOG_FMT(LPFUNC, "%s: %zu] '%s' [%s] proto due to %s\n", __func__,
              fcn->orig_line, fcn->text(), get_token_name(fcn->type),
              get_token_name(start->type));
      set_chunk_type(fcn, CT_FUNC_PROTO);
...
static chunk_t *pawn_process_func_def(chunk_t *pc) {
  ...
  if (last != nullptr) {
    LOG_FMT(LPFUNC, "%s: %zu] last is '%s' [%s]\n", __func__, last->orig_line,
            last->text(), get_token_name(last->type));
  }
...
chunk_t *pawn_check_vsemicolon(chunk_t *pc) {
  ...
  if (prev == nullptr || prev == vb_open || prev->flags.test(PCF_IN_PREPROC) ||
      pawn_continued(prev, vb_open->level + 1)) {
    if (prev != nullptr) {
      LOG_FMT(LPVSEMI, "%s:  no  VSEMI on line %zu, prev='%s' [%s]\n", __func__,
              prev->orig_line, prev->text(), get_token_name(prev->type));
    }
    return (pc);
  }
