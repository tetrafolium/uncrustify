 ...
 */
enum class direction_e : unsigned int { FORWARD, BACKWARD };
...
static chunk_t *chunk_search(chunk_t *cur, const check_t check_fct,
                             ...
                             const direction_e dir = direction_e::FORWARD,
 ...
 */
static chunk_t *chunk_ppa_search(chunk_t *cur, const check_t check_fct,
                                 const bool cond = true);
 ...
 *
 * Traverses a chunk list in the specified direction until a chunk of a given
 * type is found.
 *
 * This function is a specialization of chunk_search.
 *
 ...
 * @retval chunk_t  pointer to the found chunk
 */
static chunk_t *chunk_search_typelevel(chunk_t *cur, c_token_t type,
                                       scope_e scope = scope_e::ALL,
                                       direction_e dir = direction_e::FORWARD,
                                       int level = -1);
 ...
 * @retval chunk_t  pointer to the found chunk
 */
static chunk_t *chunk_search_str(chunk_t *cur, const char *str, size_t len,
 ...
 */
static chunk_t *chunk_add(const chunk_t *pc_in, chunk_t *ref,
                          const direction_e pos = direction_e::FORWARD);
...
static chunk_t *chunk_search(chunk_t *cur, const check_t check_fct,
  ...
  do // loop over the chunk list
  {
    pc = search_function(pc, scope); // in either direction while
           ...
           && (check_fct(pc) !=
               cond)); // and the demanded chunk was not found either
...
static chunk_t *chunk_ppa_search(chunk_t *cur, const check_t check_fct,
  ...
  if (cur && !cur->flags.test(PCF_IN_PREPROC)) {
    ...
    return (
        chunk_search(cur, check_fct, scope_e::ALL, direction_e::FORWARD, cond));
  }
  chunk_t *pc = cur;
...
static void chunk_log_msg(chunk_t *chunk, const log_sev_t log,
                          const char *str) {
  LOG_FMT(log, "%s orig_line is %zu, orig_col is %zu, ", str, chunk->orig_line,
          chunk->orig_col);
...
static void chunk_log(chunk_t *pc, const char *text) {
  if (pc != nullptr && (cpd.unc_stage != unc_stage_e::TOKENIZE) &&
    ...
    LOG_FMT(log, " stage is %s", get_unc_stage_name(cpd.unc_stage));
...
chunk_t *chunk_get_next_nl(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_newline, scope, direction_e::FORWARD, true));
}

chunk_t *chunk_get_prev_nl(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_newline, scope, direction_e::BACKWARD, true));
}

chunk_t *chunk_get_next_nnl(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_newline, scope, direction_e::FORWARD, false));
}

chunk_t *chunk_get_prev_nnl(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_newline, scope, direction_e::BACKWARD, false));
}

chunk_t *chunk_get_next_ncnl(chunk_t *cur, scope_e scope) {
  return (chunk_search(cur, chunk_is_comment_or_newline, scope,
                       direction_e::FORWARD, false));
}
...
chunk_t *chunk_get_next_nblank(chunk_t *cur, scope_e scope) {
  return (chunk_search(cur, chunk_is_comment_newline_or_blank, scope,
                       direction_e::FORWARD, false));
}

chunk_t *chunk_get_prev_nblank(chunk_t *cur, scope_e scope) {
  return (chunk_search(cur, chunk_is_comment_newline_or_blank, scope,
                       direction_e::BACKWARD, false));
}

chunk_t *chunk_get_next_nc(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_comment, scope, direction_e::FORWARD, false));
}

chunk_t *chunk_get_next_nisq(chunk_t *cur, scope_e scope) {
  return (chunk_search(cur, chunk_is_balanced_square, scope,
                       direction_e::FORWARD, false));
}
...
chunk_t *chunk_get_prev_ncnlni(chunk_t *cur, scope_e scope) {
  return (chunk_search(cur, chunk_is_comment_or_newline_or_ignored, scope,
                       direction_e::BACKWARD, false));
}

chunk_t *chunk_get_prev_nc(chunk_t *cur, scope_e scope) {
  return (
      chunk_search(cur, chunk_is_comment, scope, direction_e::BACKWARD, false));
}

chunk_t *chunk_get_next_type(chunk_t *cur, c_token_t type, int level,
                             scope_e scope) {
  return (
      chunk_search_typelevel(cur, type, scope, direction_e::FORWARD, level));
}
...
chunk_t *chunk_get_prev_type(chunk_t *cur, c_token_t type, int level,
                             scope_e scope) {
  return (
      chunk_search_typelevel(cur, type, scope, direction_e::BACKWARD, level));
}
...
void chunk_swap(chunk_t *pc1, chunk_t *pc2) { g_cl.Swap(pc1, pc2); }
...
chunk_t *chunk_get_next_nvb(chunk_t *cur, const scope_e scope) {
  return (
      chunk_search(cur, chunk_is_vbrace, scope, direction_e::FORWARD, false));
}

chunk_t *chunk_get_prev_nvb(chunk_t *cur, const scope_e scope) {
  return (
      chunk_search(cur, chunk_is_vbrace, scope, direction_e::BACKWARD, false));
}
  ...
  if (pc != nullptr) {
    ...
    if (pc->flags != nflags) {
      LOG_FMT(LSETFLG,
              ...
              __func__, __LINE__, static_cast<pcf_flags_t::int_t>(pc->flags),
              static_cast<pcf_flags_t::int_t>(pc->flags ^ nflags),
              static_cast<pcf_flags_t::int_t>(nflags), pc->orig_line,
              pc->orig_col, pc->text(), get_token_name(pc->type));
...
void set_chunk_type_real(chunk_t *pc, c_token_t token, const char *func,
                         int line) {
  LOG_FUNC_ENTRY();

  if (pc == nullptr || pc->type == token) {
    return;
  }
  ...
  LOG_FMT(LSETTYP,
          "   pc->type is %s, pc->parent_type is %s => *type is %s, "
          "*parent_type is %s\n",
...
void set_chunk_parent_real(chunk_t *pc, c_token_t token, const char *func,
                           int line) {
  LOG_FUNC_ENTRY();

  if (pc == nullptr || get_chunk_parent_type(pc) == token) {
    return;
  }
...
static chunk_t *chunk_get_ncnlnp(chunk_t *cur, const scope_e scope,
                                 const direction_e dir) {
  chunk_t *pc = cur;

  pc = (chunk_is_preproc(pc) == true)
           ? chunk_search(pc, chunk_is_comment_or_newline_in_preproc, scope,
                          dir, false)
           : chunk_search(pc, chunk_is_comment_newline_or_preproc, scope, dir,
                          false);
  return (pc);
}
...
static chunk_t *chunk_add(const chunk_t *pc_in, chunk_t *ref,
  ...
  if (pc != nullptr) {
    ...
    {
      (pos == direction_e::FORWARD) ? g_cl.AddAfter(pc, ref)
...
static chunk_t *chunk_skip_dc_member(chunk_t *start, scope_e scope,
                                     direction_e dir) {
  LOG_FUNC_ENTRY();
  ...
  const auto step_fcn =
      (dir == direction_e::FORWARD) ? chunk_get_next_ncnl : chunk_get_prev_ncnl;
