...
public:
   ...
   */
  T *GetNext(T *ref) { return ((ref != NULL) ? ref->next : NULL); }
   ...
   * @param[in] ref  pointer to current list element
   *
   * @return pointer to previous element or nullptr if no previous element
   * exists
   */
  T *GetPrev(T *ref) { return ((ref != NULL) ? ref->prev : NULL); }
