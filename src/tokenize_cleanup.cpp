 ...
 * Will mark 'test4Setter'and ':' in '@property (setter=test4Setter:, strong)
 * int test4;' as CT_OC_SEL_NAME
 */
...
void tokenize_trailing_return_types(void) {
  ...
  for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next_ncnl(pc)) {
    ...
    if (chunk_is_token(pc, CT_MEMBER) && (strcmp(pc->text(), "->") == 0)) {
...
void tokenize_cleanup(void) {
  ...
  for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next_ncnl(pc)) {
    ...
    if (chunk_is_token(pc, CT_SEMICOLON) && pc->flags.test(PCF_IN_PREPROC) &&
        !chunk_get_next_ncnl(pc, scope_e::PREPROC)) {
      LOG_FMT(LNOTE,
              "%s(%d): %s:%zu Detected a macro that ends with a semicolon. "
              "Possible failures if used.\n",
  ...
  while (pc != nullptr && next != nullptr) {
    ...
    if (chunk_is_token(pc, CT_ENUM) && chunk_is_token(next, CT_CLASS)) {
    ...
    if (chunk_is_token(next, CT_WORD)) {
      if (chunk_is_token(pc, CT_ENUM) || chunk_is_token(pc, CT_ENUM_CLASS) ||
          chunk_is_token(pc, CT_UNION) || chunk_is_token(pc, CT_STRUCT)) {
    ...
    if ((chunk_is_token(next, CT_STAR)) ||
        ...
        (language_is_set(LANG_CS) && (chunk_is_token(next, CT_QUESTION)) &&
         (strcmp(pc->text(), "null") != 0))) {
      if (chunk_is_token(pc, CT_TYPE) || chunk_is_token(pc, CT_QUALIFIER) ||
    ...
    if (chunk_is_token(pc, CT_ANGLE_OPEN) && pc->parent_type != CT_TYPE_CAST) {
       ...
       */
      if (language_is_set(LANG_OC | LANG_CPP | LANG_CS | LANG_JAVA |
                          LANG_VALA)) {
        // bug #663
        check_template(pc);
      } else {
    ...
    if (language_is_set(LANG_D)) {
      ...
      if (chunk_is_token(pc, CT_INV) &&
          (chunk_is_token(prev, CT_STRING) || chunk_is_token(prev, CT_WORD) ||
           chunk_is_token(next, CT_STRING))) {
        set_chunk_type(pc, CT_CONCAT);
      }

      // Check for the D template symbol '!' (word + '!' + word or '(')
      if (chunk_is_token(pc, CT_NOT) && chunk_is_token(prev, CT_WORD) &&
      ...
      if (chunk_is_token(pc, CT_IF) && chunk_is_str(prev, "static", 6)) {
    ...
    if (language_is_set(LANG_CPP)) {
      ...
      if (chunk_is_token(pc, CT_WORD) && chunk_is_token(next, CT_DC_MEMBER)) {
        set_chunk_type(pc, CT_TYPE);
      }
    }

    // Change get/set to CT_WORD if not followed by a brace open
    if (chunk_is_token(pc, CT_GETSET) && next->type != CT_BRACE_OPEN) {
     ...
     */
    if (chunk_is_token(pc, CT_CLASS) && !CharTable::IsKw1(next->str[0])) {
          ...
          chunk_is_not_token(next, CT_ATTRIBUTE)) // Issue #2570
      {
        ...
        if (chunk_is_token(
                next2,
                CT_INV) // CT_INV hasn't turned into CT_DESTRUCTOR just yet
            || (chunk_is_token(
                    next2,
                    CT_CLASS) // constructor isn't turned into CT_FUNC* just yet
    ...
    if (chunk_is_token(pc, CT_OPERATOR)) {
      ...
      if (chunk_is_token(next, CT_PAREN_OPEN)) {
        ...
        while ((tmp = chunk_get_next(tmp2)) != nullptr) {
          if (tmp->type != CT_WORD && tmp->type != CT_TYPE &&
              tmp->type != CT_QUALIFIER && tmp->type != CT_STAR &&
              tmp->type != CT_CARET && tmp->type != CT_AMP &&
              tmp->type != CT_TSQUARE) {
            break;
          }
      ...
      LOG_FMT(LOPERATOR, "%s(%d): %zu:%zu operator '%s'\n", __func__, __LINE__,
              pc->orig_line, pc->orig_col, next->text());
    }
    ...
    if ((chunk_is_str_case(pc, "EXEC", 4) &&
        ...
        ((*pc->str.c_str() == '$') &&
         ...
         && (!language_is_set(LANG_CS) ||
             ((chunk_is_token(pc, CT_STRING)) && (!pc->str.startswith("$\"")) &&
              (!pc->str.startswith("$@\"")))))) {
    ...
    if (chunk_is_token(pc, CT_FOR) && chunk_is_str(next, "each", 4) &&
        (next == chunk_get_next(pc))) {
    ...
    if (language_is_set(LANG_OC)) {
      if ((chunk_is_token(pc, CT_IF) || chunk_is_token(pc, CT_FOR) ||
    ...
    if (chunk_is_token(pc, CT_OC_IMPL) || chunk_is_token(pc, CT_OC_INTF) ||
    ...
    if (chunk_is_token(pc, CT_OC_INTF)) {
      ...
      while (tmp != nullptr && tmp->type != CT_OC_END) {
        if (get_token_pattern_class(tmp->type) != pattern_class_e::NONE) {
          LOG_FMT(LOBJCWORD,
                  "%s(%d): @interface %zu:%zu change '%s' (%s) to CT_WORD\n",
     ...
     */
    if (chunk_is_token(pc, CT_OC_SEL) && chunk_is_token(next, CT_PAREN_OPEN)) {
      set_chunk_parent(next, pc->type);
    ...
    if (chunk_is_token(pc, CT_PP_PRAGMA) &&
      ...
      {
        set_chunk_type(pc, (*next->str.c_str() == 'r') ? CT_PP_REGION
                                                       : CT_PP_ENDREGION);
    ...
    if (language_is_set(LANG_CS) && chunk_is_token(pc, CT_DEFAULT) &&
        chunk_is_token(next, CT_PAREN_OPEN)) {
    ...
    if (chunk_is_token(pc, CT_BOOL) && language_is_set(LANG_CPP) &&
        chunk_is_str(pc, "&&", 2)) {
     ...
     *   A::A(int) try : B() { } catch (...) { }
     */
    if (chunk_is_token(pc, CT_TRY) && chunk_is_str(pc, "try", 3) &&
     ...
     * a qualifier.
     */
    if (language_is_set(LANG_JAVA) && chunk_is_token(pc, CT_SYNCHRONIZED) &&
        next->type != CT_PAREN_OPEN) {
      set_chunk_type(pc, CT_QUALIFIER);
    }

    // change CT_DC_MEMBER + CT_FOR into CT_DC_MEMBER + CT_FUNC_CALL
    if (chunk_is_token(pc, CT_FOR) && chunk_is_token(pc->prev, CT_DC_MEMBER)) {
...
static void check_template(chunk_t *start) {
  ...
  if (chunk_is_token(prev, CT_TEMPLATE)) {
    ...
    for (pc = chunk_get_next_ncnl(start, scope_e::PREPROC); pc != nullptr;
         pc = chunk_get_next_ncnl(pc, scope_e::PREPROC)) {
      LOG_FMT(LTEMPL, "%s(%d): type is %s, level is %zu\n", __func__, __LINE__,
              get_token_name(pc->type), level);

      if ((pc->str[0] == '>') && (pc->len() > 1)) {
        if (pc->str[1] == '=') // Issue #1462 and #2565
        {
          LOG_FMT(LTEMPL,
                  "%s(%d): do not split '%s' at orig_line %zu, orig_col %zu\n",
                  __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
        } else {
          LOG_FMT(LTEMPL,
                  "%s(%d): {split '%s' at orig_line %zu, orig_col %zu}\n",
                  __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
      ...
      if (parens == 0) {
        if (chunk_is_str(pc, "<", 1)) {
          ...
          if (level == 0) {
            fprintf(stderr,
                    "%s(%d): level is ZERO, cannot be decremented, at line "
                    "%zu, column %zu\n",
    ...
    if (prev->type != CT_WORD && prev->type != CT_TYPE &&
        prev->type != CT_COMMA && prev->type != CT_QUALIFIER &&
    ...
    while ((pc = chunk_get_prev_ncnl(pc, scope_e::PREPROC)) != nullptr) {
      ...
      if (((chunk_is_token(pc, CT_IF) || chunk_is_token(pc, CT_RETURN) ||
            chunk_is_token(pc, CT_WHILE) ||
...
#define MAX_NUMBER_OF_TOKEN 1024
    ...
    for (pc = chunk_get_next_ncnl(start, scope_e::PREPROC); pc != nullptr;
      ...
      LOG_FMT(LTEMPL, "%s(%d): type is %s, num_tokens is %zu\n", __func__,
              __LINE__, get_token_name(pc->type), num_tokens);

      log_rule_B("tok_split_gte");

      if ((tokens[num_tokens - 1] == CT_ANGLE_OPEN) && (pc->str[0] == '>') &&
          (pc->len() > 1) &&
      ...
      if (chunk_is_str(pc, "<", 1)) {
      ...
      } else if (in_if && (chunk_is_token(pc, CT_BOOL) ||
        ...
        if (num_tokens == 0) {
          fprintf(stderr,
                  "%s(%d): num_tokens is ZERO, cannot be decremented, at line "
                  "%zu, column %zu\n",
                  __func__, __LINE__, pc->orig_line, pc->orig_col);
          log_flush(true);
          exit(EX_SOFTWARE);
...
static void check_template_arg(chunk_t *start, chunk_t *end) {
  ...
  LOG_FMT(LTEMPL, "%s(%d):     to   orig_line %zu, orig_col %zu\n", __func__,
          __LINE__, end->orig_line, end->orig_col);
  ...
  while (pc != end) {
    ...
    if (next->type != CT_PAREN_OPEN) {
      if (chunk_is_token(pc, CT_NUMBER) || chunk_is_token(pc, CT_ARITH)) {
        expressionIsNumeric = true;
        break;
      }
    }
