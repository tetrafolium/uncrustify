...
static bool paren_multiline_before_brace(chunk_t *brace) {
    ...
    auto paren_close =
        chunk_get_prev_type(brace, paren_t, brace->level, scope_e::ALL);
    auto paren_open = chunk_skip_to_match_rev(paren_close, scope_e::ALL);

    if (paren_close == nullptr || paren_open == nullptr || paren_close == brace ||
...
void do_braces(void) {
    ...
    while ((pc = chunk_get_next_ncnl(pc)) != nullptr) {
        ...
        chunk_t *br_open = pc;
        const c_token_t brc_type =
            c_token_t(pc->type + 1); // corresponds to closing type
        // Detect empty bodies
    ...
    if ((options::mod_full_brace_if() | options::mod_full_brace_do() |
            options::mod_full_brace_for() | options::mod_full_brace_using() |
            options::mod_full_brace_while()) &
            IARF_REMOVE) {
        examine_braces();
    }
    ...
    if ((options::mod_full_brace_if() | options::mod_full_brace_do() |
            options::mod_full_brace_for() | options::mod_full_brace_function() |
            options::mod_full_brace_using() | options::mod_full_brace_while()) &
            IARF_ADD) {
...
static void examine_braces(void) {
    ...
    for (auto pc = chunk_get_tail(); pc != nullptr;) {
        ...
        if (chunk_is_token(pc, CT_BRACE_OPEN) && !pc->flags.test(PCF_IN_PREPROC) &&
...
static bool should_add_braces(chunk_t *vbopen) {
    ...
    LOG_FMT(LBRDEL, "%s(%d): start on %zu : ", __func__, __LINE__,
            vbopen->orig_line);
...
static bool can_remove_braces(chunk_t *bopen) {
    ...
    LOG_FMT(
        LBRDEL,
        "%s(%d):  - begin with token '%s', orig_line is %zu, orig_col is %zu\n",
        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);

    while (pc != nullptr && pc->level >= level) {
        LOG_FMT(LBRDEL,
                "%s(%d): test token '%s', orig_line is %zu, orig_col is %zu\n",
                __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
        ...
        if (chunk_is_newline(pc)) {
            ...
            if (chunk_is_token(pc, CT_BRACE_OPEN)) {
                ...
                if (br_count == 0) {
                    fprintf(stderr,
                            "%s(%d): br_count is ZERO, cannot be decremented, at line "
                            "%zu, column %zu\n",
            ...
            if (pc->level == level) {
                ...
                LOG_FMT(LBRDEL,
                        "%s(%d): text() '%s', orig_line is %zu, semi_count is %zu\n",
                ...
                if (chunk_is_semicolon(pc) || chunk_is_token(pc, CT_IF) ||
                        chunk_is_token(pc, CT_ELSEIF) || chunk_is_token(pc, CT_FOR) ||
                        chunk_is_token(pc, CT_DO) || chunk_is_token(pc, CT_WHILE) ||
                    ...
                    if (++semi_count > 1) {
                        LOG_FMT(LBRDEL,
                                "%s(%d):  bailed on %zu because of '%s' on line %zu\n",
                                __func__, __LINE__, bopen->orig_line, pc->text(),
                                pc->orig_line);
                        return (false);
                    }
                }
    ...
    if (chunk_is_token(pc, CT_BRACE_CLOSE) &&
        ...
        if (chunk_is_token(next, CT_ELSE) &&
            ...
            LOG_FMT(LBRDEL,
                    "%s(%d):  - bailed on '%s'[%s] on line %zu due to 'if' and "
                    "'else' sequence\n",
                    __func__, __LINE__, get_token_name(pc->type),
                    get_token_name(get_chunk_parent_type(pc)), pc->orig_line);
            return (false);
        }
    }
    LOG_FMT(
        LBRDEL,
        "%s(%d):  - end on '%s' on line %zu. if_count is %zu semi_count is %zu\n",
        __func__, __LINE__, get_token_name(pc->type), pc->orig_line, if_count,
        semi_count);
...
static void examine_brace(chunk_t *bopen) {
    ...
    while (pc != nullptr && pc->level >= level) {
        ...
        if (chunk_is_newline(pc)) {
            ...
            LOG_FMT(LBRDEL,
                    "%s(%d): for pc->text() '%s', pc->level is %zu,  bopen->level is "
                    "%zu\n",
            ...
            if (chunk_is_token(pc, CT_BRACE_OPEN) && pc->level == bopen->level) {
                br_count++;
                LOG_FMT(LBRDEL,
                        "%s(%d): br_count is now %d, pc->level is %zu,  bopen->level "
                        "is %zu\n",
            ...
            if (pc->level == level) {
                ...
                LOG_FMT(LBRDEL,
                        "%s(%d): for pc->text() '%s', pc->level is %zu,  bopen->level "
                        "is %zu\n",
                        __func__, __LINE__, pc->text(), pc->level, bopen->level);
    ...
    LOG_FMT(LBRDEL,
            "%s(%d):  - end on '%s' on line %zu. if_count is %zu, semi_count is "
            "%zu\n",
            __func__, __LINE__, get_token_name(pc->type), pc->orig_line, if_count,
            semi_count);
    ...
    if (chunk_is_token(pc, CT_BRACE_CLOSE)) {
        ...
        if (next != nullptr) {
            ...
            if (next != nullptr) {
                LOG_FMT(LBRDEL,
                        "%s(%d): orig_line is %zu, orig_col is %zu, next is '%s'\n",
            ...
            if (if_count > 0 &&
                ...
                LOG_FMT(LBRDEL,
                        "%s(%d):  bailed on because 'else' is next and %zu ifs\n",
        ...
        if (semi_count > 0) {
            LOG_FMT(LBRDEL, "%s(%d): bopen->parent_type is %s\n", __func__, __LINE__,
                    get_token_name(get_chunk_parent_type(bopen)));
            ...
            if (get_chunk_parent_type(bopen) == CT_ELSE) {
                ...
                if (chunk_is_token(tmp_next, CT_IF)) {
                    ...
                    LOG_FMT(LBRDEL,
                            "%s(%d):  else-if removing braces on line %zu and %zu\n",
            ...
            LOG_FMT(LBRDEL,
                    "%s(%d): we have a pair of braces with only 1 statement inside\n",
                    __func__, __LINE__);
    ...
    if (chunk_is_newline(tmp)) {
        if (tmp->nl_count > 1) {
            if (!br->flags.test(PCF_ONE_LINER)) // Issue #2232
            {
                if (tmp->nl_count == 0) {
                    fprintf(stderr,
                            "%s(%d): tmp->nl_count is ZERO, cannot be decremented, at "
                            "line %zu, column %zu\n",
...
chunk_t *insert_comment_after(chunk_t *ref, c_token_t cmt_type,
    ...
    if (cmt_type == CT_COMMENT_CPP) {
        ...
        if (chunk_is_token(ref, CT_PP_ELSE)) { // make test c/ 02501 stable
            new_cmt.str.append(" ");
        }
        new_cmt.str.append("/* ");
        new_cmt.str.append(cmt_text);
    ...
    for (chunk_t *pc = chunk_get_head(); pc != nullptr;
        ...
        while ((tmp = chunk_get_next(tmp, scope_e::PREPROC)) != nullptr) {
                        ...
                        tmp_next->type != CT_BRACE_OPEN) // anonymous namespace -> ignore
                {
                    ...
                    append_tag_name(xstr, tmp_next);
                       ...
                       (!language_is_set(LANG_CPP) // proceed if not C++
                        || (chunk_is_token(br_close,
                                           CT_SEMICOLON)))) // else a C++ class needs
            ...
            if (nl_min > 0 && nl_count >= nl_min && tag_pc != nullptr) {
                ...
                const c_token_t style =
                    language_is_set(LANG_CPP | LANG_CS) ? CT_COMMENT_CPP : CT_COMMENT;
...
static void move_case_break(void) {
    ...
    for (chunk_t *pc = chunk_get_head(); pc != nullptr;
            pc = chunk_get_next_ncnl(pc)) {
        if (chunk_is_token(pc, CT_BREAK) && chunk_is_token(prev, CT_BRACE_CLOSE) &&
...
static chunk_t *mod_case_brace_remove(chunk_t *br_open) {
    LOG_FUNC_ENTRY();
    LOG_FMT(LMCB, "%s(%d): line %zu", __func__, __LINE__, br_open->orig_line);

    // Find the matching brace close
    chunk_t *next = chunk_get_next_ncnl(br_open, scope_e::PREPROC);
    chunk_t *br_close = chunk_get_next_type(br_open, CT_BRACE_CLOSE,
                                            br_open->level, scope_e::PREPROC);
    ...
    if (pc == nullptr ||
            (pc->type != CT_BREAK && pc->type != CT_RETURN && pc->type != CT_CASE &&
             pc->type != CT_GOTO && pc->type != CT_BRACE_CLOSE)) {
    ...
    for (chunk_t *tmp_pc = br_open; tmp_pc != br_close;
    ...
    LOG_FMT(LMCB, "%s(%d):  - removing braces on lines %zu and %zu\n", __func__,
            __LINE__, br_open->orig_line, br_close->orig_line);

    for (chunk_t *tmp_pc = br_open; tmp_pc != br_close;
            tmp_pc = chunk_get_next_ncnl(tmp_pc, scope_e::PREPROC)) {
        if (tmp_pc->brace_level == 0) {
            fprintf(stderr,
                    "%s(%d): tmp_pc->brace_level is ZERO, cannot be decremented, at "
                    "line %zu, column %zu\n",
        ...
        if (tmp_pc->level == 0) {
            fprintf(stderr,
                    "%s(%d): tmp_pc->level is ZERO, cannot be decremented, at line "
                    "%zu, column %zu\n",
                    __func__, __LINE__, tmp_pc->orig_line, tmp_pc->orig_col);
            log_flush(true);
            exit(EX_SOFTWARE);
...
static chunk_t *mod_case_brace_add(chunk_t *cl_colon) {
    LOG_FUNC_ENTRY();
    LOG_FMT(LMCB, "%s(%d): line %zu", __func__, __LINE__, cl_colon->orig_line);
    ...
    LOG_FMT(LMCB, "%s(%d):  - adding before '%s' on line %zu\n", __func__,
            __LINE__, last->text(), last->orig_line);
    ...
    for (pc = chunk_get_next(br_open, scope_e::PREPROC); pc != br_close;
    ...
    while (pc != nullptr) {
        ...
        if (options::mod_case_brace() == IARF_REMOVE &&
            ...
            pc = mod_case_brace_remove(pc);
                   ...
                   next->type != CT_BRACE_OPEN && next->type != CT_BRACE_CLOSE &&
                   next->type != CT_CASE) {
            log_rule_B("mod_case_brace");
...
static void process_if_chain(chunk_t *br_start) {
    ...
    LOG_FMT(LBRCH, "%s(%d): if starts on line %zu, orig_col is %zu.\n", __func__,
            __LINE__, br_start->orig_line, br_start->orig_col);
    ...
    while (pc != nullptr) {
        ...
        if (chunk_is_token(pc, CT_BRACE_OPEN)) {
            ...
            LOG_FMT(LBRCH,
                    "%s(%d): braces.size() is %zu, line is %zu, - can%s remove %s\n",
                    __func__, __LINE__, braces.size(), pc->orig_line,
                    tmp ? "" : "not", get_token_name(pc->type));
            ...
            LOG_FMT(LBRCH, "%s(%d): braces.size() is %zu, line is %zu, - %s %s\n",
                    __func__, __LINE__, braces.size(), pc->orig_line,
                    tmp ? "should add" : "ignore", get_token_name(pc->type));
        }
        braces.push_back(pc);
        ...
        if (chunk_is_token(pc, CT_ELSEIF)) {
            while (pc != nullptr && pc->type != CT_VBRACE_OPEN &&
    ...
    if (must_have_braces) {
        LOG_FMT(LBRCH, "%s(%d): add braces on lines[%zu]:", __func__, __LINE__,
                braces.size());
        ...
        for (auto itc = braces.rbegin(); itc != ite; ++itc) {
            ...
            if (chunk_is_token(brace, CT_VBRACE_OPEN) ||
                ...
                LOG_FMT(LBRCH, "%s(%d):  %zu", __func__, __LINE__, brace->orig_line);
        ...
        for (auto itc = braces.begin(); itc != ite; ++itc) {
            ...
            if ((chunk_is_token(brace, CT_BRACE_OPEN) ||
                ...
                LOG_FMT(LBRCH,
                        "%s(%d): brace->orig_line is %zu, brace->orig_col is %zu\n",
