...
void quick_align_again(void) {
	...
	for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
		LOG_FMT(
			LALAGAIN,
			"%s(%d): pc->orig_line is %zu, pc->orig_col is %zu, pc->text() '%s'\n",
		...
		if (pc->align.next != nullptr && pc->flags.test(PCF_ALIGN_START)) {
			...
			as.m_star_style =
				static_cast<AlignStack::StarStyle>(pc->align.star_style);
			...
			for (chunk_t *tmp = pc->align.next; tmp != nullptr;
			     tmp = tmp->align.next) {
				chunk_flags_set(tmp, PCF_WAS_ALIGNED);
				as.Add(tmp->align.start);
				LOG_FMT(LALAGAIN,
				        "%s(%d):    => tmp->text() is '%s', orig_line is %zu\n",
