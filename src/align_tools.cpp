...
chunk_t *scan_ib_line(chunk_t *start, bool first_pass) {
    ...
    if (pc != nullptr) {
        LOG_FMT(LSIB,
                "%s(%d): start: orig_line is %zu, orig_col is %zu, column is %zu, "
                "type is %s\n",
                __func__, __LINE__, pc->orig_line, pc->orig_col, pc->column,
                get_token_name(pc->type));
    }

    while (pc != nullptr && !chunk_is_newline(pc) && pc->level >= start->level) {
        ...
        if (next == nullptr || chunk_is_comment(next)) {
            ...
            if (idx >= cpd.al_cnt) {
                ...
                LOG_FMT(LSIB,
                        "%s(%d):   New idx is %2.1zu, pc->column is %2.1zu, text() "
                        "'%s', token_width is %zu, type is %s\n",
                        __func__, __LINE__, idx, pc->column, pc->text(), token_width,
                        get_token_name(pc->type));
                ...
                if (cpd.al_cnt == AL_SIZE) {
                    fprintf(stderr,
                            "Number of 'entry' to be aligned is too big for the current "
                            "value %d,\n",
                            AL_SIZE);
                ...
                if (cpd.al[idx].type == pc->type) {
                    LOG_FMT(LSIB,
                            "%s(%d):   Match? idx is %2.1zu, orig_line is %2.1zu, column "
                            "is %2.1zu, token_width is %zu, type is %s\n",
                            __func__, __LINE__, idx, pc->orig_line, pc->column,
                            token_width, get_token_name(pc->type));

                    // Shift out based on column
                    if (prev_match == nullptr) {
                        if (pc->column > cpd.al[idx].col) {
                            LOG_FMT(LSIB,
                                    "%s(%d): [ pc->column (%zu) > cpd.al[%zu].col(%zu) ] \n",
                        ...
                        LOG_FMT(LSIB,
                                "%s(%d):   prev_match '%s', prev_match->orig_line is %zu, "
                                "prev_match->orig_col is %zu\n",
                                __func__, __LINE__, prev_match->text(),
                                prev_match->orig_line, prev_match->orig_col);
                    ...
                    LOG_FMT(LSIB,
                            "%s(%d): at ende of the loop: now is col %zu, len is %zu\n",
