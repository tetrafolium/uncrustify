...
void align_left_shift(void) {
    ...
    while (pc != nullptr) {
        if (chunk_is_newline(pc)) {
            ...
            LOG_FMT(LAVDB,
                    "%s(%d): orig_line is %zu, orig_col is %zu, pc->text() '%s'\n",
        ...
        if (start != nullptr &&
            ...
            start = nullptr;
                   ...
                   !pc->flags.test(PCF_IN_TYPEDEF) && chunk_is_str(pc, "<<", 2)) {
            ...
            /*
             * check if the given statement is on a line of its own, immediately
             * following << and then it. Eg:
             *
             *      cout <<
             *          "something";
             */
