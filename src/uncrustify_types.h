...
//! performs abs on Y after static casting it to the type (with removed
//! reference) of X
...
constexpr auto pcf_bit(size_t b) -> decltype(0ULL) { return (1ULL << b); }
...
enum pcf_flag_e : decltype(0ULL) {
  ...
  PCF_IN_WHERE_SPEC = pcf_bit(
      15), /* inside C# 'where' constraint clause on class or function def */
...
struct chunk_t {
  ...
  const char *text() const { return (str.c_str()); }
  ...
  size_t
      orig_col; //! column where chunk started in the input file, is always > 0
  size_t orig_col_end; //! column where chunk ended in the input file, is always
                       //! > 1
  ...
  size_t nl_column;     //! column of the subsequent newline entries(all of them
