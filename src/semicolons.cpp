...
static void remove_semicolon(chunk_t *pc) {
    LOG_FUNC_ENTRY();
    LOG_FMT(LDELSEMI,
            "%s(%d): Removed semicolon: orig_line is %zu, orig_col is %zu",
...
void remove_extra_semicolons(void) {
    ...
    while (pc != nullptr) {
        ...
        if (chunk_is_token(pc, CT_SEMICOLON) && !pc->flags.test(PCF_IN_PREPROC) &&
            ...
            LOG_FMT(LSCANSEMI,
                    "%s(%d): Semi orig_line is %zu, orig_col is %zu, parent is %s, "
                    "prev = '%s' [%s/%s]\n",
                    __func__, __LINE__, pc->orig_line, pc->orig_col,
                    get_token_name(get_chunk_parent_type(pc)), prev->text(),
...
static void check_unknown_brace_close(chunk_t *semi, chunk_t *brace_close) {
    LOG_FUNC_ENTRY();
    chunk_t *pc =
        chunk_get_prev_type(brace_close, CT_BRACE_OPEN, brace_close->level);
    pc = chunk_get_prev_ncnl(pc);

    if (pc != nullptr && pc->type != CT_RETURN && pc->type != CT_WORD &&
            pc->type != CT_TYPE && pc->type != CT_SQUARE_CLOSE &&
            pc->type != CT_ANGLE_CLOSE && pc->type != CT_TSQUARE &&
