 ...
 * "test/config/MS-calling_conventions.cfg" typedef <return type>func(params);
 ...
 */
static void mark_function_return_type(chunk_t *fname, chunk_t *pc,
 ...
 *
 */
static pcf_flags_t mark_where_chunk(chunk_t *pc, c_token_t parent_type,
                                    pcf_flags_t flags);
 ...
 */
static chunk_t *handle_oc_md_type(chunk_t *paren_open, c_token_t ptype,
                                  pcf_flags_t flags, bool &did_it);
...
void flag_series(chunk_t *start, chunk_t *end, pcf_flags_t set_flags,
                 pcf_flags_t clr_flags, scope_e nav) {
	LOG_FUNC_ENTRY();
...
chunk_t *set_paren_parent(chunk_t *start, c_token_t parent) {
	...
	if (end != nullptr) {
		LOG_FMT(
			LFLPAREN,
			"%s(%d): %zu:%zu '%s' and %zu:%zu '%s' type is %s, parent_type is %s",
			__func__, __LINE__, start->orig_line, start->orig_col, start->text(),
			end->orig_line, end->orig_col, end->text(), get_token_name(start->type),
			get_token_name(parent));
...
static void flag_asm(chunk_t *pc) {
	...
	for (tmp = chunk_get_next_ncnl(po, scope_e::PREPROC);
		...
		if (chunk_is_token(tmp, CT_COLON)) {
			...
			if (chunk_is_token(chunk_get_next_ncnl(tmp, scope_e::PREPROC),
			                   CT_STRING) &&
			    chunk_is_token(chunk_get_prev_ncnlni(tmp, scope_e::PREPROC),
			                   CT_STRING)) // Issue #2279
			{
				chunk_t nc;
...
static bool chunk_ends_type(chunk_t *start) {
	...
	{
		LOG_FMT(LFTYPE,
		        "%s(%d): type is %s, text() '%s', orig_line %zu, orig_col %zu\n   ",
		...
		if (chunk_is_token(pc, CT_WORD) || chunk_is_token(pc, CT_TYPE) ||
		    chunk_is_token(pc, CT_PTR_TYPE) || chunk_is_token(pc, CT_STRUCT) ||
		    chunk_is_token(pc, CT_DC_MEMBER) || chunk_is_token(pc, CT_PP) ||
		    chunk_is_token(pc, CT_QUALIFIER) ||
		    (language_is_set(LANG_CS) && (chunk_is_token(pc, CT_MEMBER)))) {
			cnt++;
			last_expr =
				pc->flags.test(PCF_EXPR_START) && !pc->flags.test(PCF_IN_FCN_CALL);
			last_lval = pc->flags.test(PCF_LVALUE);
			continue;
		}

		if ((chunk_is_semicolon(pc) && !pc->flags.test(PCF_IN_FOR)) ||
		    chunk_is_token(pc, CT_TYPEDEF) || chunk_is_token(pc, CT_BRACE_OPEN) ||
		    chunk_is_token(pc, CT_BRACE_CLOSE) ||
		    chunk_is_token(pc, CT_VBRACE_CLOSE) ||
		    chunk_is_token(pc, CT_FPAREN_CLOSE) || chunk_is_forin(pc) ||
		    chunk_is_token(pc, CT_MACRO) || chunk_is_token(pc, CT_PP_IF) ||
		    chunk_is_token(pc, CT_PP_ELSE) || chunk_is_token(pc, CT_PP_ENDIF) ||
		    ((chunk_is_token(pc, CT_COMMA) && !pc->flags.test(PCF_IN_FCN_CALL)) &&
		     last_expr) ||
...
void do_symbol_check(chunk_t *prev, chunk_t *pc, chunk_t *next) {
	...
	if ((chunk_is_token(prev, CT_FPAREN_CLOSE) ||
	    ...
	    (chunk_is_token(next, CT_DEFAULT) || chunk_is_token(next, CT_DELETE) ||
	...
	// D stuff
	if (language_is_set(LANG_D) && chunk_is_token(pc, CT_QUALIFIER) &&
	    chunk_is_str(pc, "const", 5) && chunk_is_token(next, CT_PAREN_OPEN)) {
		set_chunk_type(pc, CT_D_CAST);
		set_paren_parent(next, pc->type);
	}

	if (chunk_is_token(next, CT_PAREN_OPEN) &&
	    (chunk_is_token(pc, CT_D_CAST) || chunk_is_token(pc, CT_DELEGATE) ||
		...
		if (chunk_is_token(pc, CT_DELEGATE)) {
			...
			for (tmp = chunk_get_prev_ncnlni(pc); tmp != nullptr;
			     tmp = chunk_get_prev_ncnlni(tmp)) // Issue #2279
			{
				if (chunk_is_semicolon(tmp) || chunk_is_token(tmp, CT_BRACE_OPEN) ||
	...
	// clang stuff - A new derived type is introduced to C and, by extension,
	...
	if (language_is_set(LANG_CS)) {
		...
		if (chunk_is_token(next, CT_BRACE_OPEN) &&
		    ...
		    (chunk_is_token(pc, CT_SQUARE_CLOSE) ||
		     chunk_is_token(pc, CT_ANGLE_CLOSE) || chunk_is_token(pc, CT_WORD))) {
	...
	if (language_is_set(LANG_JAVA) && chunk_is_token(pc, CT_LAMBDA) &&
	    chunk_is_token(next, CT_BRACE_OPEN)) {
		set_paren_parent(next, pc->type);
	}
	...
	if (chunk_is_token(pc, CT_SQL_EXEC) || chunk_is_token(pc, CT_SQL_BEGIN) ||
	...
	if (chunk_is_token(pc, CT_ENUM) || chunk_is_token(pc, CT_STRUCT) ||
	...
	if (chunk_is_token(pc, CT_WORD) && chunk_is_token(next, CT_ANGLE_OPEN) &&
	...
	if (chunk_is_token(next, CT_PAREN_OPEN)) {
		...
		if ((language_is_set(LANG_C | LANG_CPP | LANG_OC)) &&
			...
			// This is the case where a block literal is passed as the first argument
			// of a C-style method invocation.
	...
	if (chunk_is_token(pc, CT_FUNC_DEF) || chunk_is_token(pc, CT_FUNC_CALL) ||
	...
	if (!pc->flags.test_any(PCF_IN_TYPEDEF | PCF_IN_TEMPLATE) &&
	    ...
	    !pc->flags.test(PCF_IN_PREPROC) && !is_oc_block(pc) &&
	    get_chunk_parent_type(pc) != CT_OC_MSG_DECL &&
	    get_chunk_parent_type(pc) != CT_OC_MSG_SPEC && chunk_is_str(pc, ")", 1) &&
	    chunk_is_str(next, "(", 1)) {
		if (language_is_set(LANG_D)) {
	...
	if (!language_is_set(LANG_D)) {
		...
		if (chunk_is_token(pc, CT_PAREN_OPEN) &&
		    ...
		    && (chunk_is_token(next, CT_WORD) || chunk_is_token(next, CT_TYPE) ||
		        ...
		        chunk_is_token(next, CT_ENUM) || chunk_is_token(next, CT_UNION)) &&
		    prev->type != CT_DECLTYPE && prev->type != CT_SIZEOF &&
	...
	if (language_is_set(LANG_CPP)) {
		...
		if (pc->flags.test_any(PCF_IN_STRUCT | PCF_IN_CLASS) &&
		    chunk_is_token(pc, CT_ASSIGN) && chunk_is_token(nnext, CT_SEMICOLON) &&
		    (chunk_is_token(next, CT_DEFAULT) || chunk_is_token(next, CT_DELETE) ||
			...
			for (; (pprev != nullptr && pprev->level >= level &&
			        pprev->type != CT_SEMICOLON && pprev->type != CT_ACCESS_COLON);
		...
		if (chunk_is_token(pc, CT_WORD) || chunk_is_token(pc, CT_TYPE) ||
		    chunk_is_token(pc, CT_ASSIGN) || chunk_is_token(pc, CT_RETURN) ||
		    chunk_is_token(pc, CT_COMMA) || chunk_is_token(pc, CT_ANGLE_CLOSE) ||
		    chunk_is_token(pc, CT_SQUARE_CLOSE) || chunk_is_token(pc, CT_TSQUARE) ||
		    chunk_is_token(pc, CT_FPAREN_OPEN) || chunk_is_token(pc, CT_QUESTION) ||
	...
	if (pc->flags.test(PCF_EXPR_START) ||
		...
		if (language_is_set(LANG_CPP) && chunk_is_token(pc, CT_CARET) &&
		    chunk_is_token(prev, CT_ANGLE_CLOSE)) {
			set_chunk_type(pc, CT_PTR_TYPE);
		}

		if (language_is_set(LANG_CS) && (chunk_is_token(pc, CT_QUESTION)) &&
		    (chunk_is_token(prev, CT_ANGLE_CLOSE))) {
			set_chunk_type(pc, CT_PTR_TYPE);
		}
	...
	if (!pc->flags.test(PCF_IN_TYPEDEF) &&
	    ...
	    (chunk_is_token(pc, CT_STRUCT) || chunk_is_token(pc, CT_UNION) ||
	     chunk_is_token(pc, CT_CLASS) || chunk_is_token(pc, CT_ENUM))) {
	...
	if (chunk_is_token(pc, CT_MACRO_OPEN) || chunk_is_token(pc, CT_MACRO_ELSE) ||
	...
	if (chunk_is_token(pc, CT_STAR) ||
		...
		if (chunk_is_paren_close(next) || chunk_is_token(next, CT_COMMA)) {
			...
			LOG_FMT(LFCNR,
			        "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is '%s', "
			        "type is %s\n   ",
			        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
		           ...
		           !language_is_set(LANG_OC)) // issue # 408
		{
			...
			if (chunk_is_token(prev, CT_TYPE)) {
				...
				// Issue 1402
				set_chunk_type(pc, (prev->flags.test(PCF_PUNCTUATOR) &&
	...
	if (chunk_is_token(pc, CT_AMP)) {
		if (chunk_is_token(prev, CT_DELETE)) {
			...
			// fix the bug #654
			// connect(&mapper, SIGNAL(mapped(QString &)), this,
			// SLOT(onSomeEvent(QString &)));
			set_chunk_type(pc, CT_BYREF);
			...
			// Issue # 1398
			if (pc->flags.test(PCF_IN_FCN_DEF) && chunk_is_token(prev, CT_WORD) &&
			    chunk_is_token(pc, CT_AMP) && chunk_is_token(next, CT_WORD)) {
				/*
	...
	if (chunk_is_token(pc, CT_MINUS) || chunk_is_token(pc, CT_PLUS)) {
		if (chunk_is_token(prev, CT_POS) || chunk_is_token(prev, CT_NEG) ||
	...
	if (chunk_is_token(pc, CT_WORD)) // here NSString
	{
		if (pc->next != nullptr) // here *
		{
			if (pc->next->type == CT_STAR) // here *
			{
					...
					if (pc->prev->type == CT_STRING) {
						if (unc_text::compare(pc->prev->text(), "\"C\"") == 0) {
							if (pc->prev->prev->type == CT_EXTERN) {
								...
								set_chunk_type(pc->next,
								               CT_PTR_TYPE); // change CT_STAR => CT_PTR_TYPE
							}
						}
					}
				}
				...
				if ((pc->next->next != nullptr) && pc->next->next->type == CT_STAR &&
				    pc->flags.test(PCF_IN_CONST_ARGS)) {
				...
				if ((pc->next->next != nullptr) && pc->next->next->type == CT_WORD &&
	...
	if (chunk_is_token(pc, CT_USING)) {
		...
		for (temp = pc; temp != nullptr; temp = chunk_get_next_ncnl(temp)) {
			LOG_FMT(LFCNR,
			        "%s(%d): orig_line is %zu, orig_col is %zu, text() '%s', type is "
			        "%s\n",
			...
			if (chunk_is_token(temp, CT_SEMICOLON) ||
			    (is_preproc && (!temp->flags.test(PCF_IN_PREPROC) ||
	...
	if (pc->type == CT_BOOL && !pc->flags.test(PCF_IN_PREPROC) &&
	    chunk_is_str(pc, "&&", 2) && chunk_ends_type(pc->prev)) {
		set_chunk_type(pc, CT_BYREF);
	}

	// Issue #1704
	if (chunk_is_token(pc, CT_INCDEC_AFTER) && pc->flags.test(PCF_IN_PREPROC)) {
...
static void check_double_brace_init(chunk_t *bo1) {
	LOG_FUNC_ENTRY();
	LOG_FMT(LJDBI, "%s(%d): orig_line is %zu, orig_col is %zu", __func__,
	        __LINE__, bo1->orig_line, bo1->orig_col);
...
void fix_symbols(void) {
	...
	for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next_ncnl(pc)) {
		...
		if (is_cpp && chunk_is_token(pc, CT_BRACE_OPEN) &&
	...
	while (pc != nullptr) {
		...
		LOG_FMT(LFCNR,
		        "%s(%d): pc->orig_line       is %zu, orig_col is %zu, text() is "
		        "'%s', type is %s\n",
		        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
		        get_token_name(pc->type));
		...
		if (prev == nullptr) {
			...
			// Issue #2279
			LOG_FMT(LFCNR,
			        "%s(%d): prev(ni)->orig_line is %zu, orig_col is %zu, text() is "
			        "'%s', type is %s\n",
	...
	while (pc != nullptr) {
		...
		if (square_level < 0 && pc->flags.test(PCF_STMT_START) &&
		    (chunk_is_token(pc, CT_QUALIFIER) || chunk_is_token(pc, CT_TYPE) ||
		     chunk_is_token(pc, CT_TYPENAME) || chunk_is_token(pc, CT_WORD)) &&
		    get_chunk_parent_type(pc) != CT_ENUM && !pc->flags.test(PCF_IN_ENUM)) {
			pc = fix_var_def(pc);
		} else {
			pc = chunk_get_next_ncnl(pc);
		}
...
static void mark_lvalue(chunk_t *pc) {
	...
	for (prev = chunk_get_prev_ncnlni(pc);                // Issue #2279
	     prev != nullptr; prev = chunk_get_prev_ncnlni(prev)) // Issue #2279
	{
		if (prev->level < pc->level || chunk_is_token(prev, CT_ASSIGN) ||
		    chunk_is_token(prev, CT_COMMA) || chunk_is_token(prev, CT_BOOL) ||
		    chunk_is_semicolon(prev) || chunk_is_str(prev, "(", 1) ||
		    chunk_is_str(prev, "{", 1) || chunk_is_str(prev, "[", 1) ||
...
static void mark_function_return_type(chunk_t *fname, chunk_t *start,
                                      c_token_t parent_type) {
	LOG_FUNC_ENTRY();
	chunk_t *pc = start;
	...
	if (pc != nullptr) {
		...
		LOG_FMT(LFCNR,
		        "%s(%d): (backwards) return type for '%s' @ orig_line is %zu, "
		        "orig_col is %zu\n",
		        __func__, __LINE__, fname->text(), fname->orig_line,
		        fname->orig_col);
		...
		while (pc != nullptr) {
			...
			if (chunk_is_token(pc, CT_ANGLE_CLOSE)) {
				...
				if (pc == nullptr || chunk_is_token(pc, CT_TEMPLATE)) {
					// either expression is not complete or this is smth like 'template<T>
					// void func()'
					//  - we are not interested in 'template<T>' part
					break;
				} else {
					// this is smth like 'vector<int> func()' and 'pc' is currently on
					// 'vector' - just proceed
				}
			}

			if ((!chunk_is_type(pc) && pc->type != CT_OPERATOR &&
			     pc->type != CT_WORD && pc->type != CT_ADDR) ||
		...
		while (pc != nullptr) {
			LOG_FMT(LFCNR, " text() '%s', type is %s", pc->text(),
			        get_token_name(pc->type));
...
static bool mark_function_type(chunk_t *pc) {
	...
	LOG_FMT(
		LFTYPE,
		"%s(%d): type is %s, text() '%s' @ orig_line is %zu, orig_col is %zu\n",
		__func__, __LINE__, get_token_name(pc->type), pc->text(), pc->orig_line,
		pc->orig_col);
	...
	if (varcnk != nullptr && !chunk_is_word(varcnk)) {
		if (language_is_set(LANG_OC) && chunk_is_str(varcnk, "^", 1) &&
		    chunk_is_paren_open(chunk_get_prev_ncnlni(varcnk))) // Issue #2279
		{
			...
			LOG_FMT(LFTYPE,
			        "%s(%d): not a word: text() '%s', type is %s, @ orig_line is "
			        "%zu:, orig_col is %zu\n",
	...
	if (apc != nullptr && (!chunk_is_paren_open(apo) ||
	...
	while ((tmp = chunk_get_prev_ncnlni(tmp)) != nullptr) // Issue #2279
	{
		...
		LOG_FMT(LFTYPE, " -- type is %s, %s on orig_line %zu, orig_col is %zu",
		        get_token_name(tmp->type), tmp->text(), tmp->orig_line,
		        tmp->orig_col);
		...
		if (chunk_is_star(tmp) || chunk_is_token(tmp, CT_PTR_TYPE) ||
		...
		} else if (chunk_is_word(tmp) || chunk_is_token(tmp, CT_WORD) ||
			...
			LOG_FMT(LFTYPE,
			        " --  unexpected token: type is %s, text() '%s', on orig_line "
			        "%zu, orig_col %zu\n",
			        get_token_name(tmp->type), tmp->text(), tmp->orig_line,
			        tmp->orig_col);
			goto nogo_exit;
	...
	if (star_count > 1 ||
		...
		LOG_FMT(LFTYPE, "%s(%d): bad counts word: %zu, star: %zu\n", __func__,
		        __LINE__, word_count, star_count);
		goto nogo_exit;
	}
	...
	if (chunk_is_semicolon(aft)) {
		set_chunk_parent(aft, aft->flags.test(PCF_IN_TYPEDEF) ? CT_TYPEDEF
		                 : CT_FUNC_VAR);
	...
	{
		LOG_FMT(
			LFTYPE, " ++ type is %s, text() '%s', on orig_line %zu, orig_col %zu\n",
			get_token_name(tmp->type), tmp->text(), tmp->orig_line, tmp->orig_col);
...
nogo_exit:
	...
	if (chunk_is_paren_open(tmp)) {
		LOG_FMT(LFTYPE,
		        "%s(%d): setting FUNC_CALL on orig_line is %zu, orig_col is %zu\n",
...
static chunk_t *process_return(chunk_t *pc) {
		...
		if (chunk_is_token(semi, CT_NEWLINE) || chunk_is_semicolon(semi)) {
			...
			if (options::mod_paren_on_return() == IARF_REMOVE) {
				...
				for (temp = next; temp != cpar; temp = chunk_get_next(temp)) {
					if (temp->level == 0) {
						fprintf(stderr,
						        "%s(%d): temp->level is ZERO, cannot be decremented, at "
						        "line %zu, column %zu\n",
	...
	if (language_is_set(LANG_CPP) && chunk_is_token(next, CT_BRACE_OPEN) &&
		...
		LOG_FMT(LRETURN,
		        "%s(%d): not adding parens around braced initializer"
		        " on orig_line %zd\n",
...
static void fix_casts(chunk_t *start) {
	...
	LOG_FMT(LCASTS,
	        "%s(%d): start->text() is '%s', orig_line is %zu, orig_col is %zu\n",
	...
	while (pc != nullptr &&
	       (chunk_is_type(pc) || chunk_is_token(pc, CT_WORD) ||
	        chunk_is_token(pc, CT_QUALIFIER) ||
	        chunk_is_token(pc, CT_DC_MEMBER) || chunk_is_token(pc, CT_PP) ||
	        chunk_is_token(pc, CT_STAR) || chunk_is_token(pc, CT_QUESTION) ||
	        chunk_is_token(pc, CT_CARET) || chunk_is_token(pc, CT_TSQUARE) ||
	        ((chunk_is_token(pc, CT_ANGLE_OPEN) ||
	          chunk_is_token(pc, CT_ANGLE_CLOSE)) &&
	         language_is_set(LANG_OC | LANG_JAVA)) ||
	        ((chunk_is_token(pc, CT_QUESTION) || chunk_is_token(pc, CT_COMMA) ||
	          chunk_is_token(pc, CT_MEMBER)) &&
	         language_is_set(LANG_JAVA)) ||
	        chunk_is_token(pc, CT_AMP))) {
		LOG_FMT(LCASTS,
		        "%s(%d): pc->text() is '%s', orig_line is %zu, orig_col is %zu, "
		        "type is %s\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
		        get_token_name(pc->type));

		if (chunk_is_token(pc, CT_WORD) || (chunk_is_token(last, CT_ANGLE_CLOSE) &&
		                                    chunk_is_token(pc, CT_DC_MEMBER))) {
			word_count++;
		} else if (chunk_is_token(pc, CT_DC_MEMBER) ||
		           chunk_is_token(pc, CT_MEMBER) || chunk_is_token(pc, CT_PP)) {
	...
	if (pc == nullptr || pc->type != CT_PAREN_CLOSE ||
		...
		LOG_FMT(LCASTS, "%s(%d):  -- not a cast, hit type is %s\n", __func__,
		        __LINE__, pc == nullptr ? "NULL" : get_token_name(pc->type));
		return;
	}
	...
	if (chunk_is_token(last, CT_STAR) || chunk_is_token(last, CT_CARET) ||
	    chunk_is_token(last, CT_PTR_TYPE) || chunk_is_token(last, CT_TYPE) ||
		...
		if ((last->len() > 3) && (last->str[last->len() - 2] == '_') &&
		...
		if (chunk_is_ptr_operator(pc)) {
			...
			if (chunk_is_token(after, CT_NUMBER_FP) ||
			    ...
			    chunk_is_token(after, CT_STRING) || doubtful_cast) {
				nope = true;
			}
		...
		} else if (pc->type != CT_NUMBER_FP && pc->type != CT_NUMBER &&
		           pc->type != CT_WORD && pc->type != CT_THIS &&
		           pc->type != CT_TYPE && pc->type != CT_PAREN_OPEN &&
		           pc->type != CT_STRING && pc->type != CT_DECLTYPE &&
		           pc->type != CT_SIZEOF &&
		           get_chunk_parent_type(pc) != CT_SIZEOF &&
		           pc->type != CT_FUNC_CALL && pc->type != CT_FUNC_CALL_USER &&
		           pc->type != CT_FUNCTION && pc->type != CT_BRACE_OPEN &&
		           (!(chunk_is_token(pc, CT_SQUARE_OPEN) &&
		              language_is_set(LANG_OC)))) {
			LOG_FMT(LCASTS,
			        "%s(%d):  -- not a cast - followed by text() '%s', type is %s\n",
		...
		if (nope) {
			LOG_FMT(LCASTS,
			        "%s(%d):  -- not a cast - text() '%s' followed by type %s\n",
			        __func__, __LINE__, pc->text(), get_token_name(after->type));
			return;
		}
	}
	// if the 'cast' is followed by a semicolon, comma, bool or close parenthesis,
	// it isn't
	pc = chunk_get_next_ncnl(paren_close);
...
static void fix_enum_struct_union(chunk_t *pc) {
	...
	// the next item is either a type, an attribute (TODO), an identifier, a colon
	// or open brace
	...
	while (next != nullptr && !chunk_is_semicolon(next) &&
		...
		if (next->level == pc->level) {
			if (chunk_is_token(next, CT_WORD)) {
				...
				LOG_FMT(LCASTS,
				        "%s(%d): orig_line is %zu, orig_col is %zu, text() '%s', set "
				        "PCF_VAR_1ST\n",
				        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());
			}
	...
	if (next != nullptr && chunk_is_token(next, CT_SEMICOLON)) {
		set_chunk_parent(next, pc->type);
	}
...
static void fix_typedef(chunk_t *start) {
	...
	LOG_FMT(LTYPEDEF, "%s(%d): typedef @ orig_line %zu, orig_col %zu\n", __func__,
	        __LINE__, start->orig_line, start->orig_col);
	...
	if (last_op != nullptr && !(language_is_set(LANG_OC) &&
		...
		if (chunk_is_paren_close(the_type)) {
			...
			the_type =
				chunk_get_prev_ncnlni(the_type, scope_e::PREPROC); // Issue #2279
		...
		if (options::align_typedef_func() != 0) {
			LOG_FMT(LTYPEDEF,
			        "%s(%d):  -- align anchor on text() %s, @ orig_line %zu, "
			        "orig_col %zu\n",
			        __func__, __LINE__, the_type->text(), the_type->orig_line,
			        the_type->orig_col);
	...
	if (after->type != CT_ENUM && after->type != CT_STRUCT &&
	    after->type != CT_UNION) {
		if (the_type != nullptr) {
	...
	if (chunk_is_token(next, CT_BRACE_OPEN)) {
		// Skip to the closing brace
		chunk_t *br_c = chunk_get_next_type(next, CT_BRACE_CLOSE, next->level,
		                                    scope_e::PREPROC);
	...
	if (var_name != nullptr) {
		...
		while ((word_type = cs.Pop_Back()) != nullptr) {
			if (chunk_is_token(word_type, CT_WORD) ||
				...
				LOG_FMT(
					LFCNP,
					"%s(%d): parameter on orig_line %zu, orig_col %zu: <%s> as TYPE\n",
					__func__, __LINE__, var_name->orig_line, var_name->orig_col,
					word_type->text());
		...
		if (chunk_is_token(var_name, CT_WORD)) {
			if (word_cnt) {
				LOG_FMT(
					LFCNP,
					"%s(%d): parameter on orig_line %zu, orig_col %zu: <%s> as VAR\n",
					__func__, __LINE__, var_name->orig_line, var_name->orig_col,
					var_name->text());
...
static void fix_fcn_def_params(chunk_t *start) {
	...
	LOG_FMT(LFCNP,
	        "%s(%d): text() '%s', type is %s, on orig_line %zu, level is %zu\n",
	...
	while ((pc = chunk_get_next_ncnl(pc)) != nullptr) {
		if (((start->len() == 1) && (start->str[0] == ')')) || pc->level < level) {
...
static chunk_t *fix_var_def(chunk_t *start) {
	...
	while (chunk_is_token(pc, CT_TYPE) || chunk_is_token(pc, CT_WORD) ||
	       chunk_is_token(pc, CT_QUALIFIER) || chunk_is_token(pc, CT_TYPENAME) ||
	       chunk_is_token(pc, CT_DC_MEMBER) || chunk_is_token(pc, CT_MEMBER) ||
	       chunk_is_ptr_operator(pc)) {
		LOG_FMT(LFVD, "%s(%d):   1:pc->text() '%s', type is %s\n", __func__,
		        __LINE__, pc->text(), get_token_name(pc->type));
		cs.Push_Back(pc);
		pc = chunk_get_next_ncnl(pc);
	...
	if ((cs.Len() >= 3) && ((cs.Get(cs.Len() - 2)->m_pc->type == CT_MEMBER) ||
		...
		while (idx > 0) {
			...
			if (idx == 0) {
				fprintf(stderr,
				        "%s(%d): idx is ZERO, cannot be decremented, at line %zu, "
				        "column %zu\n",
	...
	LOG_FMT(LFVD2, "%s(%d): orig_line is %zu, TYPE : ", __func__, __LINE__,
	        start->orig_line);
...
static chunk_t *mark_variable_definition(chunk_t *start) {
	...
	LOG_FMT(LVARDEF,
	        "%s(%d): orig_line %zu, orig_col %zu, text() '%s', type is %s\n",
	        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
	        get_token_name(pc->type));
...
static bool can_be_full_param(chunk_t *start, chunk_t *end) {
	...
	for (pc = start; pc != nullptr && pc != end;
	     pc = chunk_get_next_ncnl(pc, scope_e::PREPROC)) {
		LOG_FMT(LFPARAM, " [%s]", pc->text());

		if (chunk_is_token(pc, CT_QUALIFIER) || chunk_is_token(pc, CT_STRUCT) ||
		...
		if (chunk_is_token(pc, CT_WORD) || chunk_is_token(pc, CT_TYPE)) {
			...
			if (!chunk_is_str(tmp3, ")", 1) || !chunk_is_str(tmp1, "*", 1) ||
			    tmp2->type != CT_WORD) {
				LOG_FMT(LFPARAM, " <== [%s] not fcn type!\n", get_token_name(pc->type));
				return (false);
			}
			...
			pc = chunk_skip_to_match(pc, scope_e::PREPROC);
		} else if (word_count == 1 && language_is_set(LANG_CPP) &&
		           chunk_is_str(pc, "&&", 2)) {
	...
	if (word_count < 2 && type_count < 1 && start->brace_level > 0) {
		...
		if (brace) {
			LOG_FMT(LFPARAM, " (matching %s brace at %zu:%zu)",
			        get_token_name(get_chunk_parent_type(brace)), brace->orig_line,
			        brace->orig_col);
		}

		if (brace && (get_chunk_parent_type(brace) == CT_CLASS ||
	...
	bool ret = (word_count >= 2 || (word_count == 1 && type_count == 1));
...
static void mark_function(chunk_t *pc) {
	...
	LOG_FMT(LFCN,
	        "%s(%d): orig_line is %zu, orig_col is %zu, text() '%s, type is %s, "
	        "parent_type is %s\n",
	        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
	        get_token_name(pc->type), get_token_name(get_chunk_parent_type(pc)));
	LOG_FMT(LFCN,
	        "   level is %zu, brace_level is %zu, next->text() '%s', next->type "
	        "is %s, next->level is %zu\n",
	        pc->level, pc->brace_level, next->text(), get_token_name(next->type),
	        next->level);

	if (pc->flags.test(PCF_IN_CONST_ARGS)) {
		set_chunk_type(pc, CT_FUNC_CTOR_VAR);
		LOG_FMT(LFCN,
		        "%s(%d):   1) Marked [%s] as FUNC_CTOR_VAR on line %zu col %zu\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
	...
	if (paren_open == nullptr || paren_close == nullptr) {
		LOG_FMT(LFCN,
		        "%s(%d): No parens found for [%s] on orig_line %zu, orig_col %zu\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
		return;
	}
	/*
	 * This part detects either chained function calls or a function ptr
	 * definition. MYTYPE (*func)(void); mWriter( "class Clst_"c )(
	 * somestr.getText() )( " : Cluster {"c ).newline;
	 *
	...
	if (tmp != nullptr && chunk_is_str(tmp, "(", 1)) {
		...
		if (chunk_is_str(tmp3, ")", 1) &&
			...
			if (tmp2) {
				LOG_FMT(LFCN,
				        "%s(%d): orig_line is %zu, orig_col is %zu, function variable "
				        "'%s', changing '%s' into a type\n",
				        __func__, __LINE__, pc->orig_line, pc->orig_col, tmp2->text(),
				        pc->text());
				...
				LOG_FMT(LFCN, "%s(%d): paren open @ orig_line %zu, orig_col %zu\n",
				        __func__, __LINE__, paren_open->orig_line,
				        paren_open->orig_col);
			} else {
				LOG_FMT(LFCN,
				        "%s(%d): orig_line is %zu, orig_col is %zu, function type, "
				        "changing '%s' into a type\n",
				        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());
		...
		LOG_FMT(LFCN,
		        "%s(%d): chained function calls? text() is '%s', orig_line is %zu, "
		        "orig_col is %zu\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
	}
	...
	if (chunk_is_token(pc, CT_FUNCTION)) {
		LOG_FMT(LFCN,
		        "%s(%d): examine: text() is '%s', orig_line is %zu, orig_col is "
		        "%zu, type is %s\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
		        get_token_name(pc->type));
		...
		if (temp != nullptr) {
			LOG_FMT(LFCN,
			        "%s(%d): assigment found, orig_line is %zu, orig_col is %zu, "
			        "text() '%s'\n",
			        __func__, __LINE__, temp->orig_line, temp->orig_col,
			        temp->text());
			set_chunk_type(pc, CT_FUNC_CALL);
		} else {
			set_chunk_type(pc, (get_chunk_parent_type(pc) == CT_OPERATOR)
			               ? CT_FUNC_DEF
			               : CT_FUNC_CALL);
		}
	}
	LOG_FMT(LFCN,
	        "%s(%d): Check for C++ function def, text() is '%s', orig_line is "
	        "%zu, orig_col is %zu, type is %s\n",
	        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
	        get_token_name(pc->type));
	...
	if (chunk_is_token(pc, CT_FUNC_CLASS_DEF) ||
	    (prev != nullptr &&
	     (chunk_is_token(prev, CT_INV) || chunk_is_token(prev, CT_DC_MEMBER)))) {
		...
		if (chunk_is_token(prev, CT_DC_MEMBER)) {
			prev = chunk_get_prev_ncnlnp(prev);
			LOG_FMT(LFCN,
			        "%s(%d): prev->text() is '%s', orig_line is %zu, orig_col is "
			        "%zu, type is %s\n",
			        __func__, __LINE__, prev->text(), prev->orig_line, prev->orig_col,
			        get_token_name(prev->type));
			...
			if (chunk_is_token(prev, CT_WORD) || chunk_is_token(prev, CT_TYPE)) {
				if (pc->str.equals(prev->str)) {
					LOG_FMT(LFCN,
					        "%s(%d): pc->text() is '%s', orig_line is %zu, orig_col is "
					        "%zu, type is %s\n",
					        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
					        get_token_name(prev->type));
					set_chunk_type(pc, CT_FUNC_CLASS_DEF);
					LOG_FMT(LFCN,
					        "%s(%d): orig_line is %zu, orig_col is %zu - FOUND "
					        "%sSTRUCTOR for '%s', type is %s\n",
					        __func__, __LINE__, prev->orig_line, prev->orig_col,
					        (destr != nullptr) ? "DE" : "CON", prev->text(),
					        get_token_name(prev->type));
	...
	if (chunk_is_token(pc, CT_FUNC_CALL) &&
		...
		if (prev == nullptr) {
			...
			LOG_FMT(
				LFCN,
				"%s(%d): Checking func call: prev->text() '%s', prev->type is %s\n",
		...
		while (prev != nullptr) {
			LOG_FMT(LFCN,
			        "%s(%d): next step with: prev->orig_line is %zu, orig_col is "
			        "%zu, text() '%s'\n",
			        __func__, __LINE__, prev->orig_line, prev->orig_col,
			        prev->text());
			...
			if (chunk_is_token(prev, CT_DC_MEMBER) ||
				...
				while (chunk_is_token(prev, CT_DC_MEMBER) ||
					...
					// Issue #1112
					// clarification: this will skip the CT_WORD, CT_TYPE or CT_THIS
					// landing on either another CT_DC_MEMBER or CT_MEMBER or a token that
					// indicates the context of the token in question; therefore, exit
					// loop when not a CT_DC_MEMBER or CT_MEMBER
					prev = chunk_get_prev_ncnlnp(prev);
					...
					if (prev == nullptr) {
						...
						LOG_FMT(LFCN,
						        "%s(%d): orig_line is %zu, orig_col is %zu, text() '%s'\n",
						        __func__, __LINE__, prev->orig_line, prev->orig_col,
						        prev->text());
					}
				}
			...
			if (chunk_is_token(prev, CT_TYPE) || chunk_is_token(prev, CT_WORD)) {
				...
				if (!chunk_is_token(prev_prev, CT_QUESTION)) // Issue #1753
				{
					...
					LOG_FMT(LFCN,
					        "%s(%d): prev is '%s', orig_line is %zu, orig_col is %zu, "
					        "type is %s, parent_type is %s\n",
					        __func__, __LINE__, prev->text(), prev->orig_line,
					        prev->orig_col, get_token_name(prev->type),
			...
			if (prev->type != CT_OPERATOR && prev->type != CT_TSQUARE &&
			    prev->type != CT_ANGLE_CLOSE && prev->type != CT_QUALIFIER &&
			    prev->type != CT_TYPE && prev->type != CT_WORD &&
			    !chunk_is_ptr_operator(prev)) {
				LOG_FMT(LFCN,
				        "%s(%d):  --> Stopping on prev is '%s', orig_line is %zu, "
				        "orig_col is %zu, type is %s\n",
				        __func__, __LINE__, prev->text(), prev->orig_line,
				        prev->orig_col, get_token_name(prev->type));

				// certain tokens are unlikely to precede a prototype or definition
				if (chunk_is_token(prev, CT_ARITH) || chunk_is_token(prev, CT_ASSIGN) ||
		...
		if (isa_def && prev != nullptr &&
		    ((chunk_is_paren_close(prev) &&
		      get_chunk_parent_type(prev) != CT_D_CAST) ||
		     prev->type == CT_ASSIGN || prev->type == CT_RETURN)) {
			LOG_FMT(LFCN,
			        "%s(%d): -- overriding DEF due to prev is '%s', type is %s\n",
			        __func__, __LINE__, prev->text(), get_token_name(prev->type));
			isa_def = false;
		}

		// Fixes issue #1266, identification of a tuple return type in CS.
		if (!isa_def && chunk_is_token(prev, CT_PAREN_CLOSE) &&
			...
			while (tmp != nullptr // Issue #2315
				...
				if (chunk_is_token(tmp, CT_COMMA) && tmp->level == prev->level + 1) {
					LOG_FMT(LFCN,
					        "%s(%d): -- overriding call due to tuple return type -- prev "
					        "is '%s', type is %s\n",
					        __func__, __LINE__, prev->text(), get_token_name(prev->type));
					isa_def = true;
					break;
		...
		if (isa_def) {
			LOG_FMT(
				LFCN,
				"%s(%d): pc is '%s', orig_line is %zu, orig_col is %zu, type is %s\n",
				__func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
				get_token_name(pc->type));
			set_chunk_type(pc, CT_FUNC_DEF);
			...
			for (tmp = prev; (tmp != nullptr) && tmp != pc;
			     tmp = chunk_get_next_ncnl(tmp)) {
				LOG_FMT(LFCN, "%s(%d): text() is '%s', type is %s\n", __func__,
				        __LINE__, tmp->text(), get_token_name(tmp->type));
				make_type(tmp);
			}
		}
	}
	...
	if (pc->type != CT_FUNC_DEF) {
		LOG_FMT(LFCN,
		        "%s(%d):  Detected type %s, text() is '%s', on orig_line %zu, "
		        "orig_col %zu\n",
		        __func__, __LINE__, get_token_name(pc->type), pc->text(),
		        pc->orig_line, pc->orig_col);

		tmp =
			flag_parens(next, PCF_IN_FCN_CALL, CT_FPAREN_OPEN, CT_FUNC_CALL, false);
	...
	while ((tmp = chunk_get_next_ncnl(tmp)) != nullptr) {
		...
		if (tmp->level < pc->level) {
			...
			if (chunk_is_token(tmp, CT_BRACE_OPEN)) {
				...
				set_chunk_type(pc, CT_FUNC_PROTO);
				LOG_FMT(LFCN,
				        "%s(%d):   2) Marked text() is '%s', as FUNC_PROTO on "
				        "orig_line %zu, orig_col %zu\n",
				        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
				break;
			} else if (chunk_is_token(pc, CT_COMMA)) {
				set_chunk_type(pc, CT_FUNC_CTOR_VAR);
				LOG_FMT(LFCN,
				        "%s(%d):   2) Marked text() is '%s', as FUNC_CTOR_VAR on "
				        "orig_line %zu, orig_col %zu\n",
				        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
				break;
			}
	 ...
	 */
	if (language_is_set(LANG_CPP) && chunk_is_token(pc, CT_FUNC_PROTO) &&
		...
		LOG_FMT(LFPARAM, "  checking '%s' for constructor variable %s %s\n",
		        pc->text(), get_token_name(paren_open->type),
		...
		if (!is_extern && !is_param) {
			set_chunk_type(pc, CT_FUNC_CTOR_VAR);
			LOG_FMT(LFCN,
			        "%s(%d):   3) Marked text() '%s' as FUNC_CTOR_VAR on orig_line "
			        "%zu, orig_col %zu\n",
			        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
		} else if (pc->brace_level > 0) {
			chunk_t *br_open =
				chunk_get_prev_type(pc, CT_BRACE_OPEN, pc->brace_level - 1);

			if (br_open != nullptr && get_chunk_parent_type(br_open) != CT_EXTERN &&
				...
				if (!chunk_is_str(prev, "*", 1) && !chunk_is_str(prev, "&", 1)) {
					...
					if (p_op != nullptr && get_chunk_parent_type(p_op) != CT_CLASS &&
					    get_chunk_parent_type(p_op) != CT_STRUCT &&
					    get_chunk_parent_type(p_op) != CT_NAMESPACE) {
						set_chunk_type(pc, CT_FUNC_CTOR_VAR);
						LOG_FMT(LFCN,
						        "%s(%d):   4) Marked text() is'%s', as FUNC_CTOR_VAR on "
						        "orig_line %zu, orig_col %zu\n",
						        __func__, __LINE__, pc->text(), pc->orig_line,
						        pc->orig_col);
					}
	...
	fix_fcn_def_params(next);
	mark_function_return_type(pc, chunk_get_prev_ncnlni(pc),
	                          pc->type); // Issue #2279

	/* mark C# where chunk */
	if (language_is_set(LANG_CS) && ((chunk_is_token(pc, CT_FUNC_DEF)) ||
	                                 (chunk_is_token(pc, CT_FUNC_PROTO)))) {
		tmp = chunk_get_next_ncnl(paren_close);
...
static void mark_cpp_constructor(chunk_t *pc) {
	...
	LOG_FMT(LFTOR,
	        "%s(%d): orig_line is %zu, orig_col is %zu, FOUND %sSTRUCTOR for "
	        "'%s'[%s] prev '%s'[%s]\n",
	        __func__, __LINE__, pc->orig_line, pc->orig_col,
	        is_destr ? "DE" : "CON", pc->text(), get_token_name(pc->type),
	...
	after = flag_parens(paren_open, PCF_IN_FCN_CALL, CT_FPAREN_OPEN,
	                    CT_FUNC_CLASS_PROTO, false);
	...
	while (tmp != nullptr &&
		...
		if (hit_colon &&
			...
			if (chunk_is_token(var, CT_TYPE) || chunk_is_token(var, CT_WORD)) {
				...
				flag_parens(tmp, PCF_IN_FCN_CALL, CT_FPAREN_OPEN, CT_FUNC_CTOR_VAR,
				            false);
			}
		}
	}
	...
	if (chunk_is_token(tmp, CT_BRACE_OPEN)) {
		...
		LOG_FMT(LFCN,
		        "%s(%d):  Marked '%s' as FUNC_CLASS_DEF on orig_line %zu, orig_col "
		        "%zu\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
	} else {
		...
		LOG_FMT(LFCN,
		        "%s(%d):  Marked '%s' as FUNC_CLASS_PROTO on orig_line %zu, "
		        "orig_col %zu\n",
		        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
	}
...
static pcf_flags_t mark_where_chunk(chunk_t *pc, c_token_t parent_type,
                                    pcf_flags_t flags) {
	/* TODO: should have options to control spacing around the ':' as well as
	 * newline ability for the
	 * constraint clauses (should it break up a 'where A : B where C : D' on the
	 * same line? wrap? etc.) */
	...
	if (chunk_is_token(pc, CT_WHERE)) {
		...
		if (chunk_is_str(pc, ":", 1)) {
			...
			LOG_FMT(LFTOR, "%s: where-spec colon on line %zu\n", __func__,
		           ...
		           (chunk_is_token(pc, CT_CLASS))) {
			/* class/struct inside of a where-clause confuses parser for indentation;
			 * set it as a word so it looks like the rest */
			set_chunk_type(pc, CT_WORD);
		}
	}
...
static void mark_class_ctor(chunk_t *start) {
	...
	LOG_FMT(LFTOR,
	        "%s(%d): orig_line is %zu, orig_col is %zu, start is '%s', "
	        "parent_type is %s\n",
	        __func__, __LINE__, start->orig_line, start->orig_col, start->text(),
	...
	if (get_chunk_parent_type(start) == CT_TEMPLATE) {
		...
		LOG_FMT(LFTOR,
		        "%s(%d): orig_line is %zu, orig_col is %zu, openingTemplate is "
		        "'%s', type is %s\n",
		        __func__, __LINE__, openingTemplate->orig_line,
		        openingTemplate->orig_col, openingTemplate->text(),
		        get_token_name(openingTemplate->type));

		if (chunk_is_token(openingTemplate, CT_ANGLE_OPEN)) {
			chunk_t *closingTemplate = chunk_skip_to_match(openingTemplate);
			LOG_FMT(LFTOR,
			        "%s(%d): orig_line is %zu, orig_col is %zu, closingTemplate is "
			        "'%s', type is %s\n",
			        __func__, __LINE__, closingTemplate->orig_line,
			        closingTemplate->orig_col, closingTemplate->text(),
			        get_token_name(closingTemplate->type));
			chunk_t *thirdToken = chunk_get_next_ncnl(closingTemplate);
			LOG_FMT(LFTOR,
			        "%s(%d): orig_line is %zu, orig_col is %zu, thirdToken is '%s', "
			        "type is %s\n",
			...
			if (chunk_is_token(thirdToken, CT_DC_MEMBER)) {
				...
				LOG_FMT(LFTOR,
				        "%s(%d): orig_line is %zu, orig_col is %zu, pclass is '%s', "
				        "type is %s\n",
	...
	while (chunk_is_token(next, CT_TYPE) || chunk_is_token(next, CT_WORD) ||
	       chunk_is_token(next, CT_DC_MEMBER)) {
		pclass = next;
		LOG_FMT(LFTOR, "%s(%d): pclass is '%s'\n", __func__, __LINE__,
		        pclass->text());
	...
	if (language_is_set(LANG_D) &&
	    chunk_is_token(next, CT_PAREN_OPEN)) // Coverity CID 76004
	{
	...
	while (pc != nullptr) {
		...
		if (pc->brace_level > level || pc->level > pc->brace_level ||
		    pc->flags.test(PCF_IN_PREPROC)) {
			pc = chunk_get_next_ncnl(pc);
			continue;
		}
		...
		if (chunkstack_match(cs, pc)) {
			LOG_FMT(LFTOR, "%s(%d): pc is '%s', orig_line is %zu, orig_col is %zu\n",
			        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
			// Issue #1333 Formatter removes semicolon after variable initializer at
			// class level(C#) if previous chunk is 'new' operator it is variable
			// initializer not a CLASS_FUNC_DEF.
			chunk_t *prev =
				chunk_get_prev_ncnlni(pc, scope_e::PREPROC); // Issue #2279
			LOG_FMT(LFTOR,
			        "%s(%d): prev is '%s', orig_line is %zu, orig_col is %zu, type "
			        "is %s\n",
			        __func__, __LINE__, prev->text(), prev->orig_line, prev->orig_col,
			        get_token_name(prev->type));
			...
			if (prev != nullptr && (prev->type != CT_NEW)) {
				...
				} else if (chunk_is_token(next, CT_ANGLE_OPEN)) // Issue # 1737
				{
					...
					LOG_FMT(LFTOR,
					        "%s(%d): text() is '%s', orig_line is %zu, orig_col is %zu, "
					        "type is %s\n",
					        __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col,
					        get_token_name(pc->type));
					make_type(pc);
	...
	while (pc) {
		...
		if (!(chunk_is_token(pc, CT_WORD) || chunk_is_token(pc, CT_TYPE))) {
			LOG_FMT(LPCU,
			        ...
			        __func__, colon->orig_line, colon->orig_col, pc->orig_line,
			        pc->orig_col);
			return (colon);
		}
		// Get next token
...
static void mark_define_expressions(void) {
	...
	while (pc != nullptr) {
		if (!in_define) {
			if (chunk_is_token(pc, CT_PP_DEFINE) || chunk_is_token(pc, CT_PP_IF) ||
			...
			if (!pc->flags.test(PCF_IN_PREPROC) || chunk_is_token(pc, CT_PREPROC)) {
				...
				if (pc->type != CT_MACRO &&
				    (first || chunk_is_token(prev, CT_PAREN_OPEN) ||
				     chunk_is_token(prev, CT_ARITH) || chunk_is_token(prev, CT_CARET) ||
				     chunk_is_token(prev, CT_ASSIGN) ||
				     chunk_is_token(prev, CT_COMPARE) ||
				     chunk_is_token(prev, CT_RETURN) || chunk_is_token(prev, CT_GOTO) ||
				     chunk_is_token(prev, CT_CONTINUE) ||
				     chunk_is_token(prev, CT_FPAREN_OPEN) ||
				     chunk_is_token(prev, CT_SPAREN_OPEN) ||
				     chunk_is_token(prev, CT_BRACE_OPEN) || chunk_is_semicolon(prev) ||
				     chunk_is_token(prev, CT_COMMA) || chunk_is_token(prev, CT_COLON) ||
...
static void handle_cpp_lambda(chunk_t *sq_o) {
	...
	if (prev == nullptr ||
	    (prev->type != CT_ASSIGN && prev->type != CT_COMMA &&
	     prev->type != CT_PAREN_OPEN // allow Js like self invoking lambda syntax:
	     // ([](){})();
	     && prev->type != CT_FPAREN_OPEN && prev->type != CT_SQUARE_OPEN &&
	     prev->type != CT_BRACE_OPEN && prev->type != CT_SEMICOLON &&
	     prev->type != CT_RETURN)) {
		return;
	}
	...
	if (br_o == nullptr || br_o->type != CT_BRACE_OPEN) {
		return;
	}
	// and now find the '}'
...
static void handle_d_template(chunk_t *pc) {
	...
	// if (!name || (name->type != CT_WORD && name->type != CT_WORD))  Coverity
	// CID 76000 Same on both sides, 2016-03-16
	if (!name || name->type != CT_WORD) {
		// TODO: log an error, expected NAME
		return;
	}

	if (po == nullptr || po->type != CT_PAREN_OPEN) {
...
static void mark_template_func(chunk_t *pc, chunk_t *pc_next) {
	...
	chunk_t *angle_close =
		chunk_get_next_type(pc_next, CT_ANGLE_CLOSE, pc->level);
		...
		if (chunk_is_str(after, "(", 1)) {
			if (angle_close->flags.test(PCF_IN_FCN_CALL)) {
				LOG_FMT(LTEMPFUNC, "%s(%d): marking '%s' in line %zu as a FUNC_CALL\n",
				        __func__, __LINE__, pc->text(), pc->orig_line);
				set_chunk_type(pc, CT_FUNC_CALL);
				flag_parens(after, PCF_IN_FCN_CALL, CT_FPAREN_OPEN, CT_FUNC_CALL,
				            false);
			} else {
				/*
				...
				LOG_FMT(LTEMPFUNC,
				        "%s(%d): marking '%s' in line %zu as a FUNC_CALL 2\n", __func__,
...
static void mark_exec_sql(chunk_t *pc) {
	...
	if (pc->type != CT_SQL_BEGIN || tmp == nullptr || tmp->type != CT_SEMICOLON) {
		return;
	}

	for (tmp = chunk_get_next(tmp); tmp != nullptr && tmp->type != CT_SQL_END;
	     tmp = chunk_get_next(tmp)) {
		tmp->level++;
	}
}
...
chunk_t *skip_template_next(chunk_t *ang_open) {
	if (chunk_is_token(ang_open, CT_ANGLE_OPEN)) {
		chunk_t *pc =
			chunk_get_next_type(ang_open, CT_ANGLE_CLOSE, ang_open->level);
...
chunk_t *skip_template_prev(chunk_t *ang_close) {
	if (chunk_is_token(ang_close, CT_ANGLE_CLOSE)) {
		chunk_t *pc =
			chunk_get_prev_type(ang_close, CT_ANGLE_OPEN, ang_close->level);
...
static void handle_oc_class(chunk_t *pc) {
	...
	bool passed_name = false; // Did we pass the name of the class and now there
	// can be only protocols, not generics
	int generic_level = 0; // level of depth of generic
	angle_state_e as = angle_state_e::NONE;

	LOG_FMT(LOCCLASS, "%s(%d): start [%s] [%s] line %zu\n", __func__, __LINE__,
	        pc->text(), get_token_name(get_chunk_parent_type(pc)), pc->orig_line);
	...
	while ((tmp = chunk_get_next_nnl(tmp)) != nullptr) {
		LOG_FMT(LOCCLASS, "%s(%d):       orig_line is %zu, [%s]\n", __func__,
		        __LINE__, tmp->orig_line, tmp->text());
		...
		if (chunk_is_str(tmp, ">", 1)) {
			...
			if (passed_name) {
				...
				if (generic_level == 0) {
					fprintf(stderr,
					        "%s(%d): generic_level is ZERO, cannot be decremented, at "
					        "line %zu, column %zu\n",
					        __func__, __LINE__, tmp->orig_line, tmp->orig_col);
					log_flush(true);
					exit(EX_SOFTWARE);
...
static void handle_oc_block_literal(chunk_t *pc) {
	...
	if (pc == nullptr || prev == nullptr || next == nullptr) {
	...
	if (apo) {
		...
		if (chunk_is_paren_close(apc)) {
			LOG_FMT(LOCBLK,
			        " -- marking parens @ apo->orig_line is %zu, apo->orig_col is "
			        "%zu and apc->orig_line is %zu, apc->orig_col is %zu\n",
...
static void handle_oc_block_type(chunk_t *pc) {
	...
	if (pc->flags.test(PCF_IN_TYPEDEF)) {
		LOG_FMT(LOCBLK,
		        "%s(%d): skip block type @ orig_line is %zu, orig_col is %zu, -- "
		        "in typedef\n",
		        __func__, __LINE__, pc->orig_line, pc->orig_col);
		return;
	}
	...
	if (chunk_is_paren_open(tpo)) {
		...
		chunk_t *nam =
			chunk_get_prev_ncnlni(tpc); // name (if any) or '^'   Issue #2279
		...
		if (chunk_is_paren_close(apc)) {
			...
			LOG_FMT(LOCBLK,
			        "%s(%d): block type @ orig_line is %zu, orig_col is %zu, text() "
			        "'%s'[%s]\n",
			        __func__, __LINE__, pc->orig_line, pc->orig_col, nam->text(),
			        get_token_name(nam->type));
			set_chunk_type(pc, CT_PTR_TYPE);
			...
			fix_fcn_def_params(apo);
			mark_function_return_type(nam, chunk_get_prev_ncnlni(tpo),
			                          pt); // Issue #2279
		}
	}
...
static chunk_t *handle_oc_md_type(chunk_t *paren_open, c_token_t ptype,
	...
	for (chunk_t *cur = chunk_get_next_ncnl(paren_open); cur != paren_close;
...
static void handle_oc_message_decl(chunk_t *pc) {
	...
	LOG_FMT(LOCMSGD, "%s(%d): %s @ orig_line is %zu, orig_col is %zu -", __func__,
	        __LINE__, get_token_name(pt), pc->orig_line, pc->orig_col);
	...
	if (chunk_is_token(pc, CT_COLON) || chunk_is_token(pc, CT_OC_COLON)) {
		...
		while (true) {
			...
			if (!did_it) {
				LOG_FMT(LWARN,
				        "%s(%d): orig_line is %zu, orig_col is %zu expected type\n",
				        __func__, __LINE__, pc->orig_line, pc->orig_col);
				break;
			}
			// attributes for a method parameter sit between the parameter type and
			// the parameter name
...
static void handle_oc_message_send(chunk_t *os) {
	...
	LOG_FMT(LOCMSG, "%s(%d): orig_line is %zu, orig_col is %zu\n", __func__,
	        __LINE__, os->orig_line, os->orig_col);
	...
	if (chunk_is_token(tmp, CT_SQUARE_OPEN) ||
	    chunk_is_token(tmp, CT_PAREN_OPEN) || (chunk_is_token(tmp, CT_OC_AT))) {
		...
		if ((chunk_is_token(tmp, CT_OC_AT)) && tt) {
			if ((chunk_is_token(tt, CT_PAREN_OPEN)) ||
				...
				LOG_FMT(LOCMSG,
				        "%s(%d): tmp->orig_line is %zu, tmp->orig_col is %zu, expected "
				        "identifier, not '%s' [%s]\n",
		...
		tmp = chunk_skip_to_match(tmp);
	           ...
	           tmp->type != CT_THIS && tmp->type != CT_STRING) {
		LOG_FMT(LOCMSG,
		        "%s(%d): orig_line is %zu, orig_col is %zu, expected identifier, "
		        "not '%s' [%s]\n",
		        __func__, __LINE__, tmp->orig_line, tmp->orig_col, tmp->text(),
		        get_token_name(tmp->type));
		return;
		...
		if (chunk_is_paren_open(tt)) {
			...
			tmp = chunk_get_prev_ncnlni(
				set_paren_parent(tt, CT_FUNC_CALL)); // Issue #2279
		} else {
	...
	// handle 'object.property' and 'collection[index]'
		...
		while (tmp) {
			...
			} else if (chunk_is_token(tmp,
			                          CT_SQUARE_OPEN)) // move past [collection[index]
			{
		...
		if (tmp->level == cs->level + 1) {
			if (chunk_is_token(tmp, CT_COLON)) {
				...
				if (chunk_is_token(prev, CT_WORD) || chunk_is_token(prev, CT_TYPE)) {
					...
					if (pp != nullptr && pp->type != CT_OC_COLON &&
					    pp->type != CT_ARITH && pp->type != CT_CARET) {
...
static void handle_oc_property_decl(chunk_t *os) {
	...
	if (options::mod_sort_oc_properties()) {
		...
		std::vector<ChunkGroup>
		ref_chunks; // retain, copy, assign, weak, strong, unsafe_unretained
		std::vector<ChunkGroup> getter_chunks; // getter
		std::vector<ChunkGroup> setter_chunks; // setter
		std::vector<ChunkGroup>
		nullability_chunks; // nonnull, nullable, null_unspecified,
		// null_resettable
		...
		if (chunk_is_token(next, CT_PAREN_OPEN)) {
			...
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(class_w, class_chunks));
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(thread_w, thread_chunks));
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(readwrite_w, readwrite_chunks));
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(ref_w, ref_chunks));
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(getter_w, getter_chunks));
			sorted_chunk_map.insert(
				pair<int, std::vector<ChunkGroup> >(setter_w, setter_chunks));
			sorted_chunk_map.insert(pair<int, std::vector<ChunkGroup> >(
							nullability_w, nullability_chunks));

			chunk_t *curr_chunk = open_paren;

			for (multimap<int, std::vector<ChunkGroup> >::reverse_iterator it =
				     sorted_chunk_map.rbegin();
			     it != sorted_chunk_map.rend(); ++it) {
...
static void handle_cs_property(chunk_t *bro) {
	...
	while ((pc = chunk_get_prev_ncnlni(pc)) != nullptr) // Issue #2279
	{
		if (pc->level == bro->level) {
			// prevent scanning back past 'new' in expressions like new List<int>
			// {1,2,3}
...
static void handle_cs_array_type(chunk_t *pc) {
	...
	for (prev = chunk_get_prev(pc); chunk_is_token(prev, CT_COMMA);
	     prev = chunk_get_prev(prev)) {
		// empty
	}
...
static void handle_wrap(chunk_t *pc) {
	...
	iarf_e pav = (pc->type == CT_FUNC_WRAP) ? options::sp_func_call_paren()
	             : options::sp_cpp_cast_paren();

	log_rule_B("sp_inside_fparen");
	log_rule_B("sp_inside_paren_cast");
	iarf_e av = (pc->type == CT_FUNC_WRAP) ? options::sp_inside_fparen()
...
static void handle_proto_wrap(chunk_t *pc) {
	...
	while ((tmp = chunk_get_prev_ncnlni(tmp)) != nullptr) // Issue #2279
	{
		if (!chunk_is_type(tmp) && tmp->type != CT_OPERATOR &&
		    tmp->type != CT_WORD && tmp->type != CT_ADDR) {
			break;
		}
		set_chunk_parent(tmp, pc->type);
		make_type(tmp);
