...
static iarf_e do_space(chunk_t *first, chunk_t *second, int &min_sp) {
  ...
  LOG_FMT(LSPACE,
          "%s(%d): orig_line is %zu, orig_col is %zu, first->text() '%s', type "
          "is %s\n",
          __func__, __LINE__, first->orig_line, first->orig_col, first->text(),
          get_token_name(first->type));
  ...
  if (chunk_is_token(first, CT_PP_IGNORE) &&
    ...
    // Leave spacing alone between PP_IGNORE tokens as we don't want the default
    // behavior (which is ADD).
    log_rule("PP_IGNORE");
    return (IARF_IGNORE);
  }
  ...
  if (chunk_is_token(second, CT_POUND) && second->flags.test(PCF_IN_PREPROC) &&
  ...
  if (chunk_is_token(first, CT_VBRACE_OPEN) && second->type != CT_NL_CONT &&
  ...
  if (chunk_is_token(first, CT_CASE) && ((CharTable::IsKw1(second->str[0]) ||
                                          chunk_is_token(second, CT_NUMBER)))) {
    // Fix the spacing between 'case' and the label. Only 'ignore' and 'force'
    // make sense here.
  ...
  if (chunk_is_token(first, CT_QUESTION) &&
    ...
    if (options::sp_cond_ternary_short() != IARF_IGNORE) {
      ...
      // '?' and ':'. Overrides all other sp_cond_* options.
  ...
  if (chunk_is_token(first, CT_QUESTION) ||
    ...
    // replace "if (chunk_is_token(first, CT_PAREN_CLOSE) &&
    // chunk_is_token(second, CT_QUESTION))"
  ...
  if (chunk_is_token(first, CT_COND_COLON) ||
    ...
    // Issue #2596
    // replace "if (chunk_is_token(first, CT_WORD) && chunk_is_token(second,
    // CT_COND_COLON))" Add or remove space around the ':' in 'b ? t : f'.
  ...
  if (chunk_is_token(second, CT_SEMICOLON)) {
    if (get_chunk_parent_type(second) == CT_FOR) {
          ...
          || chunk_is_token(first, CT_SEMICOLON)) // b
      {
        ...
        //                ^ is first   // b
        //                 ^ is second // b
        // Add or remove space before a semicolon of an empty part of a for
        // statement.
    ...
    if (chunk_is_token(first, CT_SPAREN_CLOSE) &&
      ...
      // Add or remove space before empty statement ';' on 'if', 'for' and
      // 'while'.
  ...
  if (chunk_is_token(first, CT_SEMICOLON)) {
    if (get_chunk_parent_type(first) == CT_FOR) {
      if ((options::sp_after_semi_for_empty() != IARF_IGNORE) &&
        ...
        // Add or remove space after the final semicolon of an empty part of a
        // for statement, as in 'for ( ; ; <here> )'.
  ...
  if ((chunk_is_token(first, CT_NEG) || chunk_is_token(first, CT_POS) ||
       chunk_is_token(first, CT_ARITH)) &&
      (chunk_is_token(second, CT_NEG) || chunk_is_token(second, CT_POS) ||
  ...
  if (chunk_is_token(first, CT_PAREN_CLOSE) &&
      chunk_is_token(second, CT_DC_MEMBER) && second->next != nullptr &&
  ...
  if (chunk_is_token(second, CT_DC_MEMBER)) {
    /* '::' at the start of an identifier is not member access, but global scope
     * operator. Detect if previous chunk is keyword
     */
    switch (first->type) {
    case CT_SBOOL:
    case CT_SASSIGN:
    ...
    // Issue #1005
    /* '::' at the start of an identifier is not member access, but global scope
     * operator. Detect if previous chunk is a type and previous-previous is
     * "friend"
     */
    if (chunk_is_token(first, CT_TYPE) && first->prev != nullptr &&
        (first->prev->type == CT_FRIEND && first->next->type != CT_DC_MEMBER)) {
      log_rule("FORCE");
      return (IARF_FORCE);
    }

    if ((chunk_is_token(first, CT_WORD) || chunk_is_token(first, CT_TYPE) ||
  ...
  if (chunk_is_token(first, CT_COMMA)) {
    if (get_chunk_parent_type(first) == CT_TYPE) {
      ...
      // (C#) Add or remove space between ',' and ']' in multidimensional array
      // type like 'int[,,]'.
  ...
  if (chunk_is_token(second, CT_ELLIPSIS)) {
    ...
    if (chunk_is_token(tmp, CT_PTR_TYPE) || chunk_is_token(tmp, CT_BYREF)) {
      tmp = chunk_get_prev_ncnl(tmp);
    }

    if (chunk_is_token(tmp, CT_TYPE) || chunk_is_token(tmp, CT_QUALIFIER)) {
  ...
  if (chunk_is_token(first, CT_ELLIPSIS)) {
    ...
    if (chunk_is_token(second, CT_PAREN_OPEN) && first->prev &&
  ...
  if (language_is_set(LANG_OC) && chunk_is_token(first, CT_CATCH) &&
  ...
  if (language_is_set(LANG_OC) && chunk_is_token(first, CT_OC_CLASS) &&
  ...
  if (language_is_set(LANG_OC) && chunk_is_token(first, CT_SYNCHRONIZED) &&
      chunk_is_token(second, CT_SPAREN_OPEN)) {
    // (OC) Add or remove space between '@synchronized' and the open
    // parenthesis, i.e. '@synchronized(foo)' vs. '@synchronized (foo)'.
  ...
  if (chunk_is_token(second, CT_OC_MSG_FUNC)) {
    if ((options::sp_after_oc_msg_receiver() == IARF_REMOVE) &&
        ((first->type != CT_SQUARE_CLOSE) && (first->type != CT_FPAREN_CLOSE) &&
  ...
  // c++17 structured bindings e.g., "auto [x, y, z]" vs a[x, y, z]" or "auto
  // const [x, y, z]" vs "auto const[x, y, z]"
  if (language_is_set(LANG_CPP) &&
      (chunk_is_token(first, CT_BYREF) || chunk_is_token(first, CT_QUALIFIER) ||
  ...
  if (chunk_is_token(first, CT_ANGLE_OPEN) ||
    ...
    if ((op == IARF_REMOVE) && !options::enable_digraphs() &&
  ...
  if (chunk_is_token(first, CT_ANGLE_CLOSE)) {
    ...
    if (chunk_is_token(second, CT_FPAREN_OPEN) ||
      ...
      if (chunk_is_token(next, CT_FPAREN_CLOSE)) {
        // Add or remove space between '>' and '()' as found in 'new
        // List<byte>();'.
    ...
    if (second->type != CT_BYREF && second->type != CT_PTR_TYPE &&
        second->type != CT_BRACE_OPEN && second->type != CT_PAREN_CLOSE) {
  ...
  if (chunk_is_token(first, CT_BYREF)) {
    if (options::sp_after_byref_func() != IARF_IGNORE &&
        (get_chunk_parent_type(first) == CT_FUNC_DEF ||
         get_chunk_parent_type(first) == CT_FUNC_PROTO)) {
      // Add or remove space after a reference sign '&', if followed by a
  ...
  if (chunk_is_token(second, CT_BYREF) &&
    ...
    if (options::sp_before_byref_func() != IARF_IGNORE) {
      ...
      if (next != nullptr && (get_chunk_parent_type(next) == CT_FUNC_DEF ||
    ...
    if (chunk_is_token(second, CT_BRACE_OPEN)) {
      if (get_chunk_parent_type(second) == CT_CATCH) {
        if (language_is_set(LANG_OC) &&
          ...
          // (OC) Add or remove space before the '{' of a '@catch' statement, if
          // the '{' and '@catch' are on the same line, as in '@catch (decl)
          // <here> {'. If set to ignore, sp_catch_brace is used.
          log_rule("sp_oc_catch_brace");
          return (options::sp_oc_catch_brace());
        }

        if (options::sp_catch_brace() != IARF_IGNORE) {
          // Add or remove space before the '{' of a 'catch' statement, if the
  ...
  if (chunk_is_token(second, CT_FPAREN_OPEN) &&
    ...
    if ((options::sp_after_operator_sym_empty() != IARF_IGNORE) &&
      ...
      if (chunk_is_token(next, CT_FPAREN_CLOSE)) {
        // Overrides sp_after_operator_sym when the operator has no arguments,
        // as in 'operator *()'.
  ...
  if (language_is_set(LANG_VALA) && chunk_is_token(first, CT_FUNC_CALL)) {
    if (chunk_is_str(first, "_", 1) && chunk_is_token(second, CT_FPAREN_OPEN) &&
  ...
  if (chunk_is_token(first, CT_FUNC_CALL_USER)) {
    ...
    // calls. You need to set a keyword to be a user function in the config
    // file, like:
  ...
  if (chunk_is_token(first, CT_FUNC_DEF)) {
    if ((options::sp_func_def_paren_empty() != IARF_IGNORE) &&
      ...
      if (chunk_is_token(next, CT_FPAREN_CLOSE)) {
        // Add or remove space between function name and '()' on function
  ...
  if (chunk_is_token(first, CT_PAREN_CLOSE) &&
    ...
    if (get_chunk_parent_type(first) == CT_C_CAST ||
      ...
      // Add or remove space after C/D cast, i.e. 'cast(int)a' vs. 'cast(int) a'
      // or
      // '(int)a' vs. '(int) a'.
      log_rule("sp_after_cast");
      return (options::sp_after_cast());
  ...
  if (chunk_is_token(first, CT_FUNC_TYPE) &&
      chunk_is_token(second, CT_FPAREN_OPEN)) {
    // Add or remove space between function name and '(' with a typedef
    // specifier.
  ...
  if (chunk_is_token(first, CT_FUNC_CLASS_DEF) ||
    ...
    if ((options::sp_func_class_paren_empty() != IARF_IGNORE) &&
      ...
      if (chunk_is_token(next, CT_FPAREN_CLOSE)) {
        // Add or remove space between a constructor without parameters or
        // destructor and '()'.
  ...
  if (chunk_is_token(second, CT_BRACE_CLOSE)) {
    ...
    if (get_chunk_parent_type(second) == CT_BRACED_INIT_LIST) {
      ...
      if (options::sp_inside_type_brace_init_lst() != IARF_IGNORE) {
        // Add or remove space inside an unnamed temporary
        // direct-list-initialization.
  ...
  if (chunk_is_token(first, CT_FPAREN_CLOSE) &&
    ...
    if (get_chunk_parent_type(second) == CT_DOUBLE_BRACE) {
      // (Java) Add or remove space between ')' and '{{' of double brace
      // initializer.
    ...
    if (get_chunk_parent_type(first) == CT_FUNC_CALL) {
      chunk_t *tmp = chunk_get_prev_type(first, get_chunk_parent_type(first),
                                         first->level);
      tmp = chunk_get_prev_ncnl(tmp);
  ...
  if (chunk_is_token(first, CT_WORD) && chunk_is_token(second, CT_BRACE_OPEN)) {
    ...
    if (get_chunk_parent_type(first) == CT_NONE &&
        get_chunk_parent_type(second) == CT_NONE) {
  ...
  if (chunk_is_token(first, CT_TPAREN_OPEN) ||
    ...
    // Add or remove space inside the first parentheses in a function type, as
    // in 'void (*x)(...)'.
  ...
  if (chunk_is_token(first, CT_SQUARE_OPEN) ||
    ...
    if (language_is_set(LANG_OC) &&
      ...
      // (OC) Add or remove space inside a non-empty Objective-C boxed array
      // '@[' and
  ...
  if (chunk_is_token(second, CT_CLASS_COLON)) {
    if (get_chunk_parent_type(second) == CT_OC_CLASS &&
        (!chunk_get_prev_type(second, CT_OC_INTF, second->level,
                              scope_e::ALL) &&
         !chunk_get_prev_type(second, CT_OC_IMPL, second->level,
                              scope_e::ALL))) {
      if (get_chunk_parent_type(second) == CT_OC_CLASS &&
          !chunk_get_prev_type(second, CT_OC_INTF, second->level,
                               scope_e::ALL)) {
  ...
  if ((options::sp_after_constr_colon() != IARF_IGNORE) &&
    ...
    min_sp =
        options::indent_ctor_init_leading() - 1; // default indent is 1 space
  ...
  if (chunk_is_token(first, CT_ARITH) || chunk_is_token(first, CT_CARET) ||
      chunk_is_token(second, CT_ARITH) || chunk_is_token(second, CT_CARET)) {
    // Add or remove space around arithmetic operators '+' and '-'.
    // Overrides sp_arith.
    if (options::sp_arith_additive() != IARF_IGNORE) {
      auto arith_char =
          (chunk_is_token(first, CT_ARITH) || chunk_is_token(first, CT_CARET))
              ? first->str[0]
              : second->str[0];
  ...
  if (chunk_is_token(first, CT_BOOL) || chunk_is_token(second, CT_BOOL)) {
    ...
    if ((options::pos_bool() != TP_IGNORE) &&
        first->orig_line != second->orig_line && arg != IARF_REMOVE) {
      arg = arg | IARF_ADD;
    }
    // TODO check it
    log_rule("sp_bool");
  ...
  if (chunk_is_token(first, CT_PTR_TYPE) && CharTable::IsKw1(second->str[0])) {
    ...
    if (chunk_is_token(prev, CT_IN)) {
      // Add or remove space after the '*' (dereference) unary operator. This
      // does not affect the spacing after a '*' that is part of a type.
      log_rule("sp_deref");
  ...
  if (chunk_is_token(first, CT_BRACE_CLOSE)) {
    ...
    if (language_is_set(LANG_OC) && chunk_is_token(second, CT_CATCH) &&
  ...
  if (chunk_is_token(first, CT_BRACE_CLOSE) &&
      ...
      (get_chunk_parent_type(first) == CT_ENUM ||
       ...
       get_chunk_parent_type(first) == CT_UNION)) {
    // Add or remove space between '}' and the name of a typedef on the same
    // line.
  ...
  if (language_is_set(LANG_VALA) && chunk_is_token(second, CT_QUESTION)) {
    // Issue #2090
  ...
  if (chunk_is_token(first, CT_ADDR)) {
    // Add or remove space after the '&' (address-of) unary operator. This does
  ...
  if (chunk_is_token(first, CT_OC_DICT_COLON)) {
    // (OC) Add or remove space after the colon in immutable dictionary
    // expression 'NSDictionary *test = @{@"foo" :@"bar"};'.
    log_rule("sp_after_oc_dict_colon");
    return (options::sp_after_oc_dict_colon());
  }

  if (chunk_is_token(second, CT_OC_DICT_COLON)) {
    // (OC) Add or remove space before the colon in immutable dictionary
    // expression 'NSDictionary *test = @{@"foo" :@"bar"};'.
  ...
  if (chunk_is_token(first, CT_NEW) || chunk_is_token(first, CT_DELETE) ||
  ...
  if (chunk_is_token(first, CT_ANNOTATION) && chunk_is_paren_open(second)) {
    // (Java) Add or remove space between an annotation and the open
    // parenthesis.
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_ASSIGN_FUNC_PROTO) &&
      chunk_is_token(second, CT_DEFAULT)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_ASSIGN_FUNC_PROTO) &&
      chunk_is_token(second, CT_DELETE)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_COND_COLON) &&
      chunk_is_token(second, CT_STRING)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  if (chunk_is_token(first, CT_CONSTR_COLON) &&
      chunk_is_token(second, CT_FUNC_CTOR_VAR)) {
    log_rule("ADD");
    return (IARF_ADD);
  }

  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_CS_PROPERTY) &&
      chunk_is_token(second, CT_BRACE_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_FPAREN_CLOSE) &&
      chunk_is_token(second, CT_ASSIGN_FUNC_PROTO)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_LABEL_COLON) &&
      chunk_is_token(second, CT_STRING)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_NUMBER_FP) &&
      chunk_is_token(second, CT_NUMBER)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_OC_CLASS) &&
      chunk_is_token(second, CT_CLASS_COLON)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_OC_IMPL) &&
      chunk_is_token(second, CT_OC_CLASS)) {
    log_rule("ADD");
    return (IARF_ADD);
  }

  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_OC_INTF) &&
      chunk_is_token(second, CT_OC_CLASS)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_OC_MSG_DECL) &&
      chunk_is_token(second, CT_BRACE_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // if (chunk_is_token(first, CT_PAREN_CLOSE) && chunk_is_token(second,
  // CT_QUESTION)) Issue #2596 look at "sp_cond_question"
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_DEFINE) &&
      chunk_is_token(second, CT_MACRO_FUNC)) {
    log_rule("ADD");
    return (IARF_ADD);
  }

  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_ELSE) &&
      chunk_is_token(second, CT_PAREN_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_IF) &&
      chunk_is_token(second, CT_PAREN_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_IF) &&
      chunk_is_token(second, CT_PP_DEFINED)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_INCLUDE) &&
      chunk_is_token(second, CT_STRING)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_PP_OTHER) &&
      chunk_is_token(second, CT_PREPROC_BODY)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_SQL_EXEC) &&
      chunk_is_token(second, CT_SQL_WORD)) {
    log_rule("ADD");
    return (IARF_ADD);
  }

  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_SQL_WORD) &&
      chunk_is_token(second, CT_PAREN_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_TSQUARE) &&
      chunk_is_token(second, CT_BRACE_OPEN)) {
    log_rule("ADD");
    return (IARF_ADD);
  }
  ...
  // TODO: if necessary create a new option
  if (chunk_is_token(first, CT_TYPENAME) &&
  ...
  // if (chunk_is_token(first, CT_WORD) && chunk_is_token(second,
  // CT_COND_COLON)) Issue #2596 look at "sp_cond_colon"
  ...
  D_LOG_FMT(LSPACE, "\n\n%s(%d): WARNING: unrecognize do_space:\n", __func__,
            __LINE__);
  D_LOG_FMT(LSPACE,
            "   first->orig_line  is %zu, first->orig_col  is %zu, "
            "first->text()  '%s', first->type is  %s\n",
            first->orig_line, first->orig_col, first->text(),
            get_token_name(first->type));
  D_LOG_FMT(LSPACE,
            "   second->orig_line is %zu, second->orig_col is %zu, "
            "second->text() '%s', second->type is %s\n",
            second->orig_line, second->orig_col, second->text(),
            get_token_name(second->type));
...
static iarf_e ensure_force_space(chunk_t *first, chunk_t *second, iarf_e av) {
  if (first->flags.test(PCF_FORCE_SPACE)) {
    LOG_FMT(LSPACE, " <force between '%s' and '%s'>", first->text(),
            second->text());
    return (av | IARF_ADD);
  }
  return (av);
...
void space_text(void) {
  ...
  while (pc != nullptr) {
    if (chunk_is_token(pc, CT_NEWLINE)) {
      ...
      LOG_FMT(LSPACE,
              "%s(%d): orig_line is %zu, orig_col is %zu, '%s' type is %s\n",
    ...
    if ((options::use_options_overriding_for_qt_macros()) &&
      ...
      chunk_flags_set(
          pc, PCF_IN_QT_MACRO); // flag the chunk for a second processing
    ...
    if (options::sp_skip_vbrace_tokens()) {
      ...
      while (chunk_is_blank(next) && !chunk_is_newline(next) &&
        ...
        LOG_FMT(
            LSPACE,
            "%s(%d): orig_line is %zu, orig_col is %zu, Skip %s (%zu+%zu)\n",
            __func__, __LINE__, next->orig_line, next->orig_col,
            get_token_name(next->type), pc->column, pc->str.size());
     ...
     * of the next item
     */
    if (chunk_is_token(pc, CT_NEWLINE) || chunk_is_token(pc, CT_NL_CONT) ||
      ...
      if ((pc->len() > 0) && !chunk_is_str(pc, "[]", 2) &&
          !chunk_is_str(pc, "{{", 2) && !chunk_is_str(pc, "}}", 2) &&
          !chunk_is_str(pc, "()", 2) && !pc->str.startswith("@\"")) {
        ...
        if (tmp != nullptr && tmp->len() > 0) {
          ...
          if (kw1 && kw2) {
            ...
            LOG_FMT(LSPACE,
                    "%s(%d): back-to-back words need a space: pc->text() '%s', "
                    "next->text() '%s'\n",
          ...
          // TODO:  what is the meaning of 4
            ...
            if (ct != nullptr && (strlen(ct->tag) != pc->len())) {
               ...
               */
              // (C++11) Permit removal of the space between '>>' in
              // 'foo<bar<int> >'. Note that sp_angle_shift cannot remove the
              // space without this option.
              if (((language_is_set(LANG_CPP) &&
                ...
                LOG_FMT(LSPACE,
                        "%s(%d): : pc->text() is %s, next->text() is %s\n",
                        __func__, __LINE__, pc->text(), next->text());
                chunk_flags_set(pc, PCF_FORCE_SPACE);
              }
            }
      ...
      int min_sp;
      LOG_FMT(LSPACE,
              "%s(%d): orig_line is %zu, orig_col is %zu, pc-text() '%s', type "
              "is %s\n",
              __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
              get_token_name(pc->type));
      ...
      if (chunk_is_comment(next) && chunk_is_newline(chunk_get_next(next)) &&
          column < next->orig_col) {
        /*
        ...
        if ((options::sp_before_tr_emb_cmt() == IARF_IGNORE ||
          ...
          if (options::indent_relative_single_line_comments()) {
            ...
            LOG_FMT(LSPACE,
                    "%s(%d): pc is '%s', pc->orig_col is %zu, next->orig_col "
                    "is %zu, pc->orig_col_end is %zu\n",
                    __func__, __LINE__, pc->text(), pc->orig_col,
                    next->orig_col, pc->orig_col_end);
             ...
             */
            size_t col_min =
                pc->column + pc->len() + ((next->orig_prev_sp > 0) ? 1 : 0);
            column = next->orig_col;
...
void space_text_balance_nested_parens(void) {
  ...
  while (first != nullptr) {
    ...
    if (chunk_is_str(first, "(", 1) && chunk_is_str(next, "(", 1)) {
      ...
      chunk_t *closing = chunk_get_next_type(
          first, (c_token_t)(first->type + 1), first->level);
      ...
      chunk_t *opening =
          chunk_get_prev_type(next, (c_token_t)(next->type - 1), next->level);
...
size_t space_col_align(chunk_t *first, chunk_t *second) {
  ...
  LOG_FMT(LSPACE,
          "%s(%d): first->orig_line is %zu, orig_col is %zu, [%s/%s], text() "
          "'%s' <==>\n",
          __func__, __LINE__, first->orig_line, first->orig_col,
          get_token_name(first->type),
          get_token_name(get_chunk_parent_type(first)), first->text());
  LOG_FMT(
      LSPACE,
      "%s(%d): second->orig_line is %zu, orig_col is %zu [%s/%s], text() '%s',",
      __func__, __LINE__, second->orig_line, second->orig_col,
      get_token_name(second->type),
      get_token_name(get_chunk_parent_type(second)), second->text());
