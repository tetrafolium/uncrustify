...
struct include_category {
	include_category(const std::string &pattern) : regex(pattern) {
	}
	std::regex regex;
};
...
static int compare_chunks(chunk_t *pc1, chunk_t *pc2, bool tcare) {
	...
	while (pc1 != nullptr && pc2 != nullptr) {
		...
		LOG_FMT(LSORT,
		        "%s(%d): text is %s, pc1->len is %zu, line is %zu, column is %zu\n",
		        __func__, __LINE__, pc1->text(), pc1->len(), pc1->orig_line,
		        pc1->orig_col);
		LOG_FMT(LSORT,
		        "%s(%d): text is %s, pc2->len is %zu, line is %zu, column is %zu\n",
		        __func__, __LINE__, pc2->text(), pc2->len(), pc2->orig_line,
		        pc2->orig_col);
		auto const &s1 = chunk_sort_str(pc1);
		auto const &s2 = chunk_sort_str(pc2);
		int ret_val =
			unc_text::compare(s1, s2, std::min(s1.size(), s2.size()), tcare);
		...
		if (pc1 == nullptr || chunk_is_newline(pc1) || pc2 == nullptr ||
...
static void do_the_sort(chunk_t **chunks, size_t num_chunks) {
	...
	LOG_FMT(LSORT, "%s(%d): %zu chunks:", __func__, __LINE__, num_chunks);
	...
	for (start_idx = 0; start_idx < (num_chunks - 1); start_idx++) {
		...
		for (size_t idx = start_idx + 1; idx < num_chunks; idx++) {
			if (compare_chunks(chunks[idx], chunks[min_idx], take_care) <
			    0) // Issue #2091
			{
				min_idx = idx;
			}
...
void sort_imports(void) {
	LOG_FUNC_ENTRY();
	chunk_t *
	        chunks[MAX_NUMBER_TO_SORT]; // MAX_NUMBER_TO_SORT should be enough, right?
	size_t num_chunks = 0;
		...
		if (chunk_is_newline(pc)) {
			...
			if (p_imp != nullptr && p_last != nullptr &&
				...
				if (num_chunks < MAX_NUMBER_TO_SORT) {
					...
					fprintf(stderr,
					        "Number of 'import' to be sorted is too big for the current "
					        "value %d.\n",
					        MAX_NUMBER_TO_SORT);
