...
void AlignStack::Add(chunk_t *start, size_t seqnum) {
  ...
  while (
      (prev = chunk_get_prev(prev)) != nullptr &&
  ...
  if (m_star_style != SS_IGNORE) {
    ...
    if (chunk_is_token(tmp_prev, CT_TPAREN_OPEN)) {
      ...
      // CID 76021 (#1 of 1): Unused value (UNUSED_VALUE)returned_pointer:
      // Assigning value from chunk_get_prev(ali, nav_e::ALL) to prev here, but
  ...
  if (!options::align_keep_extra_space()) {
    ...
    while (tmp != nullptr && tmp != start) {
      ...
      if (next != nullptr) {
        LOG_FMT(LAS,
                "AlignStack::%s(%d): next->orig_line is %zu, orig_col is %zu, "
                "text() '%s', level is %zu, type is %s\n",
                __func__, __LINE__, next->orig_line, next->orig_col,
                next->text(), next->level, get_token_name(next->type));
        tmp_col += space_col_align(tmp, next);
        LOG_FMT(LAS,
                "AlignStack::%s(%d): next->column is %zu, level is %zu, "
                "tmp_col is %zu\n",
  ...
  if (m_max_col == 0 || m_thresh == 0 ||
      (((start->column + m_gap) <=
        (m_thresh +
         (m_absolute_thresh
              ? m_min_col
              : m_max_col))) // don't use subtraction here to prevent underflow
       && ((start->column + m_gap + m_thresh) >=
               m_max_col // change the expression to mind negative expression
    ...
    const size_t endcol =
        ali->column + col_adj + (gap < m_gap ? m_gap - gap : 0);
    ...
    LOG_FMT(LAS,
            "AlignStack::%s(%d): Add-[%s]: ali->orig_line is %zu, ali->column "
            "is %zu, type is %s\n",
            __func__, __LINE__, ali->text(), ali->orig_line, ali->column,
            get_token_name(ali->type));
    LOG_FMT(LAS,
            "AlignStack::%s(%d):    ali->align.col_adj is %d, ref '%s', endcol "
            "is %zu\n",
    ...
    if (endcol > m_max_col) {
      LOG_FMT(LAS,
              "AlignStack::%s(%d): Add-aligned: seqnum is %zu, m_nl_seqnum is "
              "%zu, m_seqnum is %zu\n",
              __func__, __LINE__, seqnum, m_nl_seqnum, m_seqnum);
      LOG_FMT(LAS,
              "AlignStack::%s(%d):    ali->orig_line is %zu, ali->column is "
              "%zu, max_col old is %zu, new is %zu, m_min_col is %zu\n",
              __func__, __LINE__, ali->orig_line, ali->column, m_max_col,
              endcol, m_min_col);
      m_max_col = endcol;
    ...
    LOG_FMT(LAS,
            "AlignStack::Add-skipped [%zu/%zu/%zu]: line %zu, col %zu <= %zu + "
            "%zu\n",
            seqnum, m_nl_seqnum, m_seqnum, start->orig_line, start->column,
            m_max_col, m_thresh);
  }
} // AlignStack::Add
...
void AlignStack::NewLines(size_t cnt) {
  ...
  m_seqnum += cnt;
  LOG_FMT(LAS,
          "AlignStack::Newlines(%d): m_seqnum is %zu, m_nl_seqnum is %zu, "
          "m_span is %zu, \n",
...
void AlignStack::Flush() {
  ...
  for (size_t idx = 0; idx < m_aligned.Len(); idx++) {
    ...
    if (m_star_style == SS_DANGLE) {
      chunk_t *tmp =
          (chunk_is_token(pc, CT_TPAREN_OPEN)) ? chunk_get_next(pc) : pc;

      if (chunk_is_ptr_operator(tmp)) {
        col_adj = pc->align.start->column - pc->column;
        gap = pc->align.start->column -
              (pc->align.ref->column + pc->align.ref->len());
      }
    }
    ...
    // See if this pushes out the max_col
    const size_t endcol =
        pc->column + col_adj + (gap < m_gap ? m_gap - gap : 0);
  ...
  for (size_t idx = 0; idx < m_aligned.Len(); idx++) {
    ...
    if (idx == 0) {
      if (m_skip_first && pc->column != tmp_col) {
        LOG_FMT(LAS,
                "AlignStack::%s(%d): orig_line is %zu, orig_col is %zu, "
                "dropping first item due to skip_first\n",
    ...
    LOG_FMT(LAS,
            "AlignStack::%s(%d): orig_line is %zu, text() '%s', to col %zu "
            "(adj is %d)\n",
            __func__, __LINE__, pc->orig_line, pc->text(), tmp_col,
            pc->align.col_adj);
