...
void align_to_column(chunk_t *pc, size_t column) {
    ...
    const auto col_delta =
        static_cast<int>(column) - static_cast<int>(pc->column);
    ...
    do {
        ...
        if (chunk_is_comment(pc) && get_chunk_parent_type(pc) != CT_COMMENT_EMBED) {
            ...
            almod = (chunk_is_single_line_comment(pc) &&
                    ...
                    ? align_mode_e::KEEP_REL
                    : align_mode_e::KEEP_ABS;
        }
        ...
        if (almod == align_mode_e::KEEP_ABS) {
            ...
            auto orig_delta =
                static_cast<int>(pc->orig_col) - static_cast<int>(prev->orig_col);
        ...
        } else // SHIFT
        {
            ...
            pc->column =
                (col_delta >= 0 || cast_abs(pc->column, col_delta) < pc->column)
                ? pc->column + col_delta
                : 0;
            pc->column = max(pc->column, min_col);
        }
        LOG_FMT(LINDLINED,
                "%s(%d):   %s set column of '%s', type is %s, orig_line is %zu, to "
                "col %zu (orig_col was %zu)\n",
                __func__, __LINE__,
                (almod == align_mode_e::KEEP_ABS)
                ? "abs"
                : (almod == align_mode_e::KEEP_REL) ? "rel" : "sft",
                pc->text(), get_token_name(pc->type), pc->orig_line, pc->column,
                pc->orig_col);
...
void reindent_line(chunk_t *pc, size_t column) {
    LOG_FUNC_ENTRY();
    LOG_FMT(LINDLINE,
            "%s(%d): orig_line is %zu, orig_col is %zu, on '%s' [%s/%s] => %zu\n",
            __func__, __LINE__, pc->orig_line, pc->column, pc->text(),
    ...
    do {
        if (QT_SIGNAL_SLOT_found) {
            ...
            // SLOT(onSomeEvent(QString &))); look for end of SIGNAL/SLOT block
        ...
        log_rule_B("indent_relative_single_line_comments");
        const bool keep = (is_comment && chunk_is_single_line_comment(pc) &&
                           options::indent_relative_single_line_comments());

        if (is_comment && get_chunk_parent_type(pc) != CT_COMMENT_EMBED && !keep) {
            pc->column = max(pc->orig_col, min_col);
            LOG_FMT(LINDLINE,
                    "%s(%d): set comment on line %zu to col %zu (orig %zu)\n",
...
#define indent_column_set(X)                                                   \
  do {                                                                         \
    LOG_FMT(                                                                   \
        LINDENT2,                                                              \
        "%s(%d): orig_line is %zu, indent_column changed from %zu to %zu\n",   \
...
#define log_indent()                                                           \
  do {                                                                         \
    _log_indent(__func__, __LINE__, frm);                                      \
  } while (false)

static void _log_indent(const char *func, const uint32_t line,
                        const ParseFrame &frm) {
    LOG_FMT(LINDLINE, "%s(%d): frm.pse_tos is %zu, ...indent is %zu\n", func,
            line, frm.size() - 1, frm.top().indent);
}

#define log_prev_indent()                                                      \
  do {                                                                         \
    _log_prev_indent(__func__, __LINE__, frm);                                 \
  } while (false)

static void _log_prev_indent(const char *func, const uint32_t line,
                             const ParseFrame &frm) {
    LOG_FMT(LINDLINE, "%s(%d): frm.pse_tos is %zu, prev....indent is %zu\n", func,
            line, frm.size() - 1, frm.prev().indent);
}

#define log_indent_tmp()                                                       \
  do {                                                                         \
    _log_indent_tmp(__func__, __LINE__, frm);                                  \
  } while (false)

static void _log_indent_tmp(const char *func, const uint32_t line,
                            const ParseFrame &frm) {
    LOG_FMT(LINDLINE, "%s(%d): frm.pse_tos is %zu, ...indent_tmp is %zu\n", func,
            line, frm.size() - 1, frm.top().indent_tmp);
}
...
static void quick_indent_again(void) {
    ...
    for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
        ...
        LOG_FMT(LINDENTAG,
                "%s(%d): [%zu] indent [%s] to %zu based on [%s] @ %zu:%zu\n",
...
void indent_text(void) {
    ...
    while (pc != nullptr) {
        ...
        if (chunk_is_token(pc, CT_NEWLINE)) {
            ...
            LOG_FMT(LINDLINE,
                    "%s(%d): orig_line is %zu, orig_col is %zu, column is %zu, for "
                    "'%s'\n   ",
                    __func__, __LINE__, pc->orig_line, pc->orig_col, pc->column,
                    pc->text());
        ...
        const size_t parent_token_indent =
            (options::indent_brace_parent())
            ? token_indent(get_chunk_parent_type(pc))
            : 0;
        ...
        if (options::indent_func_def_force_col1()) {
            if (!in_func_def) {
                ...
                if (get_chunk_parent_type(pc) == CT_FUNC_DEF ||
                        (chunk_is_token(pc, CT_COMMENT) && next != nullptr &&
        ...
        if (!in_preproc) {
            while (!frm.empty() && frm.top().in_preproc) {
                 ...
                 */
                if (type == CT_PP_ENDREGION && frm.top().type == CT_PP_REGION_INDENT) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
        ...
        } else if (chunk_is_token(pc, CT_PREPROC)) // #
        {
            ...
            if (frm.top().type == CT_CASE && !options::indent_switch_pp()) {
                frm.push(pc, __func__, __LINE__);
            ...
            if (options::pp_if_indent_code() &&
                ...
                if (should_indent_preproc) {
                    ...
                    size_t extra =
                        (pc->pp_level == 0 && ifdef_over_whole_file()) ? 0 : indent_size;
            ...
            if (options::indent_member_single()) {
                if (get_chunk_parent_type(pc) == CT_PP_IF) {
                    ...
                    if (frm.top().type == CT_MEMBER && frm.top().pop_pc &&
                    ...
                    if (frmbkup.top().type == CT_MEMBER && frm.top().type == CT_MEMBER) {
            ...
            if (get_chunk_parent_type(pc) == CT_PP_DEFINE ||
                ...
                frm.top().indent_tmp =
                    options::pp_define_at_level() ? frm.prev().indent_tmp : 1;
        ...
        if (options::indent_xml_string() > 0) {
            if (chunk_is_token(pc, CT_STRING)) {
                if (pc->len() > 4 && xml_indent > 0 && pc->str[1] == '<' &&
                        pc->str[2] == '/') {
                    log_rule_B("indent_xml_string");
        ...
        do {
            ...
            if (frm.top().level >= pc->level) {
                ...
                if (chunk_is_token(pc, CT_BRACE_CLOSE) &&
                    ...
                    LOG_FMT(LINDLINE,
                            "%s(%d): prev_ncnl is '%s', prev_ncnl->orig_line is %zu, "
                            "prev_ncnl->orig_col is %zu\n",
                            __func__, __LINE__, prev_ncnl->text(), prev_ncnl->orig_line,
                            prev_ncnl->orig_col);
                ...
                if (chunk_is_semicolon(pc) &&
                        ((frm.top().type == CT_IMPORT) || (frm.top().type == CT_USING))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                if (!token_used && (frm.top().type == CT_MACRO_OPEN) &&
                ...
                if (((frm.top().type == CT_CLASS_COLON) ||
                        ...
                        (chunk_is_token(pc, CT_BRACE_OPEN) ||
                         chunk_is_token(pc, CT_OC_END) || chunk_is_token(pc, CT_OC_SCOPE) ||
                         chunk_is_token(pc, CT_OC_PROPERTY) || chunk_is_semicolon(pc))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                // End ObjC class colon stuff inside of generic definition (like
                // Test<T1: id<T3>>)
                if ((frm.top().type == CT_CLASS_COLON) &&
                ...
                // TODO: ideally formatting would know which opens occurred on a line
                // and group closes in the same manor
                if (language_is_set(LANG_OC) && chunk_is_token(pc, CT_SQUARE_CLOSE) &&
                        ...
                        while (count-- > 0) {
                            if (frm.top().type == CT_SQUARE_OPEN) {
                                if (frm.paren_count == 0) {
                                    fprintf(stderr,
                                            "%s(%d): frm.paren_count is ZERO, cannot be "
                                            "decremented, at line %zu, column %zu\n",
                            ...
                            LOG_FMT(LINDLINE,
                                    "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() "
                                    "is '%s', type is %s\n",
                                    __func__, __LINE__, pc->orig_line, pc->orig_col,
                ...
                if ((frm.top().type == CT_MEMBER) && frm.top().pop_pc == pc) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                if ((frm.top().type == CT_LAMBDA) &&
                        (chunk_is_token(pc, CT_SEMICOLON) || chunk_is_token(pc, CT_COMMA) ||
                         chunk_is_token(pc, CT_BRACE_OPEN))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                ...
                if (chunk_is_semicolon(pc) &&
                        ((frm.top().type == CT_RETURN) || (frm.top().type == CT_THROW))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                if ((frm.top().type == CT_TYPEDEF) &&
                        (chunk_is_semicolon(pc) || chunk_is_paren_open(pc) ||
                         chunk_is_token(pc, CT_BRACE_OPEN))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                ...
                if ((frm.top().type == CT_SQL_EXEC) && chunk_is_semicolon(pc)) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                if ((frm.top().type == CT_CLASS) &&
                        (chunk_is_token(pc, CT_CLASS_COLON) ||
                         chunk_is_token(pc, CT_BRACE_OPEN) || chunk_is_semicolon(pc))) {
                    LOG_FMT(LINDLINE,
                            "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is "
                            "'%s', type is %s\n",
                            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                ...
                if ((frm.top().type == (pc->type - 1)) &&
                    ...
                    if (frm.paren_count == 0) {
                        fprintf(stderr,
                                "%s(%d): frm.paren_count is ZERO, cannot be decremented, "
        ...
        if (chunk_is_token(pc, CT_NEWLINE) && options::indent_single_newlines()) {
        ...
        if (!chunk_is_newline(pc) && !chunk_is_comment(pc) && log_sev_on(LINDPC)) {
            LOG_FMT(LINDPC, "%s(%d):\n", __func__, __LINE__);
            LOG_FMT(LINDPC,
                    "   -=[ pc->orig_line is %zu, orig_col is %zu %s ]=-, frm.size() "
                    "is %zu\n",
                    pc->orig_line, pc->orig_col, pc->text(), frm.size());

            for (size_t ttidx = frm.size() - 1; ttidx > 0; ttidx--) {
                LOG_FMT(LINDPC,
                        "     [%zu %zu:%zu %s %s/%s tmp=%zu ind=%zu bri=%zu tab=%zu "
                        "cont=%d lvl=%zu blvl=%zu]\n",
                        ttidx, frm.at(ttidx).pc->orig_line, frm.at(ttidx).pc->orig_col,
                        frm.at(ttidx).pc->text(), get_token_name(frm.at(ttidx).type),
                        get_token_name(frm.at(ttidx).pc->parent_type),
                        frm.at(ttidx).indent_tmp, frm.at(ttidx).indent,
                        frm.at(ttidx).brace_indent, frm.at(ttidx).indent_tab,
                        frm.at(ttidx).indent_cont, frm.at(ttidx).level,
                        frm.at(ttidx).pc->brace_level);
            }
        }
        LOG_FMT(LINDENT2,
                "%s(%d): orig_line is %zu, orig_col is %zu, text() is '%s'\n",
                __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());

        // Issue #672
        if (chunk_is_token(pc, CT_BRACE_OPEN) && classFound) {
        ...
        if (chunk_is_token(pc, CT_BRACE_CLOSE)) {
            if (language_is_set(LANG_OC)) {
                if (frm.top().type == CT_BRACE_OPEN && frm.top().level >= pc->level) {
                    ...
                    // TODO: ideally formatting would know which opens occurred on a line
                    // and group closes in the same manor
                    if (count > 0 && chunk_is_token(pc, CT_BRACE_CLOSE) &&
                            get_chunk_parent_type(pc) == CT_OC_AT) {
                        if (frm.top().ip.ref) {
            ...
            if (!options::indent_macro_brace() && frm.prev().type == CT_PP_DEFINE &&
                ...
                if (are_chunks_in_same_line(frm.prev().pc,
                                            chunk_get_prev_ncnlnp(frm.top().pc))) {
                ...
                // Issue #1165
                LOG_FMT(LINDENT2,
                        "%s(%d): orig_line is %zu, pc->brace_level is %zu, for '%s', "
                        "pc->level is %zu, pc(-1)->level is %zu\n",
                        __func__, __LINE__, pc->orig_line, pc->brace_level, pc->text(),
                        pc->level, frm.prev().pc->level);
                frm.top().brace_indent = 1 + ((pc->brace_level + 1) * indent_size);
                indent_column_set(frm.top().brace_indent);
            ...
            // any '{' that is inside of a '(' overrides the '(' indent
                ...
                LOG_FMT(LINDENT2,
                        ...
                        __func__, __LINE__, pc->orig_line, pc->brace_level, pc->text(),
                        pc->level, frm.prev().pc->level);
                // FIXME: I don't know how much of this is necessary, but it seems to
                // work
                frm.top().brace_indent = 1 + (pc->brace_level * indent_size);
                indent_column_set(frm.top().brace_indent);
                frm.top().indent = indent_column + indent_size;
                ...
                if (frm.top().pc->parent_type == CT_OC_BLOCK_EXPR) {
                    ...
                    if (pc->flags.test(PCF_IN_OC_MSG) && options::indent_oc_block_msg()) {
                        frm.top().ip.ref =
                            oc_msg_block_indent(pc, false, false, false, true);
                        log_rule_B("indent_oc_block_msg");
                    ...
                    if (options::indent_oc_block() ||
                        ...
                        bool indent_from_keyword =
                            options::indent_oc_block_msg_from_keyword() && in_oc_msg;
                        log_rule_B("indent_oc_block_msg_from_colon");
                        bool indent_from_colon =
                            options::indent_oc_block_msg_from_colon() && in_oc_msg;
                        log_rule_B("indent_oc_block_msg_from_caret");
                        bool indent_from_caret =
                            options::indent_oc_block_msg_from_caret() && in_oc_msg;
                        log_rule_B("indent_oc_block_msg_from_brace");
                        bool indent_from_brace =
                            options::indent_oc_block_msg_from_brace() && in_oc_msg;
                         ...
                         *  - otherwise, indent from previous block (the "else" statement
                         * here)
                         */
                        log_rule_B("indent_oc_block_msg_xcode_style");
                        ...
                        chunk_t *ref =
                            oc_msg_block_indent(pc, indent_from_brace, indent_from_caret,
                                                indent_from_colon, indent_from_keyword);
                ...
                // Issue # 1620, UNI-24090.cs
                    ...
                    log_rule_B("indent_align_paren");
                    // We are inside ({ ... }) -- where { and ( are on the same line,
                    // avoiding double indentations.
                    frm.top().brace_indent = frm.prev().indent - indent_size;
                    indent_column_set(frm.top().brace_indent);
                    frm.top().indent = frm.prev().indent_tmp;
                    log_indent();
                } else if (are_chunks_in_same_line(
                               frm.prev().pc, chunk_get_prev_ncnlnp(frm.top().pc)) &&
                           !options::indent_align_paren() &&
                           chunk_is_paren_open(frm.prev().pc) &&
                           !pc->flags.test(PCF_ONE_LINER)) {
                    log_rule_B("indent_align_paren");
                    // We are inside ({ ... }) -- where { and ( are on adjacent lines,
                    // avoiding indentation of brace.
                    frm.top().brace_indent = frm.prev().indent - indent_size;
                    indent_column_set(frm.top().brace_indent);
                    frm.top().indent = frm.prev().indent_tmp;
                    log_indent();
                ...
                // C++11 initialization list (CT_BRACED_INIT_LIST), use indent from the
                // return.
                if (frm.prev().indent_cont) {
                    frm.top().indent = frm.prev().indent_tmp;
                } else {
                ...
                if (get_chunk_parent_type(pc) == CT_IF ||
                    ...
                    } else // indenting 'short' namespace
                    {
                        ...
                        if (options::indent_namespace_level() > 0) {
                            ...
                            frm.top().indent += options::indent_namespace_level();
                            log_indent();
                        }
                    }
            ...
            const size_t tmp = frm.top().indent + options::indent_switch_case();
            frm.push(pc, __func__, __LINE__);
            ...
            if (val > 0) {
                ...
                chunk_t *next =
                    chunk_get_next(chunk_get_next(pc)); // colon + possible statement

                if (next != nullptr &&
                        !chunk_is_newline(next)
                        // label (+ 2, because there is colon and space after it) must fit
                        // into indent
                        && (val + static_cast<int>(pc->len()) + 2 <=
                            static_cast<int>(pse_indent))) {
            ...
            if (options::indent_access_spec_body()) {
                ...
                indent_column_set(frm.top().indent_tmp);
                // Issue 1161
                // comments before 'access specifier' need to be aligned with the
                // 'access specifier'
                chunk_t *pct = pc;
            ...
            if (options::indent_class_colon() && chunk_is_token(pc, CT_CLASS_COLON)) {
                ...
                if (options::indent_class_on_colon()) {
                    ...
                    /*
                     * If the std::max() calls were specialized with size_t (the type of
                     * the underlying variable), they would never actually do their job,
                     * because size_t is unsigned and therefore even a "negative" result
                     * would be always greater than zero. Using ptrdiff_t (a standard
                     * signed type of the same size as size_t) in order to avoid that.
                     */
                    frm.top().indent = std::max<ptrdiff_t>(
                                           frm.top().indent + options::indent_ctor_init(), 0);
                    log_indent();
                    frm.top().indent_tmp = std::max<ptrdiff_t>(
                                               frm.top().indent_tmp + options::indent_ctor_init(), 0);
                    frm.top().indent_tab = std::max<ptrdiff_t>(
                                               frm.top().indent_tab + options::indent_ctor_init(), 0);
                    log_indent_tmp();
                    indent_column_set(frm.top().indent_tmp);
                } else {
                    chunk_t *next = chunk_get_next(pc);
            ...
            if (chunk_is_newline(chunk_get_prev(pc)) && pc->column != indent_column) {
            ...
            if (chunk_is_newline(chunk_get_prev(pc)) && pc->column != indent_column &&
                ...
                LOG_FMT(LINDENT, "%s[line %d]: %zu] indent => %zu [%s]\n", __func__,
                        __LINE__, pc->orig_line, indent_column, pc->text());
                reindent_line(pc, indent_column);
            }
            ...
            if ((chunk_is_token(pc, CT_FPAREN_OPEN) ||
                    ...
                    ((options::indent_func_call_param() &&
                     ...
                     (!options::indent_func_def_param() // Issue #931
                      ...
                      pc->orig_col >
                      options::indent_func_def_param_paren_pos_threshold()))) {
                ...
                while (((idx > 0 && frm.at(idx).type != CT_BRACE_OPEN &&
                         ...
                         frm.at(idx).type != CT_CASE && frm.at(idx).type != CT_MEMBER &&
                ...
                if (options::indent_func_param_double()) {
                    // double is: Use both values of the options indent_columns and
                    // indent_param
                    frm.top().indent += indent_size;
                    log_indent();
                }
                ...
                frm.top().indent_tab = frm.top().indent;
                skipped = true;
            } else if ((chunk_is_str(pc, "(", 1) && !options::indent_paren_nl()) ||
                       (chunk_is_str(pc, "<", 1) &&
                        !options::indent_paren_nl()) // TODO: add indent_angle_nl?
                       ||
                       (chunk_is_str(pc, "[", 1) && !options::indent_square_nl())) {
                ...
                if (chunk_is_newline(next) && !options::indent_paren_after_func_def() &&
                    ...
                    if (!options::indent_align_paren()) {
                        ...
                        while (sub > 0 &&
                            ...
                            if (sub == 0) {
                                fprintf(stderr,
                                        "%s(%d): sub is ZERO, cannot be decremented, at line "
                                        "%zu, column %zu\n",
                                        __func__, __LINE__, pc->orig_line, pc->orig_col);
                                log_flush(true);
                                exit(EX_SOFTWARE);
            ...
            if (get_chunk_parent_type(pc) != CT_OC_AT &&
                    options::indent_continue() != 0 && !skipped) {
                frm.top().indent = frm.prev().indent;
                log_indent();
                         ...
                         chunk_is_token(pc, CT_ANGLE_OPEN))) // Issue #1170
                {
                    ...
                    if ((options::use_indent_continue_only_once()) &&
                            (frm.top().indent_cont) && vardefcol != 0) {
                        /*
            ...
            if (tmp != nullptr) {
                if (chunk_is_token(tmp, CT_FUNC_CALL)) {
                    tmp = chunk_get_next_ncnlnp(
                              chunk_get_next_type(tmp, CT_FPAREN_CLOSE, tmp->level));
            ...
            if (tmp != nullptr &&
                    ((strcmp(tmp->text(), ".") != 0) || tmp->type != CT_MEMBER)) {
                if (chunk_is_paren_close(tmp)) {
        ...
        } else if (chunk_is_token(pc, CT_ASSIGN) || chunk_is_token(pc, CT_IMPORT) ||
            ...
            if (next != nullptr) {
                /*
                 * fixes  1260 , 1268 , 1277 (Extra indentation after line with multiple
                 * assignments) For multiple consecutive assignments in single line ,
                 * the indent of all these assignments should be same and one more than
                 * this line's indent. so poping the previous assign and pushing the new
                 * one
                 */
                ...
                if (options::indent_continue() != 0) {
                    ...
                    if (pc->level == pc->brace_level &&
                        ...
                        if ((options::use_indent_continue_only_once()) &&
                            ...
                            vardefcol =
                                frm.top().indent; // use the same variable for the next line
                            frm.top().indent_cont = true;
                        }
                    }
                } else if (chunk_is_newline(next) || !options::indent_align_assign()) {
            ...
            if (pc->level == pc->brace_level) {
                ...
                if (!options::indent_off_after_return_new() || next == nullptr ||
                        next->type != CT_NEW) {
                    frm.push(pc, __func__, __LINE__);
        ...
        if (options::indent_shift() && !pc->flags.test(PCF_IN_ENUM) &&
                get_chunk_parent_type(pc) != CT_OPERATOR && pc->type != CT_COMMENT &&
                pc->type != CT_COMMENT_CPP && pc->type != CT_COMMENT_MULTI &&
                pc->type != CT_BRACE_OPEN && pc->level > 0 && !chunk_is_blank(pc)) {
            bool in_shift = false;
            ...
            do {
                ...
                tmp = chunk_get_prev_ncnl(tmp);
            } while (!in_shift && tmp != nullptr && tmp->type != CT_SEMICOLON &&
            ...
            if ((chunk_is_semicolon(prev_nonl) ||
                    ...
                    (prev_nonl && prev_nonl->flags.test(PCF_IN_PREPROC)) !=
                    pc->flags.test(PCF_IN_PREPROC) ||
                    chunk_is_token(prev_nonl, CT_COMMA) || is_operator)) {
                in_shift = false;
            }
            LOG_FMT(LINDENT2, "%s(%d): in_shift is %s\n", __func__, __LINE__,
                    in_shift ? "TRUE" : "FALSE");
            ...
            if (chunk_is_token(prev2, CT_NEWLINE) && in_shift) {
                ...
                // Setting frm.top().indent_cont = true in the top context when the
                // indent is not also set just leads to compications when succeeding
                // statements try to indent based on being embedded in a continuation.
                // In other words setting frm.top().indent_cont = true should only be
                // set if frm.top().indent is also set.
        ...
        if (vardefcol == 0 &&
                ...
                !pc->flags.test(PCF_IN_FCN_DEF) && pc->flags.test(PCF_VAR_1ST_DEF)) {
            log_rule_B("indent_continue");
        ...
        if (chunk_is_semicolon(pc) || (chunk_is_token(pc, CT_BRACE_OPEN) &&
        ...
        if (did_newline && !chunk_is_newline(pc) && (pc->len() != 0)) {
            ...
            LOG_FMT(LINDENT2,
                    "%s(%d): orig_line is %zu, pc->column_indent is %zu, "
                    "indent_column is %zu, for '%s'\n",
                    __func__, __LINE__, pc->orig_line, pc->column_indent,
                    indent_column, pc->text());
            ...
            if (vardefcol > 0 && pc->level == pc->brace_level &&
                    (chunk_is_token(prev, CT_COMMA) || chunk_is_token(prev, CT_TYPE) ||
            ...
            if (pc->flags.test(PCF_DONT_INDENT)) {
                ...
                LOG_FMT(LINDENT, "Indent SQL: [%s] to %zu (%zu/%zu)\n", pc->text(),
                        pc->column, sql_col, sql_orig_col);
            } else if (!options::indent_member_single() &&
                ...
                LOG_FMT(LINDENT, "%s(%d): orig_line is %zu, comment => %zu\n", __func__,
                        __LINE__, pc->orig_line, frm.top().indent_tmp);
                ...
                if (frm.poped().type == c_token_t(pc->type - 1)) {
                    ...
                    LOG_FMT(LINDLINE,
                            "%s(%d): ck1->orig_line is %zu, ck1->orig_col is %zu, "
                            "ck1->text() is '%s', ck1->type is %s\n",
                            __func__, __LINE__, ck1->orig_line, ck1->orig_col,
                            ck1->text(), get_token_name(ck1->type));
                    chunk_t *ck2 = chunk_get_prev(ck1);
                    LOG_FMT(LINDLINE,
                            "%s(%d): ck2->orig_line is %zu, ck2->orig_col is %zu, "
                            "ck2->text() is '%s', ck2->type is %s\n",
                            __func__, __LINE__, ck2->orig_line, ck2->orig_col,
                            ck2->text(), get_token_name(ck2->type));
                    ...
                    if (chunk_is_newline(ck2) || (options::indent_paren_close() == 1)) {
                        ...
                        if (options::indent_paren_close() != 2) {
                            ...
                            LOG_FMT(LINDLINE,
                                    "%s(%d): [%zu:%zu] indent_paren_close is 0 or 1\n",
                                    __func__, __LINE__, ck2->orig_line, ck2->orig_col);
                            ...
                            if (options::indent_paren_close() == 1) {
                                ...
                                if (indent_column == 0) {
                                    fprintf(stderr,
                                            "%s(%d): indent_column is ZERO, cannot be "
                                            "decremented, at line %zu, column %zu\n",
                                            __func__, __LINE__, pc->orig_line, pc->orig_col);
                                    log_flush(true);
                                    exit(EX_SOFTWARE);
                                ...
                                indent_column--;
                                LOG_FMT(LINDLINE,
                                        "%s(%d): [%zu:%zu] indent_column set to %zu\n",
                                        __func__, __LINE__, ck2->orig_line, ck2->orig_col,
                                        indent_column);
                            }
                        } else {
                            ...
                            if (chunk_get_prev(pc)->type == CT_NEWLINE) {
                                LOG_FMT(LINDLINE,
                                        "%s(%d): orig_line is %zu, orig_col is %zu, text() is "
                                        "'%s', type is %s\n",
                                        __func__, __LINE__, pc->orig_line, pc->orig_col,
                                        pc->text(), get_token_name(pc->type));
                ...
                LOG_FMT(LINDENT,
                        "%s(%d): orig_line is %zu, closing parenthesis => %zu, text is "
                        "'%s'\n",
                        __func__, __LINE__, pc->orig_line, indent_column, pc->text());
                reindent_line(pc, indent_column);
                ...
                LOG_FMT(LINDENT, "%s(%d): %zu] comma => %zu [%s]\n", __func__, __LINE__,
                        pc->orig_line, indent_column, pc->text());
                reindent_line(pc, indent_column);
            } else if (options::indent_func_const() &&
                       chunk_is_token(pc, CT_QUALIFIER) &&
                       strncasecmp(pc->text(), "const", pc->len()) == 0 &&
                       (next == nullptr || chunk_is_token(next, CT_BRACED) ||
                ...
                LOG_FMT(LINDENT, "%s(%d): %zu] const => %zu [%s]\n", __func__, __LINE__,
                        pc->orig_line, indent_column, pc->text());
                reindent_line(pc, indent_column);
            } else if (options::indent_func_throw() && chunk_is_token(pc, CT_THROW) &&
                       get_chunk_parent_type(pc) != CT_NONE) {
                ...
                if (options::indent_bool_paren() && chunk_is_paren_open(frm.top().pc)) {
                    indent_column_set(frm.top().pc->column);
                ...
                reindent_line(pc, indent_column);
                       ...
                       (chunk_is_token(pc, CT_ADDR) || chunk_is_token(pc, CT_WORD) ||
                ...
                if (tmp != nullptr) {
                    ...
                    if (tmp != nullptr) {
                        LOG_FMT(LINDENT, "%s: %zu] ternarydefcol => %zu [%s]\n", __func__,
                                pc->orig_line, tmp->column, pc->text());
                ...
                LOG_FMT(LINDENT,
                        "%s(%d): pc->line is %zu, pc->column is %zu, pc->text() is "
                        "'%s, indent_column is %zu\n",
                        __func__, __LINE__, pc->orig_line, pc->column, pc->text(),
                        indent_column);
                ...
                if (pc->column != indent_column) {
                    if (use_indent &&
                            pc->type !=
                            CT_PP_IGNORE) // Leave indentation alone for PP_IGNORE tokens
                    {
                        ...
                        if ((chunk_is_token(pc, CT_QUESTION)       // Issue #2101
                            ...
                            if (options::indent_switch_break_with_case()) {
                                LOG_FMT(LINDENT,
                                        "%s(%d): orig_line is %zu, "
                                        "indent_switch_break_with_case, for '%s'\n",
                                ...
                                LOG_FMT(
                                    LINDENT,
                                    "%s(%d): orig_line is %zu, indent set to %zu, for '%s'\n",
        ...
        if (chunk_is_newline(pc) || chunk_is_token(pc, CT_COMMENT_MULTI) ||
        ...
        if (options::indent_xml_string() > 0 && chunk_is_token(pc, CT_STRING) &&
                pc->len() > 4 && pc->str[1] == '<' && pc->str[2] != '/' &&
                pc->str[pc->len() - 3] != '/') {
            if (xml_indent <= 0) {
        ...
        if (chunk_is_token(pc, CT_CLASS) && language_is_set(LANG_CPP | LANG_JAVA) &&
                options::indent_continue_class_head() != 0 && !classFound) {
            LOG_FMT(LINDENT,
                    "%s(%d): orig_line is %zu, CT_CLASS found and UO_indent_continue "
                    "!= 0, OPEN IT\n",
                    __func__, __LINE__, pc->orig_line);
            frm.push(pc, __func__, __LINE__);
...
null_pc:
    ...
    for (size_t idx_temp = 1; idx_temp < frm.size(); idx_temp++) {
        ...
        LOG_FMT(LWARN,
                "%s(%d): File: %s, open_line is %zu, parent is %s: Unmatched %s\n",
                __func__, __LINE__, cpd.filename.c_str(),
                frm.at(idx_temp).open_line, get_token_name(frm.at(idx_temp).parent),
...
static bool is_end_of_assignment(chunk_t *pc, const ParseFrame &frm) {
    return ((frm.top().type == CT_ASSIGN_NL || frm.top().type == CT_MEMBER ||
...
static size_t calc_comment_next_col_diff(chunk_t *pc) {
    ...
    do {
        ...
        LOG_FMT(LCMTIND,
                "%s(%d): newline_token->text() is '%s', orig_line is %zu, orig_col "
                "is %zu\n",
                __func__, __LINE__, newline_token->text(), newline_token->orig_line,
                newline_token->orig_col);
        ...
        if (next != nullptr) {
            LOG_FMT(
                LCMTIND,
                "%s(%d): next->text() is '%s', orig_line is %zu, orig_col is %zu\n",
    ...
    return (next->orig_col > pc->orig_col ? next->orig_col - pc->orig_col
...
static void indent_comment(chunk_t *pc, size_t col) {
    ...
    LOG_FMT(
        LCMTIND,
        "%s(%d): pc->text() is '%s', orig_line %zu, orig_col %zu, level %zu\n",
    ...
    if (pc->orig_col == 1 && !options::indent_col1_comment() &&
    ...
    if (nl != nullptr) {
        LOG_FMT(
            LCMTIND,
            "%s(%d): nl->text() is '%s', orig_line %zu, orig_col %zu, level %zu\n",
    ...
    if (pc->orig_col > 1) {
        ...
        if (prev != nullptr) {
            LOG_FMT(LCMTIND,
                    "%s(%d): prev->text() is '%s', orig_line %zu, orig_col %zu, "
                    "level %zu\n",
        ...
        if (chunk_is_comment(prev) && nl->nl_count == 1) {
            ...
            if (prev_col_diff <= indent_comment_align_thresh) {
                LOG_FMT(LCMTIND,
                        "%s(%d): prev->text() is '%s', Doxygen_comment(prev) is %s\n",
                        __func__, __LINE__, prev->text(),
                        chunk_is_Doxygen_comment(prev) ? "TRUE" : "FALSE");
                LOG_FMT(LCMTIND,
                        "%s(%d): pc->text() is '%s', Doxygen_comment(pc) is %s\n",
                        __func__, __LINE__, pc->text(),
                        chunk_is_Doxygen_comment(pc) ? "TRUE" : "FALSE");
                ...
                if (chunk_is_Doxygen_comment(prev) == chunk_is_Doxygen_comment(pc)) {
                            ...
                            next_col_diff == 5000) // FIXME: Max thresh magic number 5000
                    {
                        LOG_FMT(
                            LCMTIND,
                            "%s(%d): rule 3 - prev comment, coldiff = %zu, now in %zu\n",
    ...
    if ((options::indent_sing_line_comments() > 0) &&
        ...
        LOG_FMT(LCMTIND,
                "%s(%d): rule 4 - single line comment indent, now in %zu\n",
...
void indent_preproc(void) {
    ...
    for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
        ...
        const size_t pp_level =
            (pc->pp_level > pp_level_sub) ? pc->pp_level - pp_level_sub : 0;
        ...
        if ((!options::pp_indent_at_level() ||
                (pc->brace_level <=
                 ((get_chunk_parent_type(pc) == CT_PP_DEFINE) ? 1 : 0))) &&
                get_chunk_parent_type(pc) != CT_PP_REGION &&
                get_chunk_parent_type(pc) != CT_PP_ENDREGION) {
            log_rule_B("pp_define_at_level");
