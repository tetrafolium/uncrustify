...
static constexpr const int_fast8_t UTF8_BLOCKS =
    6; // 6 -> max utf8 blocks per char

static size_t fix_len_idx(size_t size, size_t idx, size_t len);

//! converts \n and \r chars are into NL and CR UTF8 symbols before encode_utf8
//! is called
...
static int getLogTextUtf8Len(unc_text::value_type &c0, size_t start,
                             size_t end);
...
unc_text::unc_text() { m_logtext = log_type{'\0'}; }

unc_text::unc_text(const unc_text &ref) { set(ref); }

unc_text::unc_text(const unc_text &ref, size_t idx, size_t len) {
  set(ref, idx, len);
}

unc_text::unc_text(const char *ascii_text) { set(ascii_text); }

unc_text::unc_text(const std::string &ascii_text) { set(ascii_text); }

unc_text::unc_text(const value_type &data, size_t idx, size_t len) {
  set(data, idx, len);
}

size_t unc_text::size() const { return (m_chars.size()); }
...
const unc_text::value_type &unc_text::get() const { return (m_chars); }

int unc_text::operator[](size_t idx) const {
  return ((idx < m_chars.size()) ? m_chars[idx] : 0);
}

const int &unc_text::at(size_t idx) const { return (m_chars.at(idx)); }

const int &unc_text::back() const { return (m_chars.back()); }

void unc_text::push_back(int ch) { append(ch); }
...
void unc_text::set(const unc_text &ref, size_t idx, size_t len) {
  ...
  for (size_t di = 0; len > 0; di++, idx++, len--) {
...
void unc_text::insert(size_t idx, const unc_text &ref) {
  ...
  m_chars.insert(std::next(std::begin(m_chars), idx), std::begin(ref.m_chars),
                 std::end(ref.m_chars));
}
...
void unc_text::append(int ch) {
  ...
  if (ch < 0x80 && ch != '\n' && ch != '\r') {
    ...
    m_logtext.insert(std::end(m_logtext), std::begin(utf8converted),
                     std::end(utf8converted));
  }
  m_logtext.push_back('\0');
...
void unc_text::append(const unc_text &ref) {
  ...
  m_logtext.pop_back();
  m_logtext.insert(std::end(m_logtext), std::begin(ref.m_logtext),
                   std::end(ref.m_logtext));
...
int unc_text::find(const char *search_txt, size_t start_idx) const {
  const size_t t_len =
      strlen(search_txt);      // the length of 'text' we are looking for
  ...
  if (s_len < t_len // search_txt longer than the string we are looking in
      || start_idx + t_len - 1 >=
             s_len) // starting position to high to find search_txt
  {
    return (-1);
  }
