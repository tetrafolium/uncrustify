...
struct cmt_reflow {
    ...
    unc_text
    cont_text; //! fixed text to output at the start of a line (0 to 3 chars)
 ...
 * combined.
 */
static void add_comment_text(const unc_text &text, cmt_reflow &cmt,
                             bool esc_close);
...
static void cmt_output_indent(size_t brace_col, size_t base_col,
                              size_t column) {
    log_rule_B("indent_cmt_with_tabs");
    log_rule_B("indent_with_tabs");
    size_t iwt = options::indent_cmt_with_tabs()
                 ? 2
...
void output_parsed(FILE *pfile) {
    ...
    // output.cpp, line 427: fprintf(pfile, "# Line              Tag Parent... and
    // 430: ... fprintf(pfile, "%s# %3zu>%19.19s[%19.19s] ... here xx xx   xx xx
#ifdef WIN32
    fprintf(pfile, "# Line                Tag              Parent          "
            "Columns Br/Lvl/pp     Nl  Text");
#else  // not WIN32
    fprintf(pfile, "# Line                Tag              Parent          "
            "Columns Br/Lvl/pp     Flag   Nl  Text");
#endif // ifdef WIN32
    ...
    for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
        ...
        fprintf(pfile, "%s# %3d>%19.19s[%19.19s][%3d/%3d/%3d/%3d][%d/%d/%d][%d-%d]",
                ...
                get_token_name(get_chunk_parent_type(pc)), (int)pc->column,
                (int)pc->orig_col, (int)pc->orig_col_end, (int)pc->orig_prev_sp,
        ...
        fprintf(pfile, "%s# %3zu>%19.19s[%19.19s][%3zu/%3zu/%3zu/%3d][%zu/%zu/%zu]",
                ...
                get_token_name(get_chunk_parent_type(pc)), pc->column, pc->orig_col,
                pc->orig_col_end, pc->orig_prev_sp, pc->brace_level, pc->level,
                pc->pp_level);
        fprintf(pfile, "[%11llx]", static_cast<pcf_flags_t::int_t>(pc->flags));
        fprintf(pfile, "[%zu-%d]", pc->nl_count, pc->after_tab);
#endif // ifdef WIN32
...
void output_text(FILE *pfile) {
    ...
    for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
        LOG_FMT(LCONTTEXT,
                "%s(%d): text() is '%s', type is %s, orig_col is %zu, column is "
                "%zu, nl is %zu\n",
                __func__, __LINE__, pc->text(), get_token_name(pc->type),
                pc->orig_col, pc->column, pc->nl_count);
        log_rule_B("cmt_convert_tab_to_spaces");
        cpd.output_tab_as_space =
            (options::cmt_convert_tab_to_spaces() && chunk_is_comment(pc));
        ...
        if (chunk_is_token(pc, CT_NEWLINE)) {
            ...
            if (!pc->flags.test(PCF_WAS_ALIGNED)) {
                ...
                if (options::sp_before_nl_cont() & IARF_REMOVE) {
                    log_rule_B("sp_before_nl_cont");
                    pc->column =
                        cpd.column + (options::sp_before_nl_cont() == IARF_FORCE);
                } else {
                        ...
                        if (prev != NULL && prev->nl_count == 0) {
                            ...
                            if ((int)(cpd.column + orig_sp) < 0) {
                                ...
                                fprintf(stderr,
                                        ...
                                        "prev->orig_col_end is %d\n",
                                        (int)pc->orig_col, (int)prev->orig_col_end);
#else  // not WIN32
                                fprintf(stderr,
                                        "FATAL: negative value.\n   pc->orig_col is %zu, "
                                        "prev->orig_col_end is %zu\n",
                            ...
                            // Add or remove space before a backslash-newline at the end of a
                            // line.
                            log_rule_B("sp_before_nl_cont");
            ...
            LOG_FMT(LOUTIND,
                    "%s(%d): orig_line is %zu, orig_col is %zu,\npc->text() >%s<, "
                    "pc->str.size() is %zu\n",
            ...
            LOG_FMT(LOUTIND,
                    "%s(%d): orig_line is %zu, column is %zu, non-visible stuff: "
                    "type is %s\n",
                    __func__, __LINE__, pc->orig_line, pc->column,
            ...
            if (cpd.did_newline) {
                ...
                allow_tabs = (options::indent_with_tabs() == 2) ||
                             (chunk_is_comment(pc) && options::indent_with_tabs() != 0);

                LOG_FMT(LOUTIND,
                        "%s(%d): orig_line is %zu, column is %zu, column_indent is "
                        "%zu, cpd.column is %zu\n",
                        __func__, __LINE__, pc->orig_line, pc->column,
                        pc->column_indent, cpd.column);
            } else {
                /*
                ...
                allow_tabs =
                    (options::align_with_tabs() && pc->flags.test(PCF_WAS_ALIGNED) &&
                ...
                LOG_FMT(LOUTIND, "%s(%d): at column %zu(%s)\n", __func__, __LINE__,
                        pc->column, (allow_tabs ? "true" : "FALSE"));
            }
...
static chunk_t *get_next_function(chunk_t *pc) {
    while ((pc = chunk_get_next(pc)) != nullptr) {
        if (chunk_is_token(pc, CT_FUNC_DEF) || chunk_is_token(pc, CT_FUNC_PROTO) ||
...
static void add_comment_text(const unc_text &text, cmt_reflow &cmt,
    ...
    for (; idx < len; idx++) // TODO: avoid modifying idx in loop
    {
        ...
        if (text[idx] == '\n') {
            ...
            ch_cnt = 0;
        } else if (cmt.reflow && text[idx] == ' ' && options::cmt_width() > 0 &&
            ...
            add_text(cmt.cont_text);
            // The number of spaces to insert after the star on subsequent comment
            // lines.
            log_rule_B("cmt_sp_after_star_cont");
            output_to_column(cmt.column + options::cmt_sp_after_star_cont(), false);
            ch_cnt = 0;
        } else {
            // Escape a C closure in a CPP comment
            if (esc_close &&
                    ((was_star && text[idx] == '/') || (was_slash && text[idx] == '*'))) {
                add_char(' ');
            }
...
static void output_cmt_start(cmt_reflow &cmt, chunk_t *pc) {
    ...
    // LOG_FMT(LSYS, "%s: line %zd, brace=%zd base=%zd col=%zd orig=%zd
    // aligned=%x\n",
    //        __func__, pc->orig_line, cmt.brace_col, cmt.base_col, cmt.column,
    //        pc->orig_col, pc->flags & (PCF_WAS_ALIGNED | PCF_RIGHT_COMMENT));

    if (get_chunk_parent_type(pc) == CT_COMMENT_START ||
            get_chunk_parent_type(pc) == CT_COMMENT_WHOLE) {
        log_rule_B("indent_col1_comment");

        if (!options::indent_col1_comment() && pc->orig_col == 1 &&
...
static chunk_t *output_comment_c(chunk_t *first) {
    ...
    while (can_combine_comment(pc, cmt)) {
        ...
        if (cpd.last_char == '*' && (tmp[0] == '/' || tmp[0] != ' ')) // Issue #1908
        {
            LOG_FMT(LCONTTEXT,
                    "%s(%d): add_text a "
                    "\n",
        ...
        // In case of reflow, original comment could contain trailing spaces before
        // closing the comment, we don't need them after reflow
...
static chunk_t *output_comment_cpp(chunk_t *first) {
    ...
    if (options::sp_cmt_cpp_doxygen()) // special treatment for doxygen style
        // comments (treat as unity)
    {
        ...
        {
            leadin +=
                sComment[2]; // at least one additional char (either "///" or "//!")
...
static void output_comment_multi(chunk_t *pc) {
    ...
    // cont='%s'\n",
    //        pc->orig_line, cmt_col, pc->orig_col, col_diff, cmt.xtra_indent,
    //        cmt.cont_text.c_str());
    ...
    while (cmt_idx < pc->len()) {
        ...
        if (options::cmt_reflow_mode() == 2 && ch == '\n' && cmt_idx < pc->len()) {
            ...
            while (nwidx > 0) {
                ...
                if (prev_nonempty_line < 0 && !unc_isspace(line[nwidx]) &&
                        ...
                        && (pc->flags.test(PCF_IN_PREPROC)
                            ...
                            : true)) {
                    prev_nonempty_line =
                        nwidx; // last non-whitespace char in the previous line
                }
            }
            ...
            for (size_t nxt_len = 0;
                    (nxt_len <= remaining &&
                     ...
                     && pc->str[nxt_len] != '\n');
                    nxt_len++) {
                if (next_nonempty_line < 0 && !unc_isspace(pc->str[nxt_len]) &&
                        pc->str[nxt_len] != '*' &&
                        (nxt_len == remaining ||
                         (pc->flags.test(PCF_IN_PREPROC)
                          ? (pc->str[nxt_len] != '\\' ||
                          ...
                          : true))) {
                    next_nonempty_line =
                        nxt_len; // first non-whitespace char in the next line
                }
            }
             ...
             * +-\/|.,*)
             *
             * - Doxygen/JavaDoc/etc. parameters: these often start with \ or @, at
             * least something clearly non-alphanumeric (you see where we're going
             * with this?)
             *
             * - bullet lists that are closely spaced: bullets are always
             * non-alphanumeric characters, such as '-' or '+' (or, oh horror, '*' -
             * that's bloody ambiguous to parse :-( ... with or without '*' comment
             * start prefix, that's the question, then.)
             *
             * - semi-HTML formatted code, e.g. <pre>...</pre> comment sections (NDoc,
             * etc.)
             *
             * - New lines which form a new paragraph without there having been added
             * an extra empty line between the last sentence and the new one. A bit
             * like this, really; so it is opportune to check if the last line ended
             *   in a terminal (that would be the set '.:;!?') and the new line starts
             * with a capital. Though new lines starting with comment delimiters, such
             * as '(', should be pulled up.
             *
             * So it bores down to this: the only folding (& reflowing) that's going
             * to happen is when the next line starts with an alphanumeric character
             * AND the last line didn't end with an non-alphanumeric character,
             * except: ',' AND the next line didn't start with a '*' all of a sudden
             * while the previous one didn't (the ambiguous '*'-for-bullet case!)
             */
            if (prev_nonempty_line >= 0 && next_nonempty_line >= 0 &&
                    (((unc_isalnum(line[prev_nonempty_line]) ||
                     ...
                     ('.' == line[prev_nonempty_line] // dot followed by non-capital is
                      // NOT a new sentence start
            ...
            if (line_count == 1) {
                ...
                if (line.size() == 0) {
                    ...
                    if (options::cmt_star_cont()) {
                        ...
                        // Multiline comments with can have empty lines with some spaces in
                        // them for alignment While adding * symbol and alligning them we
                        // don't want to keep these trailing spaces
                    ...
                    if (options::cmt_indent_multi() && line[0] != '*' && line[0] != '|' &&
                            line[0] != '#' && (line[0] != '\\' || unc_isalpha(line[1])) &&
                            line[0] != '+') {
                        // The number of spaces to insert at the start of subsequent comment
                        // lines.
                        log_rule_B("cmt_sp_before_star_cont");
                        ...
                        // Checks for and updates the lead chars.
                        // @return 0=not present, >0=number of chars that are part of the
                        // lead
...
static bool kw_fcn_javaparam(chunk_t *cmt, unc_text &out_txt) {
    ...
    if (tmp != nullptr && get_chunk_parent_type(tmp) == CT_OC_MSG_DECL &&
...
static const kw_subst_t kw_subst_table[] = {
    {"$(filename)", kw_fcn_filename},   {"$(class)", kw_fcn_class},
    {"$(message)", kw_fcn_message},     {"$(category)", kw_fcn_category},
    {"$(scope)", kw_fcn_scope},         {"$(function)", kw_fcn_function},
    {"$(javaparam)", kw_fcn_javaparam}, {"$(fclass)", kw_fcn_fclass},
};
    ...
    while (cmt_idx < pc->len()) {
        ...
        if (line.size() == 0 &&
            ...
            if (ch == ' ') {
                ...
                log_rule_B("input_tab_size");
                line_column =
                    calc_next_tab_column(line_column, options::input_tab_size());
                continue;
            }
        }
            ...
            if (chunk_is_newline(tmp)) {
                nl_count += tmp->nl_count;
                       ...
                       (chunk_is_token(tmp, CT_PP_ENDIF) ||
                        ((chunk_is_token(br_open, CT_PP_IF))
                         ? (chunk_is_token(tmp, CT_PP_ELSE))
                         : 0))) {
                br_close = tmp;

                LOG_FMT(LPPIF,
                        "found #if / %s section on lines %zu and %zu, nl_count=%zu\n",
                        ...
                        chunk_is_token(tmp, CT_NEWLINE)) // chunk_is_newline(tmp))
                {
                    ...
                    const char *txt =
                        ...
                        : ((chunk_is_token(tmp, CT_PP_ENDIF)) ? "#endif" : "#else");
                    LOG_FMT(LPPIF,
                            "#if / %s section candidate for augmenting when over NL "
                            "threshold %zu != 0 (nl_count=%zu)\n",
                            txt, nl_min, nl_count);

                    if (nl_min > 0 &&
                            nl_count > nl_min) // nl_count is 1 too large at all times as #if
                        // line was counted too
                    {
                        // determine the added comment style
                        c_token_t style =
                            (language_is_set(LANG_CPP)) ? CT_COMMENT_CPP : CT_COMMENT;

                        unc_text str;
                        generate_if_conditional_as_text(str, br_open);

                        LOG_FMT(LPPIF,
                                "#if / %s section over threshold %zu (nl_count=%zu) --> "
                                "insert comment after the %s: %s\n",
                ...
                // checks both the #else and #endif for a given level, only then look
                // further in the main loop
                if (chunk_is_token(br_close, CT_PP_ENDIF)) {
                    break;
                }
            }
