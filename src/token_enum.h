...
enum c_token_t {
  ...
  CT_SPAREN_OPEN, // 'special' paren after
                  // if/for/switch/while/synchronized/catch
  CT_SPAREN_CLOSE,
  ...
  CT_PROTO_WRAP, // macro: "RETVAL PROTO_WRAP( fcn_name, (PARAMS))". Parens for
                 // PARAMS are optional.
  ...
  CT_OC_CLASS_EXT,    // ObjC: a pair of empty parens after the class name in a
                      // @interface or @implementation
  CT_OC_CATEGORY, // ObjC: the category name in parens after the class name in a
                  // @interface or @implementation
  CT_OC_SCOPE,    // ObjC: the '-' or '+' in '-(void) func: (int) i;'
  CT_OC_MSG, // ObjC: parent type to '[', ']' and ';' in '[class func : param
             // name: param];'
  CT_OC_MSG_CLASS, // ObjC: 'class' in  '[class func : param name: param];' (see
                   // also PCF_IN_OC_MSG)
  CT_OC_MSG_FUNC,  // ObjC: 'func' in  '[class func : param name: param];' (see
                   // also PCF_IN_OC_MSG)
  CT_OC_MSG_NAME,  // ObjC: 'name' in  '[class func : param name: param];' (see
                   // also PCF_IN_OC_MSG)
  ...
  CT_OC_BLOCK_TYPE, // ObjC: block declaration parent type, e.g. mainly the
                    // '(^block_t)' in 'void (^block_t)(int arg);'
  CT_OC_BLOCK_EXPR, // ObjC: block expression with arg: '^(int arg) { arg++; };'
                    // and without (called a block literal): '^{ ... };'
  CT_OC_BLOCK_CARET,   // ObjC: block pointer caret: '^'
  CT_OC_AT,            // ObjC: boxed constants using '@'
  CT_OC_PROPERTY_ATTR, // ObjC: property attribute (strong, weak, readonly,
                       // etc...)
  ...
  CT_PP_IGNORE, // Dummy token for ignoring a certain preprocessor directive (do
  ...
  CT_TOKEN_COUNT_ // NOTE: Keep this the last entry because it's used as a
                  // counter.
};
