...
static void do_source_file(const char *filename_in, const char *filename_out,
                           const char *parsed_file, bool no_backup,
                           bool keep_mtime);
...
void usage(const char *argv0) {
    fprintf(
        ...
        "The '--prefix' and '--suffix' options may not be used with "
        "'--replace'.\n"
        "\n"
        "Basic Options:\n"
        " -c CFG       : Use the config file CFG, or defaults if CFG is set to "
        "'-'.\n"
        " -f FILE      : Process the single file FILE (output to stdout, use "
        "with -o).\n"
        " -o FILE      : Redirect stdout to FILE.\n"
        " -F FILE      : Read files to process from FILE, one filename per line "
        "(- is stdin).\n"
        " --check      : Do not output the new text, instead verify that nothing "
        "changes when\n"
        "                the file(s) are processed.\n"
        "                The status of every file is printed to stderr.\n"
        "                The exit code is EXIT_SUCCESS if there were no changes, "
        "EXIT_FAILURE otherwise.\n"
        " files        : Files to process (can be combined with -F).\n"
        " --suffix SFX : Append SFX to the output filename. The default is "
        "'.uncrustify'\n"
        " --prefix PFX : Prepend PFX to the output filename path.\n"
        " --replace    : Replace source files (creates a backup).\n"
        " --no-backup  : Do not create backup and md5 files. Useful if files are "
        "under source control.\n"
        " --if-changed : Write to stdout (or create output FILE) only if a "
        "change was detected.\n"
#ifdef HAVE_UTIME_H
        ...
        " --frag       : Code fragment, assume the first line is indented "
        "correctly.\n"
        " --assume FN  : Uses the filename FN for automatic language detection "
        "if reading\n"
        ...
        " --count-options          : Print the number of available options and "
        "exit.\n"
        " --show-config            : Print out option documentation and exit.\n"
        " --update-config          : Output a new config file. Use with -o "
        "FILE.\n"
        " --update-config-with-doc : Output a new config file. Use with -o "
        "FILE.\n"
        " --universalindent        : Output a config file for Universal Indent "
        "GUI.\n"
        " --detect                 : Detects the config from a source file. Use "
        "with '-f FILE'.\n"
        "                            Detection is fairly limited.\n"
        " --set <option>=<value>   : Sets a new value to a config option.\n"
        "\n"
        "Debug Options:\n"
        " -p FILE      : Dump debug info into FILE, or to stdout if FILE is set "
        "to '-'.\n"
        "                Must be used in combination with '-f FILE'\n"
        " -L SEV       : Set the log severity (see log_levels.h; note 'A' = "
        "'all')\n"
        ...
        "\n"
        "Note: Use comments containing ' *INDENT-OFF*' and ' *INDENT-ON*' to "
        "disable\n"
        "      processing of parts of the source file (these can be overridden "
        "with\n"
...
void setup_crash_handling() {
    // prevent crash popup. uncrustify is a batch processing tool and a popup is
    // unacceptable.
    ::SetErrorMode(::GetErrorMode() | SEM_NOGPFAULTERRORBOX);

    struct local {
        static LONG WINAPI
        crash_filter(_In_ struct _EXCEPTION_POINTERS *exceptionInfo) {
            __try {
                LOG_FMT(LERR, "crash_filter: exception 0x%08X at [%d:%d] (ip=%p)",
                        exceptionInfo->ExceptionRecord->ExceptionCode, cpd.line_number,
                        cpd.column, exceptionInfo->ExceptionRecord->ExceptionAddress);
                log_func_stack(LERR, " [CallStack:", "]\n", 0);

                // treat an exception the same as a parse failure. exceptions can result
                // from parse failures where we do not have specific handling
                // (null-checks for particular parse paths etc.) and callers generally
            ...
            // safety - note that this will not flush like we need, but at least will
...
#define MAXLENGTHOFTHENAME 19
    ...
    if (arg.Present("--help") || arg.Present("-h") || arg.Present("--usage") ||
            arg.Present("-?")) {
        usage(argv[0]);
        return (EXIT_SUCCESS);
    }
    ...
    if (!cfg_file.empty() && cfg_file[0] != '-') {
        ...
        if (options::nl_max() > 0) {
            ...
            if (options::nl_func_var_def_blk() >= options::nl_max()) {
                fprintf(stderr, "The option 'nl_func_var_def_blk' is too big against "
                        "the option 'nl_max'\n");
                log_flush(true);
                exit(EX_CONFIG);
            }
...
#define MAXLENGTHFORARG 256
    ...
    if (source_list != nullptr || p_arg != nullptr) {
        if (source_file != nullptr) {
            usage_error("Cannot specify both the single file option and a multi-file "
                        "option.");
            return (EX_NOUSER);
        }
    ...
    if (source_file == nullptr && source_list == nullptr && p_arg == nullptr) {
        ...
        // Doing a single file
        do_source_file(source_file, output_file, parsed_file, no_backup,
                       keep_mtime);
    } else {
...
static void make_folders(const string &filename) {
    ...
    for (int idx = 0; outname[idx] != 0; idx++) {
        ...
        if (idx > last_idx && (outname[idx] == PATH_SEP)) {
            ...
            outname[idx] =
                PATH_SEP; // reconstruct full path to search for next subpath
        }
    ...
    if (my_stat.st_size == 0) // check if file is empty
    {
        ...
        if (fread(&fm.raw[0], fm.raw.size(), 1, p_file) != 1) {
            ...
            LOG_FMT(LNOTE, "%s: '%s' encoding looks like %s (%d)\n", __func__,
                    ...
                    fm.enc == char_encoding_e::e_ASCII
                    ? "ASCII"
                    : fm.enc == char_encoding_e::e_BYTE
                    ? "BYTES"
                    : fm.enc == char_encoding_e::e_UTF8
                    ? "UTF-8"
                    : fm.enc == char_encoding_e::e_UTF16_LE
                    ? "UTF-16-LE"
                    : fm.enc == char_encoding_e::e_UTF16_BE
                    ? "UTF-16-BE"
                    : "Error",
                    (int)fm.enc);
            retval = 0;
...
static int load_mem_file_config(const std::string &filename, file_mem &fm) {
    ...
    snprintf(buf, sizeof(buf), "%.*s%s", path_dirname_len(cpd.filename.c_str()),
             cpd.filename.c_str(), filename.c_str());
...
int load_header_files() {
    ...
    if (!options::cmt_insert_file_header().empty()) {
        ...
        retval |=
            load_mem_file_config(options::cmt_insert_file_header(), cpd.file_hdr);
    }
    log_rule_B("cmt_insert_file_footer");

    if (!options::cmt_insert_file_footer().empty()) {
        retval |=
            load_mem_file_config(options::cmt_insert_file_footer(), cpd.file_ftr);
    }
    log_rule_B("cmt_insert_func_header");

    if (!options::cmt_insert_func_header().empty()) {
        retval |=
            load_mem_file_config(options::cmt_insert_func_header(), cpd.func_hdr);
    }
    log_rule_B("cmt_insert_class_header");

    if (!options::cmt_insert_class_header().empty()) {
        retval |=
            load_mem_file_config(options::cmt_insert_class_header(), cpd.class_hdr);
    }
...
static bool file_content_matches(const string &filename1,
    ...
    if ((stat(filename1.c_str(), &st1) != 0) ||
            (stat(filename2.c_str(), &st2) != 0) || st1.st_size != st2.st_size) {
        return (false);
    }
...
static void do_source_file(const char *filename_in, const char *filename_out,
     ...
     * write path.
     *
     * Future: many code paths could be simplified if 'bout' were always used and
     * not optionally selected in just for do_check and if_changed.
     */
    if (cpd.if_changed) {
        /*
    ...
    if (cpd.if_changed) {
        for (deque<UINT8>::const_iterator i = cpd.bout->begin(),
                end = cpd.bout->end();
                i != end; ++i) {
            fputc(*i, pfout);
        }
    ...
    if (did_open) {
        ...
        if (filename_tmp != filename_out) {
            // We need to compare and then do a rename (but avoid redundant test when
            // if_changed set)
            if (!cpd.if_changed && file_content_matches(filename_tmp, filename_out)) {
                 ...
                 */
                if (!MoveFileEx(filename_tmp.c_str(), filename_out,
...
#endif
                {
                    LOG_FMT(LERR, "%s: Unable to rename '%s' to '%s'\n", __func__,
                            filename_tmp.c_str(), filename_out);
                    cpd.error_count++;
                }
            }
        }
...
static void add_func_header(c_token_t type, file_mem &fm) {
    ...
    for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next_ncnlnp(pc)) {
        ...
        // Check for one liners for classes. Declarations only. Walk down the
        // chunks.
        ref = pc;

        if (chunk_is_token(ref, CT_CLASS) &&
                get_chunk_parent_type(ref) == CT_NONE && ref->next) {
            ref = ref->next;

            if (chunk_is_token(ref, CT_TYPE) && get_chunk_parent_type(ref) == type &&
                    ref->next) {
                ref = ref->next;
        ...
        // Check for one liners for functions. There'll be a closing brace w/o any
        // newlines. Walk down the chunks.
        ref = pc;
        ...
        if (chunk_is_token(ref, CT_FUNC_DEF) &&
            ...
            while (ref->type != CT_NEWLINE &&
                    (ref = ref->next)) // TODO: is the assignment of ref wanted here?,
                // better move it to the loop
            {
        ...
        while ((ref = chunk_get_prev(ref)) != nullptr) {
            ...
            if (chunk_is_token(ref, CT_ANGLE_CLOSE)) {
                ref = chunk_get_prev_type(ref, CT_ANGLE_OPEN, ref->level,
                                          scope_e::PREPROC);
                continue;
            }
            ...
            if (ref->flags.test(PCF_IN_PREPROC)) {
                ...
                if (tmp != nullptr && get_chunk_parent_type(tmp) == CT_PP_IF) {
                    ...
                    if (chunk_is_comment(tmp) && !options::cmt_insert_before_preproc()) {
                        break;
                    }
                }
            }
            ...
            if (ref->level == pc->level && (ref->flags.test(PCF_IN_PREPROC) ||
...
static void add_msg_header(c_token_t type, file_mem &fm) {
    ...
    for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next_ncnlnp(pc)) {
        ...
        while ((ref = chunk_get_prev(ref)) != nullptr) {
            ...
            // If we hit a parentheses around return type, back up to the open
            // parentheses
            if (chunk_is_token(ref, CT_PAREN_CLOSE)) {
                ref = chunk_get_prev_type(ref, CT_PAREN_OPEN, ref->level,
                                          scope_e::PREPROC);
                continue;
            }
...
void uncrustify_file(const file_mem &fm, FILE *pfout, const char *parsed_file,
    ...
    for (int idx = 0; idx < static_cast<int>(data.size()) - 1; idx++) {
        if (data[idx] == 0) {
            ...
            LOG_FMT(LERR,
                    "The file may be encoded in an unsupported Unicode format.\n");
     ...
     */
    {
        ...
        if (options::mod_sort_import() || options::mod_sort_include() ||
    ...
    if (parsed_file != nullptr) {
        ...
        if (p_file != nullptr) {
            ...
            LOG_FMT(LERR, "%s: Failed to open '%s' for write: %s (%d)\n", __func__,
                    parsed_file, strerror(errno), errno);
            cpd.error_count++;
        }
    }
...
struct lang_ext_t language_exts[] = {
    {".c", "C"},       {".cpp", "CPP"},   {".d", "D"},        {".cs", "CS"},
    {".vala", "VALA"}, {".java", "JAVA"}, {".pawn", "PAWN"},  {".p", "PAWN"},
    {".sma", "PAWN"},  {".inl", "PAWN"},  {".h", "C-Header"}, {".cxx", "CPP"},
    {".hpp", "CPP"},   {".hxx", "CPP"},   {".cc", "CPP"},     {".cp", "CPP"},
    {".C", "CPP"},     {".CPP", "CPP"},   {".c++", "CPP"},    {".di", "D"},
    {".m", "OC"},      {".mm", "OC+"},    {".sqc", "C"}, // embedded SQL
    {".es", "ECMA"},
};
