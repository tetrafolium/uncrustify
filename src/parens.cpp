...
//! Add an open parenthesis after first and add a close parenthesis before the
//! last
...
void do_parens(void) {
  ...
  if (options::mod_full_paren_if_bool()) {
    ...
    while ((pc = chunk_get_next_ncnl(pc)) != nullptr) {
      ...
      chunk_t *pclose =
          chunk_get_next_type(pc, CT_SPAREN_CLOSE, pc->level, scope_e::PREPROC);
...
static void add_parens_between(chunk_t *first, chunk_t *last) {
  ...
  LOG_FMT(LPARADD, "%s: line %zu between %s [lvl=%zu] and %s [lvl=%zu]\n",
          __func__, first->orig_line, first->text(), first->level, last->text(),
          last->level);
  ...
  for (chunk_t *tmp = first_n; tmp != last_p; tmp = chunk_get_next_ncnl(tmp)) {
    tmp->level++;
  }
...
static void check_bool_parens(chunk_t *popen, chunk_t *pclose, int nest) {
  ...
  LOG_FMT(LPARADD,
          "%s(%d): popen on %zu, col %zu, pclose on %zu, col %zu, level=%zu\n",
          __func__, nest, popen->orig_line, popen->orig_col, pclose->orig_line,
          pclose->orig_col, popen->level);
  ...
  while ((pc = chunk_get_next_ncnl(pc)) != nullptr && pc != pclose) {
    if (pc->flags.test(PCF_IN_PREPROC)) {
      LOG_FMT(LPARADD2,
              " -- bail on PP %s [%s] at line %zu col %zu, level %zu\n",
              get_token_name(pc->type), pc->text(), pc->orig_line, pc->orig_col,
              pc->level);
      return;
    }

    if (chunk_is_token(pc, CT_BOOL) || chunk_is_token(pc, CT_QUESTION) ||
        chunk_is_token(pc, CT_COND_COLON) || chunk_is_token(pc, CT_COMMA)) {
