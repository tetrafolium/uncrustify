...
chunk_t *align_var_def_brace(chunk_t *start, size_t span, size_t *p_nl_count) {
	...
	if (chunk_is_token(prev, CT_ASSIGN)) {
		LOG_FMT(LAVDB,
		        "%s(%d): start->text() '%s', type is %s, on orig_line %zu (abort "
		        "due to assign)\n",
	...
	while (pc != nullptr && (pc->level >= start->level || pc->level == 0)) {
		...
		if (fp_active && !pc->flags.test(PCF_IN_CLASS_BASE)) {
			...
			if (chunk_is_token(pc, CT_FUNC_PROTO) ||
				...
				LOG_FMT(LAVDB,
				        "%s(%d): add = '%s', orig_line is %zu, orig_col is %zu, level "
				        "is %zu\n",
			...
			} else if (fp_look_bro && chunk_is_token(pc, CT_BRACE_OPEN) &&
		...
		LOG_FMT(
			LAVDB,
			"%s(%d): pc->text() is '%s', level is %zu, pc->brace_level is %zu\n",
			__func__, __LINE__, chunk_is_newline(pc) ? "Newline" : pc->text(),
		...
		if (!pc->flags.test(PCF_IN_CLASS_BASE) && pc->type != CT_FUNC_CLASS_DEF &&
		    ...
		    pc->prev != nullptr && pc->prev->type != CT_MEMBER) {
			LOG_FMT(LAVDB, "%s(%d): did_this_line is %s\n", __func__, __LINE__,
			        did_this_line ? "TRUE" : "FALSE");
			LOG_FMT(LAVDB,
				...
				if (get_chunk_parent_type(start) == CT_STRUCT &&
					...
					while (chunk_is_token(prev_local, CT_PTR_TYPE) ||
						...
						LOG_FMT(LAVDB, "%s(%d): prev_local '%s', prev_local->type %s\n",
						        __func__, __LINE__, prev_local->text(),
						        get_token_name(prev_local->type));
