...
void align_same_func_call_params(void) {
	...
	for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
		...
		LOG_FMT(
			LASFCP,
			"%s(%d): Func Call @ orig_line is %zu, orig_col is %zu, c_str() '%s'\n",
			__func__, __LINE__, align_fcn->orig_line, align_fcn->orig_col,
			align_fcn_name.c_str());
		...
		if (add_str != nullptr) {
			LOG_FMT(LASFCP, "%s(%d): %s '%s' on line %zu -", __func__, __LINE__,
...
void align_params(chunk_t *start, deque<chunk_t *> &chunks) {
	...
	while ((pc = chunk_get_next(pc)) != nullptr) {
		if (chunk_is_newline(pc) || chunk_is_token(pc, CT_SEMICOLON) ||
		    (chunk_is_token(pc, CT_FPAREN_CLOSE) && pc->level == start->level)) {
			break;
		}
