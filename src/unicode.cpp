...
static bool is_ascii(const vector<UINT8> &data, size_t &non_ascii_cnt,
                     size_t &zero_cnt);
 ...
 */
static bool decode_utf16(const vector<UINT8> &in_data, deque<int> &out_data,
                         char_encoding_e &enc);
...
static bool decode_utf8(const vector<UINT8> &in_data, deque<int> &out_data) {
    ...
    if (in_data.size() >= 3) {
        if ((in_data[0] == 0xef) && (in_data[1] == 0xbb) && (in_data[2] == 0xbf)) {
            idx = 3; // skip it
        }
    }
...
bool decode_unicode(const vector<UINT8> &in_data, deque<int> &out_data,
                    char_encoding_e &enc, bool &has_bom) {
    // check for a BOM
    ...
    if ((zero_cnt > (in_data.size() / 4)) && (zero_cnt <= (in_data.size() / 2))) {
        // likely is UTF-16
