...
void align_oc_msg_colon(chunk_t *so) {
  ...
  while (pc != nullptr && pc->level > level) {
    if (pc->level > (level + 1)) {
      ...
      if (options::align_oc_msg_colon_xcode_like() && first_line &&
          !has_colon) {
        span = 0;
      }
      has_colon = !has_colon;
      first_line = false;
    } else if (!did_line && (lcnt < span + 1) &&
               chunk_is_token(pc, CT_OC_COLON)) {
      has_colon = true;
      cas.Add(pc);
      chunk_t *tmp = chunk_get_prev(pc);

      if (tmp != nullptr && (chunk_is_token(tmp, CT_OC_MSG_FUNC) ||
  ...
  if (longest != nullptr && options::indent_oc_msg_prioritize_first_colon() &&
      len_diff > 0 &&
