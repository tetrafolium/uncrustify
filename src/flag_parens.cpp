...
chunk_t *flag_parens(chunk_t *po, pcf_flags_t flags, c_token_t opentype,
    ...
    if (paren_close == nullptr) {
        LOG_FMT(LERR, "%s(%d): no match for '%s' at [%zu:%zu]", __func__, __LINE__,
    ...
    LOG_FMT(
        LFLPAREN,
        "%s(%d): between  po is '%s', orig_line is %zu, orig_col is %zu, and\n",
        __func__, __LINE__, po->text(), po->orig_line, po->orig_col);
    LOG_FMT(LFLPAREN,
            "%s(%d): paren_close is '%s', orig_line is %zu, orig_col is %zu, "
            "type is %s, parent_type is %s\n",
            __func__, __LINE__, paren_close->text(), paren_close->orig_line,
            paren_close->orig_col, get_token_name(opentype),
            get_token_name(parenttype));
    ...
    if (po != paren_close) {
        if (flags != PCF_NONE || (parent_all && parenttype != CT_NONE)) {
            chunk_t *pc;
