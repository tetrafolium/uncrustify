...
void do_code_width(void) {
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
    if (!chunk_is_newline(pc) && !chunk_is_comment(pc) &&
        chunk_is_not_token(pc, CT_SPACE) && is_past_width(pc)) {
      ...
      if (split_OK) {
        ...
        LOG_FMT(LSPLIT,
                "%s(%d): Bailed on orig_line=%zu, orig_col=%zu, for %s\n",
                __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());
        break;
      }
    }
...
static const token_pri pri_table[] = {
    ...
    {CT_COND_COLON, 20},
    {CT_FPAREN_OPEN,
     21}, // break after function open paren not followed by close paren
    {CT_QUALIFIER, 25},
    {CT_CLASS, 25},
    {CT_STRUCT, 25},
    {CT_TYPE, 25},
...
static void try_split_here(cw_entry &ent, chunk_t *pc) {
  ...
  if (prev == nullptr ||
    ...
    if (prev != nullptr) {
      LOG_FMT(LSPLIT,
              "%s(%d): Can't split after a newline, orig_line is %zu, return\n",
  ...
  if (chunk_is_token(pc, CT_FPAREN_OPEN)) {
    ...
    if (chunk_is_token(next, CT_FPAREN_CLOSE)) {
      LOG_FMT(LSPLIT,
              "%s(%d): Can't split a function without arguments, return\n",
              __func__, __LINE__);
      return;
    }
  }
  ...
  if (!options::ls_code_width() && pc_pri >= 20) {
    LOG_FMT(LSPLIT,
            "%s(%d): keep common groupings unless ls_code_width, return\n",
            __func__, __LINE__);
    return;
  }
  LOG_FMT(LSPLIT, "%s(%d):\n", __func__, __LINE__);
  ...
  if (pc_pri == 25) {
    ...
    if (chunk_is_not_token(next, CT_WORD) &&
      ...
      LOG_FMT(
          LSPLIT,
          "%s(%d): don't break after last term of a qualified type, return\n",
          __func__, __LINE__);
      return;
    }
  }
...
static bool split_line(chunk_t *start) {
  LOG_FUNC_ENTRY();
  LOG_FMT(LSPLIT,
          "%s(%d): start->text() '%s', orig_line is %zu, orig_col is %zu, type "
          "is %s\n",
  ...
  LOG_FMT(LSPLIT,
          "   start->parent_type %s, (PCF_IN_FCN_DEF is %s), (PCF_IN_FCN_CALL "
          "is %s)\n",
  ...
  if (ent.pc == nullptr) {
    LOG_FMT(LSPLIT,
            "\n%s(%d):    TRY_SPLIT yielded NO SOLUTION for orig_line %zu at "
            "'%s' [%s]\n",
            __func__, __LINE__, start->orig_line, start->text(),
            get_token_name(start->type));
  } else {
    LOG_FMT(LSPLIT,
            "\n%s(%d):    TRY_SPLIT yielded '%s' [%s] on orig_line %zu\n",
            __func__, __LINE__, ent.pc->text(), get_token_name(ent.pc->type),
            ent.pc->orig_line);
    LOG_FMT(LSPLIT, "%s(%d): ent at '%s', orig_col is %zu\n", __func__,
            __LINE__, ent.pc->text(), ent.pc->orig_col);
  }
  ...
  if (ent.pc == nullptr) {
    ...
    if (((chunk_is_token(ent.pc, CT_ARITH) ||
        ...
        (chunk_is_token(ent.pc, CT_BOOL) && (options::pos_bool() & TP_LEAD))) {
      pc = ent.pc;
    } else {
  ...
  if (pc == nullptr) {
    ...
    if (chunk_is_token(start, CT_PAREN_CLOSE) ||
        ...
        chunk_is_token(start, CT_VSEMICOLON) || start->len() == 0) {
  ...
  if (prev != nullptr && !chunk_is_newline(pc) && !chunk_is_newline(prev)) {
    ...
    LOG_FMT(LSPLIT,
            "%s(%d): text() '%s', type %s, started on token '%s', type %s\n",
...
static void split_fcn_params_full(chunk_t *start) {
  ...
  while ((fpo = chunk_get_prev(fpo)) != nullptr) {
    LOG_FMT(LSPLIT, "%s(%d): %s, orig_col is %zu, level is %zu\n", __func__,
            __LINE__, fpo->text(), fpo->orig_col, fpo->level);
...
static void split_fcn_params(chunk_t *start) {
  ...
  while (pc != nullptr) {
    ...
    if (chunk_is_newline(pc)) {
      ...
      if (chunk_is_token(pc, CT_COMMA) || chunk_is_token(pc, CT_FPAREN_CLOSE)) {
        if (cur_width == 0) {
          fprintf(stderr,
                  "%s(%d): cur_width is ZERO, cannot be decremented, at line "
                  "%zu, column %zu\n",
  ...
  while ((prev = chunk_get_prev(prev)) != nullptr) {
    LOG_FMT(LSPLIT,
            "%s(%d): pc '%s', pc->level is %zu, prev '%s', prev->type is %s\n",
            __func__, __LINE__, pc->text(), pc->level, prev->text(),
