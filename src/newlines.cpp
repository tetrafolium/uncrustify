...
static void newline_iarf_pair(chunk_t *before, chunk_t *after, iarf_e av,
                              bool check_nl_assign_leave_one_liners = false);
 ...
 * "code\n\ncomment\nif (...)" or "code\ncomment\nif (...)"
 */
static void newlines_if_for_while_switch_pre_blank_lines(chunk_t *start,
                                                         iarf_e nl_opt);
 ...
 * VBraces will stay VBraces, conversion to real ones should have already
 * happened "if (...)\ncode\ncode" or "if (...)\ncode\n\ncode"
 */
 ...
 */
static void newlines_struct_union(chunk_t *start, iarf_e nl_opt,
                                  bool leave_trailing);
...
static iarf_e newline_template_option(chunk_t *pc, iarf_e special, iarf_e base,
                                      iarf_e fallback);
...
static void setup_newline_add(chunk_t *prev, chunk_t *nl, chunk_t *next) {
  ...
  if (prev == nullptr || nl == nullptr || next == nullptr) {
    return;
  }
  undo_one_liner(prev);
  ...
  if (prev->flags.test(PCF_IN_PREPROC) && next->flags.test(PCF_IN_PREPROC)) {
...
chunk_t *newline_add_before(chunk_t *pc) {
  ...
  LOG_FMT(LNEWLINE,
          "%s(%d): text() '%s', on orig_line is %zu, orig_col is %zu, "
          "pc->column is %zu",
...
chunk_t *newline_add_after(chunk_t *pc) {
  ...
  LOG_FMT(LNEWLINE, "%s(%d): '%s' on line %zu", __func__, __LINE__, pc->text(),
          pc->orig_line);
  log_func_stack_inline(LNEWLINE);
...
static void newline_end_newline(chunk_t *br_close) {
  ...
  if (!chunk_is_newline(next) && !chunk_is_comment(next)) {
    ...
    if (br_close->flags.test(PCF_IN_PREPROC) && next != nullptr &&
    ...
    LOG_FMT(LNEWLINE, "%s(%d): %zu:%zu add newline after '%s'\n", __func__,
            __LINE__, br_close->orig_line, br_close->orig_col,
            br_close->text());
...
static void newline_min_after(chunk_t *ref, size_t count, pcf_flag_e flag) {
  ...
  LOG_FMT(LNEWLINE, "%s(%d): '%s' line %zu - count=%zu flg=%s:", __func__,
          __LINE__, ref->text(), ref->orig_line, count,
...
chunk_t *newline_add_between(chunk_t *start, chunk_t *end) {
  ...
  if (start == nullptr || end == nullptr || chunk_is_token(end, CT_IGNORED)) {
    return (nullptr);
  }
  LOG_FMT(LNEWLINE,
          "%s(%d): start->text() is '%s', type is %s, orig_line is %zu, "
          "orig_col is %zu\n",
          __func__, __LINE__, start->text(), get_token_name(start->type),
          start->orig_line, start->orig_col);
  LOG_FMT(
      LNEWLINE,
      "%s(%d): and end->text() is '%s', orig_line is %zu, orig_col is %zu\n  ",
...
void newline_del_between(chunk_t *start, chunk_t *end) {
  ...
  do {
    ...
    if (chunk_is_newline(pc)) {
      ...
      if ((!chunk_is_comment(prev) && !chunk_is_comment(next)) ||
          chunk_is_newline(prev) || chunk_is_newline(next)) {
  ...
  if (!start_removed && chunk_is_str(end, "{", 1) &&
      (chunk_is_str(start, ")", 1) || chunk_is_token(start, CT_DO) ||
...
void newlines_sparens() {
  ...
  for (sparen_open =
           chunk_get_next_type(chunk_get_head(), CT_SPAREN_OPEN, ANY_LEVEL);
       sparen_open != nullptr; sparen_open = chunk_get_next_type(
                                   sparen_open, CT_SPAREN_OPEN, ANY_LEVEL)) {
    chunk_t *sparen_close =
        chunk_get_next_type(sparen_open, CT_SPAREN_CLOSE, sparen_open->level);
    ...
    // Add a newline after '(' if an if/for/while/switch condition spans
    // multiple lines, as e.g. required by the ROS 2 development style
    // guidelines:
    ...
    // Add a newline before ')' if an if/for/while/switch condition spans
    // multiple lines. Overrides nl_before_if_closing_paren if both are
    // specified.
    if (is_multiline && options::nl_multi_line_sparen_close() != IARF_IGNORE) {
      ...
      if (ctrl_structure->type == CT_IF || ctrl_structure->type == CT_ELSEIF) {
        ...
        newline_iarf_pair(sparen_content_end, sparen_close,
                          options::nl_before_if_closing_paren());
      }
    }
  }
} // newlines_sparens
...
static bool newlines_if_for_while_switch(chunk_t *start, iarf_e nl_opt) {
  ...
  if (nl_opt == IARF_IGNORE ||
      (start->flags.test(PCF_IN_PREPROC) && !options::nl_define_macro())) {
    return (false);
  }
  bool retval = false;
  ...
  if (chunk_is_token(pc, CT_SPAREN_OPEN)) {
    ...
    if ((chunk_is_token(brace_open, CT_BRACE_OPEN) ||
      ...
      if (chunk_is_token(brace_open, CT_VBRACE_OPEN)) {
        ...
        if (nl_opt & IARF_ADD) {
          newline_iarf_pair(close_paren, chunk_get_next_ncnl(brace_open),
                            nl_opt);
          pc = chunk_get_next_type(brace_open, CT_VBRACE_CLOSE,
                                   brace_open->level);
...
static void newlines_if_for_while_switch_pre_blank_lines(chunk_t *start,
   ...
   */
  for (chunk_t *pc = chunk_get_prev(start); pc != nullptr;
    ...
    if (chunk_is_newline(pc)) {
      ...
      if (pc->nl_count > 1 || chunk_is_newline(chunk_get_prev_nvb(pc))) {
        ...
        if ((nl_opt & IARF_REMOVE) && !pc->flags.test(PCF_VAR_DEF)) {
          ...
          // can keep using pc because anything other than newline stops loop,
          // and we delete if newline
      ...
      // vbrace close is ok because it won't go into output, so we should skip
      // it
      last_nl = nullptr;
      continue;
    } else {
      if (do_add) // we found something previously besides a comment or a new
                  // line
      {
...
bool do_it_newlines_func_pre_blank_lines(chunk_t *last_nl,
                                         c_token_t start_type) {
  LOG_FUNC_ENTRY();
  ...
  LOG_FMT(
      LNLFUNCT,
      "%s(%d): orig_line is %zu, orig_col is %zu, type is %s, text() is '%s'\n",
      __func__, __LINE__, last_nl->orig_line, last_nl->orig_col,
      get_token_name(last_nl->type), last_nl->text());
  ...
  case CT_FUNC_CLASS_DEF: {
    ...
    if (options::nl_before_func_class_def() != last_nl->nl_count) {
      LOG_FMT(LNLFUNCT, "%s(%d):   set blank line(s) to %u\n", __func__,
              __LINE__, options::nl_before_func_class_def());
  ...
  case CT_FUNC_CLASS_PROTO: {
    ...
    if (options::nl_before_func_class_proto() != last_nl->nl_count) {
      LOG_FMT(LNLFUNCT, "%s(%d):   set blank line(s) to %u\n", __func__,
              __LINE__, options::nl_before_func_class_proto());
  ...
  case CT_FUNC_DEF: {
    LOG_FMT(
        LNLFUNCT,
        "%s(%d): nl_before_func_body_def() is %u, last_nl->nl_count is %zu\n",
    ...
    if (options::nl_before_func_body_def() != last_nl->nl_count) {
      LOG_FMT(LNLFUNCT, "%s(%d):    set blank line(s) to %u\n", __func__,
              __LINE__, options::nl_before_func_body_def());
      log_rule_B("nl_before_func_body_def");
  ...
  case CT_FUNC_PROTO: {
    ...
    if (options::nl_before_func_body_proto() != last_nl->nl_count) {
      LOG_FMT(LNLFUNCT, "%s(%d):   set blank line(s) to %u\n", __func__,
              __LINE__, options::nl_before_func_body_proto());
      log_rule_B("nl_before_func_body_proto");
  ...
  default: {
    LOG_FMT(LERR,
            "%s(%d):   setting to blank line(s) at line %zu not possible\n",
...
static void newlines_func_pre_blank_lines(chunk_t *start,
  ...
  if (start == nullptr ||
      ((start_type != CT_FUNC_CLASS_DEF ||
       ...
       (start_type != CT_FUNC_DEF || options::nl_before_func_body_def() == 0) &&
       (start_type != CT_FUNC_PROTO ||
        options::nl_before_func_body_proto() == 0))) {
    return;
  }
  LOG_FMT(LNLFUNCT,
          "%s(%d):    set blank line(s): for <NL> at line %zu, column %zu, "
          "start_type is %s\n",
          __func__, __LINE__, start->orig_line, start->orig_col,
          get_token_name(start_type));
  ...
  for (pc = chunk_get_prev(start); pc != nullptr; pc = chunk_get_prev(pc)) {
    LOG_FMT(LNLFUNCT,
            "%s(%d): orig_line is %zu, orig_col is %zu, type is %s, text() is "
            "'%s', nl_count is %zu\n",
            __func__, __LINE__, pc->orig_line, pc->orig_col,
            get_token_name(pc->type), pc->text(), pc->nl_count);

    if (chunk_is_newline(pc)) {
      last_nl = pc;
      LOG_FMT(LNLFUNCT,
              "%s(%d):    <chunk_is_newline> found at line %zu, column %zu, "
              "nl_count is %zu\n",
      ...
      LOG_FMT(LNLFUNCT,
              "%s(%d):    <chunk_is_comment> found at line %zu, column %zu\n",
              __func__, __LINE__, pc->orig_line, pc->orig_col);
      ...
      continue;
               ...
               chunk_is_token(pc, CT_TYPE) || chunk_is_token(pc, CT_TEMPLATE) ||
...
static chunk_t *get_closing_brace(chunk_t *start) {
  ...
  for (pc = start; (pc = chunk_get_next(pc)) != nullptr;) {
    ...
    // for some reason, we can have newlines between if and opening brace that
    // are lower level than either
...
static void newlines_if_for_while_switch_post_blank_lines(chunk_t *start,
  ...
  LOG_FMT(LNEWLINE,
          "%s(%d): pc->text() is '%s', type is %s, orig_line is %zu, "
          "orig_column is %zu\n",
  ...
  if (chunk_is_token(start, CT_IF)) {
    while (true) {
      ...
      if (next != nullptr &&
        ...
        LOG_FMT(LNEWLINE,
                "%s(%d): pc->text() is '%s', type %s, orig_line %zu, "
                "orig_column %zu\n",
                __func__, __LINE__, pc->text(), get_token_name(pc->type),
                pc->orig_line, pc->orig_col);
      } else {
  ...
  if (chunk_is_token(start, CT_DO)) {
    ...
    LOG_FMT(
        LNEWLINE,
        "%s(%d): pc->text() is '%s', type %s, orig_line %zu, orig_column %zu\n",
        __func__, __LINE__, pc->text(), get_token_name(pc->type), pc->orig_line,
        pc->orig_col);
  }
  ...
  if (nl_opt & IARF_ADD) {
    ...
    LOG_FMT(LNEWLINE,
            "%s(%d): next->text() is '%s', type %s, orig_line %zu, orig_column "
            "%zu\n",
            __func__, __LINE__, next->text(), get_token_name(next->type),
            next->orig_line, next->orig_col);
    ...
    if (next->type != CT_BRACE_CLOSE) {
      ...
      if (chunk_is_newline(next = chunk_get_next_nvb(pc))) {
        LOG_FMT(LNEWLINE,
                "%s(%d): next->text() is '%s', type %s, orig_line %zu, "
                "orig_column %zu\n",
                __func__, __LINE__, next->text(), get_token_name(next->type),
                next->orig_line, next->orig_col);
      ...
      } else if (nl_count == 1) // if we don't have enough newlines
      {
        ...
        if (have_pre_vbrace_nl) {
          ...
          LOG_FMT(LNEWLINE,
                  "%s(%d): prev->text() is '%s', type %s, orig_line %zu, "
                  "orig_column %zu\n",
          ...
          if (chunk_is_token(pc, CT_PREPROC) &&
            ...
            LOG_FMT(LNEWLINE,
                    "%s(%d): cannot add newline after orig_line %zu due to "
                    "nl_squeeze_ifdef\n",
...
static void newlines_struct_union(chunk_t *start, iarf_e nl_opt,
  ...
  if (chunk_is_token(pc, CT_BRACE_OPEN)) {
    ...
    if (leave_trailing && !chunk_is_comment(next) && !chunk_is_newline(next)) {
      nl_opt = IARF_IGNORE;
    }
...
// enumerator-list(optional) } enum-key attr(optional)
// nested-name-specifier(optional) identifier enum-base(optional) ; TODO
...
static void newlines_namespace(chunk_t *start) {
  ...
  LOG_FMT(
      LNEWLINE,
      "%s(%d): braceOpen->orig_line is %zu, orig_col is %zu, text() is '%s'\n",
      __func__, __LINE__, braceOpen->orig_line, braceOpen->orig_col,
      braceOpen->text());
  ...
  LOG_FMT(LNEWLINE,
          "%s(%d): beforeBrace->orig_line is %zu, orig_col is %zu, text() is "
          "'%s'\n",
          __func__, __LINE__, beforeBrace->orig_line, beforeBrace->orig_col,
          beforeBrace->text());
  // namespace {
...
static void newlines_do_else(chunk_t *start, iarf_e nl_opt) {
  ...
  if (next != nullptr && (chunk_is_token(next, CT_BRACE_OPEN) ||
...
static bool is_var_def(chunk_t *pc, chunk_t *next) {
  ...
  return (chunk_is_type(next) || chunk_is_token(next, CT_WORD) ||
...
static chunk_t *newline_def_blk(chunk_t *start, bool fn_top) {
  ...
  while (pc != nullptr && (pc->level >= start->level || pc->level == 0)) {
    ...
    if (!did_this_line && pc->type != CT_FUNC_CLASS_DEF &&
      ...
      if (chunk_is_token(pc, CT_TYPEDEF)) {
        ...
        else if (typedef_blk && (options::nl_typedef_blk_in() > 0)) {
        ...
        if (var_blk && first_var_blk && fn_top &&
          ...
          LOG_FMT(LBLANKD, "%s(%d): nl_func_var_def_blk %zu\n", __func__,
                  __LINE__, prev->orig_line);
          newline_min_after(prev, 1 + options::nl_func_var_def_blk(),
                            PCF_VAR_DEF);
        }
        ...
        if (!var_blk && !first_var_blk && options::nl_var_def_blk_start() > 0) {
        ...
        else if (typedef_blk && (options::nl_typedef_blk_end() > 0)) {
...
static bool collapse_empty_body(chunk_t *br_open) {
  ...
  for (chunk_t *pc = chunk_get_next(br_open);
       pc != nullptr && pc->type != CT_BRACE_CLOSE; pc = chunk_get_next(pc)) {
...
static void newlines_brace_pair(chunk_t *br_open) {
  ...
  if (get_chunk_parent_type(br_open) == CT_FUNC_DEF &&
        ...
        chunk_is_paren_close(tmp)) // need to check the conditions.
    {
      ...
      while (tmp != nullptr && (tmp = chunk_get_next(tmp)) != nullptr &&
             !chunk_is_closing_brace(tmp) && (chunk_get_next(tmp) != nullptr)) {
        LOG_FMT(LNL1LINE,
                "%s(%d): tmp->orig_line is %zu, tmp->orig_col is %zu, text() "
                "is '%s'\n",
      ...
      if (is_it_possible) {
        ...
        chunk_flags_set(br_open,
                        PCF_ONE_LINER); // set the one liner flag if needed
  ...
  if (!one_liner_nl_ok(br_open)) {
    LOG_FMT(LNL1LINE,
            "%s(%d): br_open->orig_line is %zu, br_open->orig_col is %zu, a "
            "new line may NOT be added\n",
  ...
  LOG_FMT(LNL1LINE,
          "%s(%d): br_open->text() '%s', br_open->type [%s], "
          "br_open->parent_type [%s]\n",
  ...
  // fixes #1245 will add new line between tsquare and brace open based on
  // nl_tsquare_brace
  ...
  if (chunk_is_token(br_open, CT_BRACE_OPEN)) {
    ...
    if (chunk_closeing_brace != nullptr) {
      if (chunk_closeing_brace->orig_line > br_open->orig_line) {
        ...
        if (chunk_is_token(prev, CT_TSQUARE) && chunk_is_newline(next)) {
...
static void newline_case(chunk_t *start) {
  ...
  } while (prev != nullptr && prev->type != CT_BRACE_OPEN &&
...
static void newline_before_return(chunk_t *start) {
  ...
  if (pc == nullptr || (chunk_is_token(pc, CT_BRACE_OPEN) ||
...
static void newline_after_return(chunk_t *start) {
  ...
  if (after == nullptr || chunk_is_token(after, CT_BRACE_CLOSE) ||
...
static void newline_iarf_pair(chunk_t *before, chunk_t *after, iarf_e av,
                              bool check_nl_assign_leave_one_liners) {
  LOG_FUNC_ENTRY();
  ...
  if (av & IARF_ADD) {
    ...
    if (nl != nullptr && av == IARF_FORCE && nl->nl_count > 1) {
      nl->nl_count = 1;
    }
...
static void newline_func_def_or_call(chunk_t *start) {
  ...
  LOG_FMT(LNFD,
          "%s(%d): called on start->text() is '%s', orig_line is %zu, orig_col "
          "is %zu, [%s/%s]\n",
          __func__, __LINE__, start->text(), start->orig_line, start->orig_col,
          get_token_name(start->type),
          get_token_name(get_chunk_parent_type(start)));
  ...
  LOG_FMT(LNFD, "%s(%d): is_def is %s, is_call is %s\n", __func__, __LINE__,
          is_def ? "TRUE" : "FALSE", is_call ? "TRUE" : "FALSE");
  ...
  if (is_call) {
    ...
    auto atmp =
        is_def ? options::nl_func_def_paren() : options::nl_func_paren();
    ...
    prev = chunk_is_paren_close(prev)
               ? nullptr
               : chunk_get_prev_ncnlni(prev); // Issue #2279

    log_rule_B("nl_func_class_scope");

    if (chunk_is_token(prev, CT_DC_MEMBER) &&
        (options::nl_func_class_scope() != IARF_IGNORE)) {
      newline_iarf(chunk_get_prev_ncnlni(prev),
                   options::nl_func_class_scope()); // Issue #2279
    }
    ...
    if (prev != nullptr && prev->type != CT_ACCESS_COLON) {
      ...
      if (tmp_next != nullptr && tmp_next->type != CT_FUNC_CLASS_DEF) {
        ...
        if (a != IARF_IGNORE && prev != nullptr) {
          LOG_FMT(LNFD,
                  "%s(%d): prev->text() '%s', orig_line is %zu, orig_col is "
                  "%zu, [%s/%s]\n",
          ...
          if (prev != nullptr && prev->type != CT_BRACE_CLOSE &&
              prev->type != CT_VBRACE_CLOSE && prev->type != CT_BRACE_OPEN &&
    ...
    if (chunk_is_str(pc, ")", 1)) {
      ...
      atmp =
          is_def ? options::nl_func_def_empty() : options::nl_func_decl_empty();
  ...
  iarf_e as =
      is_def ? options::nl_func_def_start() : options::nl_func_decl_start();
...
static bool one_liner_nl_ok(chunk_t *pc) {
  LOG_FUNC_ENTRY();
  LOG_FMT(LNL1LINE,
          "%s(%d): check type is %s, parent is %s, flag is %s, orig_line is "
          "%zu, orig_col is %zu\n",
  ...
  if (chunk_is_closing_brace(br_open)) {
    br_open = chunk_get_prev_type(br_open,
                                  chunk_is_token(br_open, CT_BRACE_CLOSE)
                                      ? CT_BRACE_OPEN
                                      : CT_VBRACE_OPEN,
                                  br_open->level, scope_e::ALL);
  } else {
    while (br_open != nullptr && br_open->flags.test(PCF_ONE_LINER) &&
  ...
  if (pc != nullptr && pc->flags.test(PCF_ONE_LINER) &&
    ...
    if (options::nl_class_leave_one_liners() && pc->flags.test(PCF_IN_CLASS)) {
    ...
    if (options::nl_getset_leave_one_liners() &&
      ...
      LOG_FMT(LNL1LINE,
              "%s(%d): false (get/set), a new line may NOT be added\n",
              __func__, __LINE__);
      return (false);
    }
    // Issue #UT-98
    ...
    if (options::nl_cs_property_leave_one_liners() &&
      ...
      LOG_FMT(LNL1LINE,
              "%s(%d): false (c# property), a new line may NOT be added\n",
              __func__, __LINE__);
      return (false);
    }
    log_rule_B("nl_func_leave_one_liners");
    ...
    if (options::nl_oc_msg_leave_one_liner() && pc->flags.test(PCF_IN_OC_MSG)) {
...
void undo_one_liner(chunk_t *pc) {
  ...
  if (pc != nullptr && pc->flags.test(PCF_ONE_LINER)) {
    LOG_FMT(LNL1LINE,
            "%s(%d): pc->text() '%s', orig_line is %zu, orig_col is %zu",
            __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
    ...
    while ((tmp = chunk_get_prev(tmp)) != nullptr) {
      if (!tmp->flags.test(PCF_ONE_LINER)) {
        LOG_FMT(LNL1LINE,
                "%s(%d): tmp->text() '%s', orig_line is %zu, orig_col is %zu, "
                "--> break\n",
                __func__, __LINE__, tmp->text(), tmp->orig_line, tmp->orig_col);
        break;
      }
      LOG_FMT(LNL1LINE,
              "%s(%d): clear for tmp->text() '%s', orig_line is %zu, orig_col "
              "is %zu",
              __func__, __LINE__, tmp->text(), tmp->orig_line, tmp->orig_col);
...
static void nl_create_list_liner(chunk_t *brace_open) {
  ...
  // See if we get a newline between the next text and the vbrace_close
  chunk_t *closing =
      chunk_get_next_type(brace_open, CT_BRACE_CLOSE, brace_open->level);
...
void newlines_cleanup_angles() {
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr;
       pc = chunk_get_next_ncnl(pc)) {
    LOG_FMT(LBLANK,
...
void newlines_cleanup_braces(bool first) {
  ...
  for (; pc != nullptr; pc = chunk_get_next_ncnl(pc)) {
    ...
    if (chunk_is_token(pc, CT_IF)) {
      ...
      if (language_is_set(LANG_OC) && (pc->str[0] == '@') &&
      ...
      case CT_OC_CLASS: {
        if (pc->level == pc->brace_level) {
          ...
          for (tmp = chunk_get_prev(pc); tmp != nullptr;
            ...
            if (chunk_is_token(tmp, CT_OC_INTF) ||
              ...
              LOG_FMT(LBLANK,
                      "%s(%d): orig_line is %zu, orig_col is %zu, may be "
                      "remove/force newline before {\n",
                      __func__, __LINE__, pc->orig_line, pc->orig_col);

              if (chunk_is_token(tmp, CT_OC_INTF)) {
                log_rule_B("nl_oc_interface_brace");
                newlines_do_else(chunk_get_prev_nnl(pc),
                                 options::nl_oc_interface_brace());
              } else {
      ...
      case CT_BRACED_INIT_LIST: {
        ...
        newline_iarf_pair(chunk_get_prev_nnl(pc), pc,
                          options::nl_type_brace_init_lst(), true);
        break;
      }
      ...
      if ((options::nl_brace_struct_var() != IARF_IGNORE) &&
        ...
        if (next != nullptr && next->type != CT_SEMICOLON &&
            ...
            next->type != CT_PAREN_CLOSE && next->type != CT_WHILE_OF_DO &&
            ...
            !pc->flags.test(PCF_IN_TYPEDEF) && !chunk_is_newline(next) &&
            !chunk_is_comment(next)) {
          // #1258
          // dont add newline between two consecutive braces closes, if the
          // second is a part of one liner.
      ...
      if (options::nl_after_vbrace_open() ||
        ...
        if (chunk_is_semicolon(next)) {
          ...
          add_it = (options::nl_after_vbrace_open() &&
                    next->type != CT_VBRACE_CLOSE && !chunk_is_comment(next) &&
      ...
      if (((get_chunk_parent_type(pc) == CT_IF ||
        ...
        if (pc->flags.test(PCF_ONE_LINER)) {
          // split one-liner
          chunk_t *end =
              chunk_get_next(chunk_get_next_type(pc->next, CT_SEMICOLON, -1));
          // Scan for clear flag
          ...
          for (chunk_t *temp = pc; temp != end; temp = chunk_get_next(temp)) {
            LOG_FMT(LNEWLINE,
                    "%s(%d): text() is '%s', type is %s, level is %zu\n   ",
                    __func__, __LINE__, temp->text(),
                    get_token_name(temp->type), temp->level);
          ...
          chunk_is_token(prev, CT_FPAREN_CLOSE)) // Issue #1122
      {
        ...
        newline_iarf(chunk_get_prev_ncnlni(pc),
                     options::nl_before_throw()); // Issue #2279
      }
      ...
      if (!pc->flags.test(PCF_IN_SPAREN) && !pc->flags.test(PCF_IN_PREPROC) &&
      ...
      if (get_chunk_parent_type(pc) == CT_TEMPLATE) {
        ...
        if (next != nullptr && next->level == next->brace_level) {
          tmp = chunk_get_prev_ncnlni(
              chunk_get_prev_type(pc, CT_ANGLE_OPEN, pc->level)); // Issue #2279
          ...
          if (chunk_is_token(tmp, CT_TEMPLATE)) {
            ...
            } else if (get_chunk_parent_type(next) ==
                       CT_FUNC_DEF) // function definition
            {
              auto const action = newline_template_option(
                  pc, options::nl_template_func_def_special(),
                  options::nl_template_func_def(), options::nl_template_func());
              log_rule_B("nl_template_func_def_special");
              log_rule_B("nl_template_func_def");
              log_rule_B("nl_template_func");
              newline_iarf(pc, action);
            } else if (get_chunk_parent_type(next) ==
                       CT_FUNC_PROTO) // function declaration
            {
              auto const action = newline_template_option(
                  pc, options::nl_template_func_decl_special(),
            ...
            {
              auto const action = newline_template_option(
                  pc, options::nl_template_class_decl_special(),
            ...
            {
              auto const action = newline_template_option(
                  pc, options::nl_template_class_def_special(),
      ...
      if (next != nullptr) {
        ...
        if (!chunk_is_token(next, CT_ASSIGN)) {
          ...
          // Issue #2186
          chunk_t *braceOpen =
              chunk_get_next_type(pc, CT_BRACE_OPEN, pc->level);

          if (braceOpen == nullptr) {
            // fatal error
            LOG_FMT(LERR,
                    "%s(%d): Missing BRACE_OPEN after namespace\n   orig_line "
                    "is %zu, orig_col is %zu\n",
                    __func__, __LINE__, pc->orig_line, pc->orig_col);
            exit(EXIT_FAILURE);
          }
          LOG_FMT(LNEWLINE,
                  "%s(%d): braceOpen->orig_line is %zu, orig_col is %zu, "
                  "text() is '%s'\n",
                  __func__, __LINE__, braceOpen->orig_line, braceOpen->orig_col,
                  braceOpen->text());
          log_pcf_flags(LNEWLINE, braceOpen->flags);
...
void newlines_insert_blank_lines(void) {
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr;
    ...
    if (chunk_is_token(pc, CT_IF)) {
      ...
      newlines_if_for_while_switch_pre_blank_lines(pc,
                                                   options::nl_before_for());
      log_rule_B("nl_before_for");
      newlines_if_for_while_switch_post_blank_lines(pc,
                                                    options::nl_after_for());
      log_rule_B("nl_after_for");
    } else if (chunk_is_token(pc, CT_WHILE)) {
      newlines_if_for_while_switch_pre_blank_lines(pc,
                                                   options::nl_before_while());
      log_rule_B("nl_before_while");
      newlines_if_for_while_switch_post_blank_lines(pc,
                                                    options::nl_after_while());
      log_rule_B("nl_after_while");
    } else if (chunk_is_token(pc, CT_SWITCH)) {
      newlines_if_for_while_switch_pre_blank_lines(pc,
                                                   options::nl_before_switch());
      log_rule_B("nl_before_switch");
      newlines_if_for_while_switch_post_blank_lines(pc,
                                                    options::nl_after_switch());
      log_rule_B("nl_after_switch");
    } else if (chunk_is_token(pc, CT_SYNCHRONIZED)) {
      newlines_if_for_while_switch_pre_blank_lines(
          pc, options::nl_before_synchronized());
      log_rule_B("nl_before_synchronized");
      newlines_if_for_while_switch_post_blank_lines(
          pc, options::nl_after_synchronized());
        ...
        if (nnl != nullptr) {
          if (pnl != nullptr) {
            if (pnl->nl_count > 1) {
              ...
              LOG_FMT(LNEWLINE,
                      "%s(%d): moved from after line %zu to after %zu\n",
          ...
          if (chunk_is_token(ppr, CT_PP_IF) ||
            ...
            if (nnl->nl_count > 1) {
              ...
              LOG_FMT(LNEWLINE,
                      "%s(%d): trimmed newlines after line %zu from %zu\n",
...
void newlines_squeeze_paren_close(void) {
  ...
  for (pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
    ...
    if (next != nullptr && prev != nullptr && chunk_is_paren_close(next) &&
...
void newlines_eat_start_end(void) {
  ...
  if (cpd.frag_cols == 0 && ((options::nl_start_of_file() & IARF_REMOVE) ||
    ...
    if (pc != nullptr) {
      if (chunk_is_token(pc, CT_NEWLINE)) {
        ...
        LOG_FMT(LNEWLINE, "%s(%d): %zu:%zu add newline before '%s'\n", __func__,
                __LINE__, pc->orig_line, pc->orig_col, pc->text());
        MARK_CHANGE();
      }
    }
  }
  ...
  if (cpd.frag_cols == 0 && ((options::nl_end_of_file() & IARF_REMOVE) ||
...
void newlines_chunk_pos(c_token_t chunk_type, token_pos_e mode) {
  ...
  if (!(mode & (TP_JOIN | TP_LEAD | TP_TRAIL)) && chunk_type != CT_COMMA) {
    return;
  }
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr;
    ...
    if (pc->type == chunk_type) {
      ...
      LOG_FMT(
          LNEWLINE,
          "%s(%d): prev->orig_line is %zu, orig_col is %zu, text() is '%s'\n",
          __func__, __LINE__, prev->orig_line, prev->orig_col, prev->text());
      LOG_FMT(
          LNEWLINE,
          "%s(%d): next->orig_line is %zu, orig_col is %zu, text() is '%s'\n",
          __func__, __LINE__, next->orig_line, next->orig_col, next->text());
      size_t nl_flag =
          ((chunk_is_newline(prev) ? 1 : 0) | (chunk_is_newline(next) ? 2 : 0));
      ...
      if (mode_local & TP_LEAD) {
        ...
        if (chunk_is_token(next2, CT_PREPROC) ||
            (chunk_type == CT_ASSIGN && chunk_is_token(next2, CT_BRACE_OPEN))) {
          continue;
        }
        ...
        LOG_FMT(LNEWLINE,
                "%s(%d): prev->orig_line is %zu, orig_col is %zu, text() is "
                "'%s', nl_count is %zu\n",
                __func__, __LINE__, prev->orig_line, prev->orig_col,
                prev->text(), prev->nl_count);
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr;
    ...
    if (pc->type == tok) {
      ...
      if (chunk_is_token(pc, CT_CONSTR_COLON)) {
        LOG_FMT(LBLANKD,
                "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() '%s', "
                "type is %s\n",
                __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                get_token_name(pc->type));
        chunk_t *paren_vor_value =
            chunk_get_next_type(pc, CT_FPAREN_OPEN, pc->level);

        if (with_acv && paren_vor_value != nullptr) {
          LOG_FMT(LBLANKD,
                  "%s(%d): paren_vor_value->orig_line is %zu, orig_col is %zu, "
                  "text() '%s', type is %s\n",
                  __func__, __LINE__, paren_vor_value->orig_line,
                  paren_vor_value->orig_col, paren_vor_value->text(),
                  get_token_name(paren_vor_value->type));
      ...
      if (chunk_is_token(pc, CT_COMMA) && pc->level == ccolon->level) {
        LOG_FMT(LBLANKD,
                "%s(%d): orig_line is %zu, orig_col is %zu, text() '%s', type "
                "is %s\n",
                __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
                get_token_name(pc->type));
        chunk_t *paren_vor_value =
...
iarf_e newline_template_option(chunk_t *pc, iarf_e special, iarf_e base,
                               iarf_e fallback) {
...
void do_blank_lines(void) {
  ...
  for (chunk_t *pc = chunk_get_head(); pc != nullptr; pc = chunk_get_next(pc)) {
    ...
    if (prev != nullptr) {
      LOG_FMT(LBLANK,
              "%s(%d): prev->orig_line is %zu, prev->text() '%s', prev->type "
              "is %s\n",
    ...
    if ((options::nl_max() > 0) && (pc->nl_count > options::nl_max())) {
      log_rule_B("nl_max");
    ...
    if ((chunk_is_token(prev, CT_SEMICOLON) ||
      ...
      while (chunk_is_token(tmp, CT_NEWLINE) && chunk_is_comment(tmp->prev)) {
    ...
    if ((chunk_is_token(prev, CT_SEMICOLON) &&
      ...
      if ((options::nl_after_func_proto_group() > pc->nl_count) &&
          next != nullptr && get_chunk_parent_type(next) != CT_FUNC_PROTO &&
    ...
    if ((chunk_is_token(prev, CT_SEMICOLON) &&
      ...
      if ((options::nl_after_func_class_proto_group() > pc->nl_count) &&
          next != nullptr && next->type != CT_FUNC_CLASS_PROTO &&
    ...
    if ((chunk_is_token(prev, CT_SEMICOLON) ||
        ...
        (get_chunk_parent_type(prev) == CT_STRUCT ||
         ...
         get_chunk_parent_type(prev) == CT_CLASS)) {
      auto &opt =
          (get_chunk_parent_type(prev) == CT_CLASS ? options::nl_after_class
      ...
      if (opt() > pc->nl_count) {
        ...
        while ((tmp = chunk_get_prev(tmp)) != nullptr) {
          ...
          LOG_FMT(LBLANK, "%s(%d): %zu:%zu token is '%s'\n", __func__, __LINE__,
                  tmp->orig_line, tmp->orig_col, tmp->text());
    ...
    if ((options::nl_comment_func_def() != 0) &&
        ...
        get_chunk_parent_type(pcmt) == CT_COMMENT_WHOLE && next != nullptr &&
    ...
    if ((options::nl_after_try_catch_finally() != 0) &&
      ...
      if (chunk_is_token(prev, CT_BRACE_CLOSE) &&
        ...
        if (next->type != CT_BRACE_CLOSE && next->type != CT_CATCH &&
    ...
    if ((options::nl_between_get_set() != 0) &&
        (options::nl_between_get_set() != pc->nl_count) && prev != nullptr &&
        next != nullptr) {
    ...
    if ((options::nl_around_cs_property() != 0) &&
        (options::nl_around_cs_property() != pc->nl_count) && prev != nullptr &&
        next != nullptr) {
  ...
  while ((pc = chunk_get_next_nc(pc)) != nullptr &&
    ...
    if ((pc->level != (open_brace->level + 1)) || pc->type != CT_COMMA ||
        (pc->type == CT_COMMA && pc->next &&
         (pc->next->type == CT_COMMENT_CPP || pc->next->type == CT_COMMENT))) {
      continue;
    }
    newline_iarf(pc, av);
  }
...
static void newlines_double_space_struct_enum_union(chunk_t *open_brace) {
  ...
  while ((pc = chunk_get_next_nc(pc)) != nullptr &&
    ...
    if (!chunk_is_comment(prev) && prev->type != CT_BRACE_OPEN &&
...
void annotations_newlines(void) {
  ...
  while ((pc = chunk_get_next_type(pc, CT_ANNOTATION, -1)) != nullptr &&
    ...
    LOG_FMT(LANNOT,
            "%s(%d): orig_line is %zu, orig_col is %zu, annotation is '%s',  "
            "end @ orig_line %zu, orig_col %zu, is '%s'\n",
            __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
...
bool newlines_between(chunk_t *pc_start, chunk_t *pc_end, size_t &newlines,
                      scope_e scope) {
