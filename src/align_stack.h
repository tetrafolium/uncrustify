...
public:
    ...
    StarStyle m_amp_style; //! do not include the first item if it causes it to be
    //! indented
    bool m_skip_first;     //! do not include the first item if it causes it to be
    //! indented

    AlignStack()
        : m_max_col(0), m_min_col(0), m_span(0), m_thresh(0), m_seqnum(0),
          m_nl_seqnum(0), m_gap(0), m_right_align(false), m_star_style(SS_IGNORE),
          m_amp_style(SS_IGNORE), m_skip_first(false), m_last_added(0) {}

    AlignStack(const AlignStack &ref)
        : m_aligned(ref.m_aligned), m_skipped(ref.m_skipped),
          m_max_col(ref.m_max_col), m_min_col(ref.m_min_col), m_span(ref.m_span),
          m_thresh(ref.m_thresh), m_seqnum(ref.m_seqnum),
