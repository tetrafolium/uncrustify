...
chunk_t *align_assign(chunk_t *first, size_t span, size_t thresh,
  ...
  LOG_FMT(LALASS,
          "%s(%d): [my_level is %zu]: start checking with '%s', on orig_line "
          "%zu, span is %zu, thresh is %zu\n",
          __func__, __LINE__, my_level, first->text(), first->orig_line, span,
          thresh);
  ...
  while (pc != nullptr) {
    LOG_FMT(LALASS,
            "%s(%d): orig_line is %zu, check pc->text() '%s', type is %s, "
            "parent_type is %s\n",
    ...
    if (chunk_is_token(pc, CT_SPAREN_OPEN)
      ...
      LOG_FMT(LALASS,
              "%s(%d): Don't check inside SPAREN, PAREN or SQUARE groups, type "
              "is %s\n",
               ...
               && !pc->flags.test(PCF_IN_FCN_CALL)) // Issue #1717
    {
      ...
      vdas.Reset();
               ...
               && !pc->flags.test(
                      PCF_IN_TEMPLATE) // and it is not inside a template #999
      ...
      LOG_FMT(LALASS, "%s(%d): align_assign_decl_func() is %d\n", __func__,
              __LINE__, options::align_assign_decl_func());
      LOG_FMT(LALASS, "%s(%d): log_pcf_flags pc->flags: ", __func__, __LINE__);
      log_pcf_flags(LALASS, pc->flags);

      log_rule_B("align_assign_decl_func");

      if (options::align_assign_decl_func() ==
              0 // Align with other assignments (default)
          && (chunk_is_token(pc, CT_ASSIGN_DEFAULT_ARG) // Foo( int bar = 777 );
              || chunk_is_token(
                     pc, CT_ASSIGN_FUNC_PROTO))) // Foo( const Foo & ) = delete;
      {
        LOG_FMT(LALASS,
                "%s(%d): fcnDefault[%zu].Add on '%s' on orig_line %zu, "
                "orig_col is %zu\n",
                __func__, __LINE__, fcn_idx, pc->text(), pc->orig_line,
                pc->orig_col);
      ...
      } else if (options::align_assign_decl_func() ==
                 1) // Align with each other
      {
        ...
        {
          LOG_FMT(LALASS,
                  "%s(%d): default: fcnDefault[%zu].Add on '%s' on orig_line "
                  "%zu, orig_col is %zu\n",
                  __func__, __LINE__, fcn_idx, pc->text(), pc->orig_line,
                  pc->orig_col);
          ...
          fcnDefault[fcn_idx].Add(pc);
        } else if (chunk_is_token(
                       pc,
                       CT_ASSIGN_FUNC_PROTO)) // Foo( const Foo & ) = delete;
        {
          LOG_FMT(LALASS,
                  "%s(%d): proto: fcnProto.Add on '%s' on orig_line %zu, "
                  "orig_col is %zu\n",
        ...
        } else if (chunk_is_token(pc, CT_ASSIGN)) // Issue #2197
        {
          LOG_FMT(
              LALASS,
              "%s(%d): vdas.Add on '%s' on orig_line %zu, orig_col is %zu\n",
              __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
          vdas.Add(pc);
        }
                 ...
                 &&
                 (chunk_is_token(pc,
                                 CT_ASSIGN_DEFAULT_ARG) // Foo( int bar = 777 );
                  || chunk_is_token(
                         pc,
                         CT_ASSIGN_FUNC_PROTO))) // Foo( const Foo & ) = delete;
      {
        ...
        if (chunk_is_token(pc, CT_ASSIGN)) {
          LOG_FMT(LALASS,
                  "%s(%d): as.Add on '%s' on orig_line %zu, orig_col is %zu\n",
                  __func__, __LINE__, pc->text(), pc->orig_line, pc->orig_col);
          as.Add(pc);
        }
      }
