...
// TODO: This can be cleaned up and simplified - we can look both forward and
// backward!
void brace_cleanup(void) {
	...
	while (pc != nullptr) {
		...
		if (language_is_set(LANG_PAWN) && frm.top().type == CT_VBRACE_OPEN &&
		...
		if (!chunk_is_comment(pc) && !chunk_is_newline(pc) &&
			...
			print_stack(LBCSAFTER,
			            (chunk_is_token(pc, CT_VBRACE_CLOSE)) ? "Virt-}"
			            : pc->str.c_str(),
			            frm);
		}
 ...
 * 'x'    [IF - 1] [PAREN OPEN]
 * ')'    [IF - 2]
 * 'x'    [IF - 2] [VBRACE OPEN]   <- VBrace open inserted before because '{'
 * was not next
 * '--'   [IF - 2] [VBRACE OPEN]
 ...
 * that they are indented independently and do not affect the rest of the
 * program.
 *
...
static void parse_cleanup(ParseFrame &frm, chunk_t *pc) {
	...
	LOG_FMT(LTOK,
	        "%s(%d): orig_line is %zu, type is %s, tos is %zu, TOS.type is %s, "
	        "TOS.stage is %s, ",
	...
	if ((frm.stmt_count == 0 || frm.expr_count == 0) && !chunk_is_semicolon(pc) &&
	    pc->type != CT_BRACE_CLOSE && pc->type != CT_VBRACE_CLOSE &&
	    !chunk_is_str(pc, ")", 1) && !chunk_is_str(pc, "]", 1)) {
		chunk_flags_set(pc, PCF_EXPR_START | ((frm.stmt_count == 0) ? PCF_STMT_START
		                                      : PCF_NONE));
		LOG_FMT(LSTMT,
		        "%s(%d): orig_line is %zu, 1.marked '%s' as %s, start stmt_count "
		        "is %zu, expr_count is %zu\n",
	...
	if (frm.sparen_count > 0) {
		...
		if (chunk_is_token(pc, CT_SEMICOLON) && frm.size() > 2 &&
	...
	if (chunk_is_token(pc, CT_PAREN_CLOSE) ||
		...
		if (pc->type != (frm.top().type + 1)) {
			...
			LOG_FMT(LWARN, "%s(%d): (frm.top().type + 1) is %s\n", __func__, __LINE__,
			        get_token_name((c_token_t)(frm.top().type + 1)));

			if (frm.top().type != CT_EOF && frm.top().type != CT_PP_DEFINE) {
				LOG_FMT(LWARN,
				        "%s(%d): File: %s, orig_line is %zu, orig_col is %zu, Error: "
				        "Unexpected '%s' for '%s', which was on line %zu\n",
				        __func__, __LINE__, cpd.filename.c_str(), pc->orig_line,
				        pc->orig_col, pc->text(), get_token_name(frm.top().pc->type),
			...
			if (chunk_is_token(pc, CT_BRACE_CLOSE) ||
			    ...
			    chunk_is_token(pc, CT_MACRO_CLOSE)) {
				frm.brace_level--;
				LOG_FMT(LBCSPOP, "%s(%d): frm.brace_level decreased to %zu", __func__,
				        __LINE__, frm.brace_level);
	...
	if (frm.top().stage == brace_stage_e::WOD_SEMI) {
		if (cpd.consumed) {
			...
			if (language_is_set(LANG_PAWN)) {
				...
				if (tmp != nullptr && tmp->type != CT_SEMICOLON &&
			...
			if (chunk_is_token(pc, CT_SEMICOLON) ||
				...
				LOG_FMT(LWARN,
				        "%s: %s(%d): %zu: Error: Expected a semicolon for WHILE_OF_DO, "
				        "but got '%s'\n",
	...
	if (chunk_is_token(pc, CT_PAREN_OPEN) || chunk_is_token(pc, CT_FPAREN_OPEN) ||
	    chunk_is_token(pc, CT_SPAREN_OPEN) || chunk_is_token(pc, CT_BRACE_OPEN)) {
		chunk_t *prev = chunk_get_prev_ncnl(pc);
		...
		if (prev != nullptr) {
			if (chunk_is_token(pc, CT_PAREN_OPEN) ||
				...
				if (chunk_is_token(prev, CT_IF) || chunk_is_token(prev, CT_ELSEIF) ||
				    chunk_is_token(prev, CT_WHILE) || chunk_is_token(prev, CT_DO) ||
				    chunk_is_token(prev, CT_FOR) || chunk_is_token(prev, CT_SWITCH) ||
	 ...
	 */
	if (chunk_is_token(pc, CT_BRACE_OPEN) || chunk_is_token(pc, CT_PAREN_OPEN) ||
	    chunk_is_token(pc, CT_FPAREN_OPEN) ||
	    chunk_is_token(pc, CT_SPAREN_OPEN) || chunk_is_token(pc, CT_ANGLE_OPEN) ||
	    chunk_is_token(pc, CT_MACRO_OPEN) || chunk_is_token(pc, CT_SQUARE_OPEN)) {
		frm.level++;
	...
	if (patcls == pattern_class_e::BRACED) {
		frm.push(pc, __func__, __LINE__,
		         (chunk_is_token(pc, CT_DO) ? brace_stage_e::BRACE_DO
	...
	if (chunk_is_token(pc, CT_SQUARE_OPEN) ||
	    ...
	    chunk_is_token(pc, CT_COLON) || chunk_is_token(pc, CT_OC_END) ||
	    (chunk_is_semicolon(pc) && frm.top().type != CT_PAREN_OPEN &&
	     frm.top().type != CT_FPAREN_OPEN && frm.top().type != CT_SPAREN_OPEN)) {
	...
	LOG_FMT(LSTMT,
	        "%s(%d): Mark expression starts: orig_line is %zu, orig_col is %zu, "
	        "text() is '%s'\n",
	        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());
	chunk_t *tmp = chunk_get_next_ncnl(pc);

	if (chunk_is_token(pc, CT_ARITH) || chunk_is_token(pc, CT_ASSIGN) ||
	    chunk_is_token(pc, CT_CASE) || chunk_is_token(pc, CT_COMPARE) ||
	    (chunk_is_token(pc, CT_STAR) && tmp != nullptr && tmp->type != CT_STAR) ||
	    chunk_is_token(pc, CT_BOOL) || chunk_is_token(pc, CT_MINUS) ||
	    chunk_is_token(pc, CT_PLUS) || chunk_is_token(pc, CT_CARET) ||
	    chunk_is_token(pc, CT_ANGLE_OPEN) || chunk_is_token(pc, CT_ANGLE_CLOSE) ||
	    chunk_is_token(pc, CT_RETURN) || chunk_is_token(pc, CT_THROW) ||
	    chunk_is_token(pc, CT_GOTO) || chunk_is_token(pc, CT_CONTINUE) ||
	    chunk_is_token(pc, CT_PAREN_OPEN) || chunk_is_token(pc, CT_FPAREN_OPEN) ||
	    chunk_is_token(pc, CT_SPAREN_OPEN) || chunk_is_token(pc, CT_BRACE_OPEN) ||
	    chunk_is_semicolon(pc) || chunk_is_token(pc, CT_COMMA) ||
	    chunk_is_token(pc, CT_NOT) || chunk_is_token(pc, CT_INV) ||
	    chunk_is_token(pc, CT_COLON) || chunk_is_token(pc, CT_QUESTION)) {
		frm.expr_count = 0;
		LOG_FMT(LSTMT,
		        "%s(%d): orig_line is %zu, orig_col is %zu, reset expr on '%s'\n",
		        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text());
	} else if (chunk_is_token(pc, CT_BRACE_CLOSE) && !cpd.consumed &&
		...
		if (!cpd.unc_off_used && pc->pp_level == file_pp_level) {
			// fatal error
			LOG_FMT(LERR,
			        "%s(%d): Unmatched BRACE_CLOSE\n   orig_line is %zu, orig_col is "
			        "%zu\n",
...
static bool check_complex_statements(ParseFrame &frm, chunk_t *pc) {
	...
	if (frm.top().stage == brace_stage_e::OP_PAREN1) {
		frm.top().stage = (pc->type != CT_PAREN_OPEN) ? brace_stage_e::BRACE2
	...
	while (frm.top().stage == brace_stage_e::CATCH) {
		if (chunk_is_token(pc, CT_CATCH) || chunk_is_token(pc, CT_FINALLY)) {
			...
			if (language_is_set(LANG_CS)) {
				frm.top().stage = (chunk_is_token(pc, CT_CATCH))
				                  ? brace_stage_e::CATCH_WHEN
				                  : brace_stage_e::BRACE2;
			} else {
				// historically this used OP_PAREN1; however, to my knowledge the
				// expression after a catch clause is only optional for C# which has
				// been handled above; therefore, this should now always expect a
				// parenthetical expression after the catch keyword and brace after the
				// finally keyword
				frm.top().stage = (chunk_is_token(pc, CT_CATCH))
				                  ? brace_stage_e::PAREN1
				                  : brace_stage_e::BRACE2;
	...
	if (frm.top().stage == brace_stage_e::CATCH_WHEN) {
		if (chunk_is_token(pc,
		                   CT_PAREN_OPEN)) // this is for the paren after "catch"
		{
	...
	if (frm.top().stage == brace_stage_e::WHILE) {
		...
		LOG_FMT(LWARN,
		        "%s(%d): %s, orig_line is %zu, Error: Expected 'while', got '%s'\n",
		        __func__, __LINE__, cpd.filename.c_str(), pc->orig_line,
		        pc->text());
		LOG_FMT(LBCSPOP,
		        "%s(%d): pc->orig_line is %zu, orig_col is %zu, text() is '%s', "
	...
	if (pc->type != CT_BRACE_OPEN && !pc->flags.test(PCF_IN_PREPROC) &&
		...
		if (language_is_set(LANG_CS) && chunk_is_token(pc, CT_USING_STMT) &&
	...
	if (frm.top().stage == brace_stage_e::PAREN1 &&
	    (frm.top().type == CT_IF || frm.top().type == CT_ELSEIF) &&
	    pc->str.equals(
		    "constexpr")) // FIXME: Take care of the "constexpr" const string.
	{
		return (false);
	}
	...
	if (pc->type != CT_PAREN_OPEN &&
	    ((frm.top().stage == brace_stage_e::PAREN1) ||
	     (frm.top().stage == brace_stage_e::WOD_PAREN))) {
		LOG_FMT(LWARN,
		        "%s(%d): %s, orig_line is %zu, Error: Expected '(', got '%s' for "
		        "'%s'\n",
...
static bool handle_complex_close(ParseFrame &frm, chunk_t *pc) {
	...
	if (frm.top().stage == brace_stage_e::PAREN1) {
		...
		if ((frm.top().type == CT_IF) || (frm.top().type == CT_ELSEIF)) {
			...
			if (next != nullptr && next->type != CT_ELSE) {
				LOG_FMT(LBCSPOP,
				        "%s(%d): no CT_ELSE, pc->orig_line is %zu, orig_col is %zu, "
				        "text() is '%s', type is %s\n",
				        __func__, __LINE__, pc->orig_line, pc->orig_col, pc->text(),
				        get_token_name(pc->type));
				frm.pop(__func__, __LINE__);
		...
		LOG_FMT(LWARN, "%s(%d): %s:%zu Error: TOS.type='%s' TOS.stage=%u\n",
		        __func__, __LINE__, cpd.filename.c_str(), pc->orig_line,
		        get_token_name(frm.top().type), (unsigned int)frm.top().stage);
		cpd.error_count++;
	}
	return (false);
	...
	while (pc != nullptr) {
		...
		if ((options::indent_namespace_limit() > 0) &&
			...
			size_t numberOfLines =
				br_close->orig_line - pc->orig_line - 1; // Issue #2345
			LOG_FMT(LTOK,
			        "%s(%d): br_close->orig_line is %zu, pc->orig_line is %zu\n",
			        __func__, __LINE__, br_close->orig_line, pc->orig_line);
			LOG_FMT(LTOK,
			        "%s(%d): numberOfLines is %zu, indent_namespace_limit() is %d\n",
			        __func__, __LINE__, numberOfLines,
			        options::indent_namespace_limit());
...
static chunk_t *insert_vbrace(chunk_t *pc, bool after, const ParseFrame &frm) {
	...
	if (!pc->flags.test(PCF_IN_PREPROC) && ref->flags.test(PCF_IN_PREPROC)) {
...
bool close_statement(ParseFrame &frm, chunk_t *pc) {
	...
	LOG_FMT(
		LTOK,
		"%s(%d): orig_line is %zu, type is %s, '%s' type is %s, stage is %u\n",
