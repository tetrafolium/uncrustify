...
static void fl_log_frms(log_sev_t logsev, const char *txt,
...
static void fl_log(log_sev_t logsev, const ParseFrame &frm) {
    LOG_FMT(logsev, "[%s] BrLevel=%zu Level=%zu PseTos=%zu\n",
            get_token_name(frm.in_ifdef), frm.brace_level, frm.level,
            frm.size() - 1);
...
int fl_check(ParseFrame &frm, chunk_t *pc) {
    ...
    if (get_chunk_parent_type(pc) != next->type) {
        LOG_FMT(LNOTE,
                "%s(%d): Preproc parent not set correctly on orig_line %zu: got %s "
                "expected %s\n",
    ...
    if (pc->flags.test(PCF_IN_PREPROC)) {
        ...
        if (get_chunk_parent_type(pc) == CT_PP_IF) {
            ...
            if (pp_level == 0) {
                fprintf(stderr,
                        "%s(%d): pp_level is ZERO, cannot be decremented, at line %zu, "
                        "column %zu\n",
            ...
            if (cpd.pp_level == 0) {
                ...
                fprintf(stderr,
                        "%s(%d): #endif found, at line %zu, column %zu, without "
                        "corresponding #if\n",
                        __func__, __LINE__, pc->orig_line, pc->orig_col);
                log_flush(true);
                exit(EX_SOFTWARE);
    ...
    if (txt != nullptr) {
        LOG_FMT(LPF,
                "%s(%d): orig_line is %zu, type is %s: %s in_ifdef is %d/%d, "
                "counts is %zu, frame_count is %zu\n",
                __func__, __LINE__, pc->orig_line,
                get_token_name(get_chunk_parent_type(pc)), txt,
                static_cast<int>(in_ifdef), static_cast<int>(frm.in_ifdef), b4_cnt,
                cpd.frames.size());
        fl_log_all(LPF);
        LOG_FMT(LPF, " <Out>");
        fl_log(LPF, frm);
