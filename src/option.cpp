...
std::string to_string(const std::string &in) { return (in); }
...
//-----------------------------------------------------------------------------
bool is_arg_sep(int ch) { return (isspace(ch) || ch == ',' || ch == '='); }

//-----------------------------------------------------------------------------
bool is_varg_sep(int ch) { return (ch == '.'); }
...
void print_description(FILE *pfile, std::string description,
  ...
  for (std::string::size_type start = 1, length = description.length();
    ...
    if (description[start] == '\n') {
      ...
      fprintf(pfile, "# %s%s", description.substr(start, end - start).c_str(),
              eol_marker);
      start = end;
    }
  }
...
bool process_option_line_compat_0_68(const std::string &cmd,
  ...
  if (cmd == "sp_cpp_lambda_paren") {
    ...
    w("option '%s' is deprecated; use '%s' instead", cmd.c_str(),
      options::sp_cpp_lambda_square_paren.name());
...
void GenericOption::warnIncompatibleReference(const GenericOption *ref) const {
  ...
  w("%s references option %s with incompatible type %s", name(), ref->name(),
    to_string(ref->type()));
}

//-----------------------------------------------------------------------------
template <typename T> bool read_enum(const char *in, Option<T> &out) {
  assert(in);
...
template <typename T> bool read_number(const char *in, Option<T> &out) {
  ...
  if (const auto *const opt = find_option(in)) {
    LOG_CONFIG("%s(%d): line_number is %d, option(%s) %s, ref(%s) %s\n",
               __func__, __LINE__, cpd.line_number, to_string(out.type()),
               out.name(), to_string(opt->type()), opt->name());
...
//-----------------------------------------------------------------------------
template <typename T> void Option<T>::reset() { m_val = m_default; }

//-----------------------------------------------------------------------------
template <typename T> std::string Option<T>::str() const {
  return (to_string(m_val));
}

//-----------------------------------------------------------------------------
template <typename T> std::string Option<T>::defaultStr() const {
...
//-----------------------------------------------------------------------------
template <> option_type_e Option<bool>::type() const { return (OT_BOOL); }
...
//-----------------------------------------------------------------------------
template <> option_type_e Option<iarf_e>::type() const { return (OT_IARF); }
...
//-----------------------------------------------------------------------------
template <> option_type_e Option<signed>::type() const { return (OT_NUM); }
...
//-----------------------------------------------------------------------------
template <> option_type_e Option<unsigned>::type() const { return (OT_UNUM); }
...
//-----------------------------------------------------------------------------
size_t get_option_count() { return (option_map.size()); }
...
void process_option_line(const std::string &config_line, const char *filename,
  ...
  if (cmd == "type") {
    ...
    if (token != CT_NONE) {
      LOG_FMT(LNOTE, "%s:%d set '%s':", filename, cpd.line_number,
              args[1].c_str());
    ...
    for (size_t i = 2; i < args.size(); ++i) {
      ...
      if (lang_name) {
        LOG_FMT(LNOTE, "%s:%d file_ext '%s' => '%s'\n", filename,
                cpd.line_number, args[i].c_str(), lang_name);
      } else {
        OptionWarning w{filename};
  ...
  for (auto &og : option_groups) {
    ...
    for (auto *option : og.options) {
      ...
      if (with_doc) {
        ...
        if (!ds.empty()) {
          fprintf(pfile, "#%s# Default: %s%s", eol_marker, ds.c_str(),
                  eol_marker);
        }
      }
      first = false;
